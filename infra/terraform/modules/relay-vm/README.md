...
- The VM's startup script installs `/usr/local/bin/provision-tenant.sh` and grants the relay service account access to both `platform-admin-key` and `internal-api-key`, so tenant provisioning needs no manual secret export. The same service account has service-scoped, read-only `roles/run.viewer` on `ikaro-backend` only to discover the environment-specific backend URI; it does not receive project-wide Cloud Run administration permissions.
