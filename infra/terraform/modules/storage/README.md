...
**M17-S45** adds a third and fourth rule on this same bucket for *promoted* booking photos (`matches_prefix = ["tenants/"]`): age 60 days → `SetStorageClass: NEARLINE` (a photo viewed months later still loads — Nearline is instant-access, just cheaper storage/pricier reads), then age `var.booking_photo_retention_days` (default **365**, confirmed at `/story-discovery` 2026-08-14 — a business + LGPD retention decision, not a cost-only knob; must be ≥ the tenant dispute/warranty window) → `Delete`. All four rule sets on this bucket are additive and non-conflicting. Deletion leaves the DB's `beforeServicePhotoUrls`/`afterServicePhotoUrls` references dangling by design — no cleanup job; the app degrades gracefully (`apps/web`'s `PhotoTile` component swaps a failed `<img>` load for a localized placeholder). The public bucket carries no age-based rule at all — hotsite/marketing assets are permanent by design (S45 decision, 2026-07-07); a future cost-trim must not "optimize" tenant marketing assets away. This split is safe because featuring a booking photo on the public hotsite gallery *copies* the object into the public bucket (`feature-booking-photo.use-case.ts`) rather than referencing the private object live — the featured copy is a permanent, decoupled editorial choice, unaffected by the source booking photo's later deletion.
