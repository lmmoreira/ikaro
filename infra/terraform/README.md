...
- **A Cloud Run v2 service's default `*.run.app` URL is deterministic: `<service-name>-<project-number>.<region>.run.app`** — not a random per-revision hash. Useful because Terraform cannot reference a resource's own computed output (e.g. `google_cloud_run_v2_service.this.uri`) from within that same resource's own configuration block — for the handful of env vars a service needs pointing at *itself* (an OAuth callback URL, a Pub/Sub push audience), construct the URL directly from the project number + region instead of trying (and failing) to self-reference.
