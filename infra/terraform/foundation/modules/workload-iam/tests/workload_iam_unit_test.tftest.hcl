...
run "workload_iam_preserves_exact_resource_scoped_bindings" {
  ...
  assert {
    condition     = google_pubsub_subscription_iam_member.member["subscriber"].subscription == "ikaro-booking-created-notifier" && google_pubsub_subscription_iam_member.member["subscriber"].role == "roles/pubsub.subscriber" && google_pubsub_topic_iam_member.member["backend_publisher"].role == "roles/pubsub.publisher"
    error_message = "Foundation must preserve the provider-normalized subscription name and exact Pub/Sub resource roles."
  }
