...
run "roles_have_exactly_the_reviewed_permissions" {
  ...
  assert {
    condition     = google_project_iam_custom_role.resource_iam_writer.role_id == "tfFoundationResourceIamWriter" && length(google_project_iam_custom_role.resource_iam_writer.permissions) == 4 && alltrue([for permission in ["secretmanager.secrets.getIamPolicy", "secretmanager.secrets.setIamPolicy", "storage.buckets.getIamPolicy", "storage.buckets.setIamPolicy"] : contains(tolist(google_project_iam_custom_role.resource_iam_writer.permissions), permission)])
    error_message = "The Foundation resource writer must contain only bucket and secret IAM-policy permissions."
  }
