locals {
  ...
  secret_accessors_base = {
    ...
    # turnstile-secret-key (M20-S14): Turnstile's siteverify call moved from the BFF to the
    # backend — the BFF's ALL_TRAFFIC egress has no Cloud NAT, so its outbound call to Cloudflare
    # had no route out; the backend's PRIVATE_RANGES_ONLY egress already bypasses the VPC for
    # public destinations, same as its existing OpenRouter calls. PR1 granted this ahead of PR2's
    # app-code+envs/* change; PR2 is now confirmed live (a real staging submission was verified
    # end-to-end against Cloudflare's real siteverify endpoint, 2026-08-28) — this PR (3 of 3)
    # removes the now-unused grant from `bff` below.
