...
variable "create_relay_vm" {
  description = "Instantiate the on-demand IAP relay VM (TD32). Defaults false — inert until a session needs it. Toggle only via a merged PR (infra/terraform/README.md's pipeline-only-apply rule), never a local terraform apply. Note: the Cloud SQL half has nothing to reach until enable_database flips at S37."
  type        = bool
  default     = false
}
