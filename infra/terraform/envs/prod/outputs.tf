...
output "bff_service_uri" {
  description = "BFF's real *.run.app URI. After the first apply, paste this into bff_real_uri (local.auto.tfvars or terraform.tfvars) and re-apply so GOOGLE_CALLBACK_URL uses the real, reachable URL — then register that same callback URL in the Google Cloud OAuth client configuration. Superseded once S22's edge module + bff.ikaro.online land."
