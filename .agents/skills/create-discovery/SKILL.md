...
description: Run a structured, proactive product-discovery process that produces a docs/discovery/<NAME>/ folder shaped exactly like what /discovery-to-milestone expects to promote later. Two modes, sharing the same downstream steps — Mode A starts from an idea (a one-liner, a longer brief, or nothing but a slug) and interviews across rounds; Mode B ingests an existing, out-of-pattern docs/discovery/ file set (built before this skill existed) and restructures + re-validates it into the canonical shape without losing anything already discovered (a mandatory reconciliation manifest before any write). Throughout both modes, actively adopts and pushes back from four expert lenses at the point each applies — a PM pressure-testing business case/use cases/edge cases, a database expert critiquing the data model's normalization/indexing/aggregate boundaries, a UX expert critiquing prototype state-coverage/consistency/information hierarchy, a tech lead checking the idea against CLAUDE.md's own anti-pattern table and whether it duplicates an existing feature instead of extending it — not passive template-filling or neutral requirement-gathering. Grounds every question in what Ikaro's real codebase and canonical docs actually do today. Never writes code, never writes canonical docs, never writes plan/journey/ directly.
metadata:
  short-description: PM/DB/UX/tech-lead-led discovery process, two modes
---
...
- **As a tech lead / architect:** check the idea against CLAUDE.md §8's anti-pattern table and `docs/ANTI_PATTERNS.md` explicitly, not just the general architecture docs — does the idea as described risk one of those named mistakes (e.g. inventing a new Port+Adapter where one already exists, a BFF route under the wrong public/private controller)? Is this genuinely new, or does Ikaro already have a feature/module/pattern this should extend instead of duplicate (name it, don't just gesture at "similar")? Does it carry a real infra/cost/complexity tail — a new Cloud Scheduler job, a new Pub/Sub topic, a new third-party dependency, a new per-tenant cap/cost-control need the way the chatbot module needed one? Say so plainly even when the user didn't ask.

**Push back, don't just ask.** A discovery session that only collects answers to a checklist isn't doing its job — every lens above exists to actively look for what's wrong, redundant, riskier, or already-solved about the idea as stated, and say so directly, not just to gather requirements neutrally. The user can always confirm, redirect, or override a challenge — but the challenge has to be raised first. Silence on a real risk or a real duplication is a failure of this skill, not politeness.
...
3. **As the tech-lead lens, explicitly:** does the idea, as understood so far, plausibly duplicate an existing feature/module/pattern (name the specific one, don't just note "something similar exists"), or plausibly collide with a named row in `docs/ANTI_PATTERNS.md` / CLAUDE.md §8? This is a real finding to raise in Step 1, not a silent background check.
4. Check `docs/discovery/` for an existing folder covering the same or clearly-overlapping ground — do not silently duplicate; if one exists, stop and ask the user how to proceed.
5. Determine the discovery's slug (kebab-case, e.g. `lead-form-module`). Confirm `docs/discovery/<slug>/` doesn't already exist.
...
Ask a focused set of questions grounded in Step 0's findings, mixing lenses deliberately — not a business-only round:
- **As a PM:** who is the actor (Customer, Guest, Staff, Manager, Platform operator)? What business problem does this solve for a Brazilian car-wash/service-vertical tenant specifically? MVP-now or explicitly-later? Monetization/plan-tier angle, or available to every tenant?
- **As a tech lead:** what's the closest existing feature to model the pattern after (Step 0 item 3) — and does the idea as described genuinely need to be new, or should it extend that instead? Any anti-pattern collision or real infra/cost tail already found?

**Proactively suggest and challenge — don't just collect.** Name 2-3 things a real PM would flag that the idea as stated doesn't specify, *and* at least one technical/architectural concern grounded in Step 0's read of the real code — not generic caution, a specific one. For a "lead-form module" idea: does a submission become a real `Customer` record or stay anonymous/unlinked? Does it need spam/rate-limit protection (a public, unauthenticated write surface)? Does the manager get notified per-submission or only see a dashboard list? Is there an export path? *And*: the nearest existing pattern is the `CONTACT` module's config panel — should this literally extend that module type instead of becoming a new one, given how similar the manager-facing config shape already is?
...
**Architecture fit (a challenge, not just a question)**
4. The `CONTACT` module already has a manager-facing config panel with a very similar shape (`docs/15-HOTSITE_DYNAMIC_ARCHITECTURE.md` § CONTACT). Extending that module type instead of adding a new one would avoid a second, parallel config pattern for the manager to learn — is there a real reason this needs to be genuinely separate, or does it just feel that way because it wasn't scoped against the existing pattern yet?
```
