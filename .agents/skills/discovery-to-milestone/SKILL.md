...
- **PRODUCT-DECISION** — distinct from both: the milestone can still be drafted and a story can still be technically implemented, but a real business/design call is needed before that implementation should actually ship — a stated default is the wrong response (it would silently pick a side), and blocking the draft is too heavy (nothing about drafting the milestone depends on the answer). Carry it into the affected story's AC as an explicit, named open item ("this story's design follows X, not Y — Y needs a product decision, not assumed"), never resolved either direction by this skill. (M20 test-slice precedent, 2026-08-22: a discovery doc's current, correct decision on waitlist-promotion acceptance conflicted with an already-built prototype that predated the decision and was never updated — neither a BLOCKER nor a DEFERRABLE default was the right response.)
...
**Bundling multiple UCs into one story needs a real coupling test, not just a resemblance to a prior story's bundling.** Bundle two UCs together only when they share the actual write boundary — the same `txManager.run()` call, the same aggregate mutation in the same request. Sharing an aggregate *type* is not enough: a read-only UC (browse/list) and an independently-triggered write (a customer joins a waitlist whenever they choose, not as a side effect of another UC) belong in their own stories even when they operate on the same aggregate as a bundled pair, unless splitting them would fragment one real transaction across two stories. "This mirrors how a prior milestone bundled its cap-enforcement layers" is not sufficient justification on its own — name the actual shared transaction boundary for *this* bundle, or split it. (M20 test-slice precedent, 2026-08-22: a story bundled book+cancel+waitlist-join+auto-promote citing a prior milestone's bundling precedent; only cancel+promote actually shared one transaction — book and waitlist-join are independently triggered and share nothing but the aggregate type. Not wrong, but the bundling rationale was weaker than stated.)
...
- **Every promoted aggregate/table earns its place, or says explicitly why not:** cross-check every new aggregate/table from 2b against the story list this step is about to produce — does at least one story actually create/read/write it? A promoted aggregate nothing in this milestone touches isn't automatically wrong (domain-model fidelity to the discovery doc's fuller scope can justify it), but it must be named explicitly in the milestone's Non-Goals (Step 6), not silently carried as if every promoted concept were load-bearing. (M20 test-slice precedent, 2026-08-22: `Resource` was promoted for fidelity, but none of the milestone's 6 stories actually read or wrote it — caught only because this check was applied retroactively during review, not during drafting.)
...
- **Milestone-level integration/E2E test plan:** name the "golden path" scenario(s) that span multiple stories in this milestone (e.g., "customer books a class → gets waitlisted → promoted → sees it in minha-conta"), and which story is responsible for actually implementing that spanning test. Every individual story's test plan can be airtight and the seams between them can still go untested — this decides where that coverage lives. **This scenario is a state-transition narrative by default — also walk it for the observation side:** for every point in the scenario a real actor needs to *see* the resulting state (not just trigger a transition), confirm some story in this milestone actually builds the read/query capability that shows it. A scenario that only chains writes ("book → fills → waitlist → cancel → promote") can read as complete while silently assuming a list/detail view exists that no story here builds. (M20 test-slice precedent, 2026-08-22: the golden path never needed a "customer views their bookings" step to complete its own write-chain narrative, so this exact gap — no "list my class-session bookings" endpoint anywhere in the milestone — went undetected here and surfaced only later, incidentally, while drafting the frontend story that needed it.)
...
Not everything here will have a clean answer, especially for a genuinely large discovery — tag exactly like Step 1 does: **BLOCKER** (must resolve before any story in the affected area is drafted), **DEFERRABLE** (state an explicit default, carry it into the relevant stories' acceptance criteria as something to revisit, never silently baked in), or **PRODUCT-DECISION** (a real business/design call needed before real implementation, but not before this milestone can be drafted — carried into the affected story's AC as a named open item, never resolved by this skill either direction). Every resolved item here — whether decided immediately or via that tagging — joins the same running Decisions log Step 1 started, so Step 4's "bake in pre-decisions" rule picks it up exactly like any other decision.
...
- Spanning E2E test: <scenario> — owned by <story ID> — observation side confirmed: every actor-visible state in the scenario has a supplying read/query story
- Unused-promotion check: <any 2b aggregate/table no story touches, and why it's still promoted — or "none">
```
...
For every story in the confirmed wave sequence, write a `### M<N>-S<NN> — <title>` block using **the canonical story schema — `docs/STORY_SCHEMA.md`**. Load that file now and draft to its skeleton exactly; nothing here should require `/story-discovery` to special-case a discovery-originated story. Do not restate the field list — this step only calls out milestone-specific emphasis on top of the schema:
...
- **Backend HTTP surface is its own explicit line, not implied by the use-case steps or BFF spec alone:** state whether this story needs a new backend-reachable HTTP endpoint (new or extended controller — exact shape deferred to `/story-discovery`, which will grep for the nearest existing precedent, e.g. `ServiceController`/`TenantSettingsController`) or reuses one that already exists, and name which. A BFF endpoint spec with no corresponding backend-surface line is exactly the gap that hit two consecutive M19 stories (S05, S06) — "the backend HTTP controller was entirely unspecified" — caught only later, at story-discovery, both times.
- **A story bundling multiple UCs (per 3a's coupling test) structures "Backend use case steps" as one labeled sub-list per UC** (e.g. `**UC-038 — Book:**` then its numbered steps, `**UC-039 — Cancel:**` then its own), not one flat numbered list mixing steps from different UCs — a reader must be able to tell which steps belong to which UC without inferring it from context.
- **Acceptance criteria — product** — including, explicitly, any DEFERRABLE assumption from the Decisions log that this story's scope touches (state it as a criterion: "assumes FIFO tie-break per Decisions log #2 — not a hidden default").
...
- **Files to create/modify** — enumerate real paths, for every story, without exception (schema requires this; see schema doc for the domain-slice-naming and `.http`-file rules). `/run-batch`'s file-overlap safety check reads this field directly from the plan file to decide whether two stories are safe to implement concurrently — a story with this field silently missing isn't just an incomplete draft, it's a story `/run-batch` can't safety-check at all. For a story several waves out, treat this list as a best-effort snapshot, not a guarantee — `/story-discovery`'s own Step 1 re-verifies it against the live repo at implementation time and corrects it there if earlier waves have since changed the codebase; that re-check is the safety net, not a reason to skip stating it now.
- **New migration / i18n keys / env vars / feature flags** — flag explicitly if this story introduces any.
- **Infra-touching stories** (Terraform/IAM/Pub/Sub/CI-CD) — include the schema's IAM/permissions, Live-verification check, and PR sequence fields; cite the exact row from `infra/terraform/README.md`'s playbook for PR sequence, never re-derive it.
...
**Cross-check the finished story against CLAUDE.md §8's anti-pattern table explicitly, not just the 3 NON-NEGOTIABLE principles above** — loading the doc that documents a rule (e.g. `docs/24-BFF_ARCHITECTURE.md`'s public/private controller split, already in this story's own "Docs to load") does not guarantee the drafted design actually follows it. (M20 test-slice precedent, 2026-08-22: a BFF story cited the correct naming-convention doc in its own "Docs to load" line and still put 3 JWT-gated write routes under `/public/` in a `.public.controller.ts` — caught only by an independent `/docs-audit` dispatch, not by drafting with the doc open. This is exactly why Step 5b's independent dispatch exists and is not optional — but checking the anti-pattern table explicitly here, before that dispatch ever runs, is strictly cheaper than relying on it to catch every instance of this class of mistake.)
...
- **Mechanical supplying-endpoint check, every frontend story:** list every distinct data-read this story's own Description implies (a list, a detail view, a status check). For each, confirm an endpoint that actually returns that data is drafted somewhere in this milestone — this story's own `Backend HTTP surface`/`BFF endpoint spec`, or a dependency's. A read with no supplying endpoint anywhere in the milestone is a **BLOCKER**, stated as one in the story's own AC, not silently assumed to already exist. This is the per-story mechanical complement to 3b's spanning-observability check — 3b catches it at the golden-path-narrative level, this catches it per story in case a read need isn't part of that named narrative at all. (M20 test-slice precedent, 2026-08-22: a Turmas list page's own supplying "list my bookings" endpoint was missing from the whole milestone, not just under-specified in one story — this check, run mechanically per story, would have caught it here instead of during a later audit.)
...
**This is a mechanical computation, not a judgment call — do not eyeball it.** Now that every story has real `Dependencies` and `Files to create/modify` (Step 4): for every pair of stories in the same wave, (1) confirm no `Dependencies:` edge exists between them in either direction, then (2) take the literal, as-written `Files to create/modify` list of each and check for a non-empty intersection — treat any shared path as a disqualifying overlap, including a path one story marks "modify" that the other marks "new" (exactly the shape `class-session.controller.ts` and `class-sessions.ts` took in the M20 test slice — S02 "creates," S03 "modifies," same file, same wave, no `Dependencies:` edge between them, and it was still a real overlap). Only a pair that survives both checks gets listed as a "likely `/run-batch` candidate." "They look unrelated" or "they're for different actors" is not the check — the literal list intersection is.

This directly anticipates `/run-batch`'s own check using this draft's own data, but it is explicitly **not** authoritative — `/run-batch` re-derives it live, against the real plan file and real repo state, at the moment the user actually wants to run a batch. A pairing that looks independent here can stop being so once an earlier wave's real implementation touches a file this draft didn't anticipate — the same staleness Step 4's own "best-effort snapshot" note already warns about for far-out waves. Label it clearly as a preview wherever it's shown (Step 6), never as a green light to skip `/run-batch`'s own check. (M20 test-slice precedent, 2026-08-22: an earlier, non-mechanical pass at this step claimed two same-wave pairs were file-disjoint; both actually shared a file. The preview mechanism is only trustworthy — even as a non-authoritative courtesy — if the computation behind it is literal, not intuited.)

---

## Step 6 — Present the milestone draft

The milestone file's own header must follow the exact shape every existing `plan/M0X-*.md` file uses — mirror `plan/M18-BOOKING-IMPROVEMENTS.md` or `plan/M19-HOTSITE-CHATBOT.md` literally, not just "in spirit": `**Phase:**`, `**Goal:**`, `**Depends on:**`, `**Blocks:**`, `**Design rationale:**` (pointing at the discovery doc, kept as the permanent *why* per Step 2b), a `**Non-Goals**` section (what the discovery doc explicitly deferred/dropped — pull straight from its own Non-Goals/Out-of-scope section, plus any 2b-promoted aggregate/table this milestone's own stories don't touch, per 3b's unused-promotion check), then a `## Build order` table + mermaid graph, before the first story block. This isn't optional polish — it's the same structure every downstream reader (a future `/docs-audit M0X` pass, a future `/story-discovery` session loading "the matching `plan/<milestone>_IMPLEMENTATION_DETAILS_IA.md`") expects to find.

If any PRODUCT-DECISION items exist (Step 1 or 3b), give them a standard, guaranteed home: a `**Known open items**` section immediately after Non-Goals, one bullet per item, each naming which story/stories carry it forward in their own AC. Don't leave a PRODUCT-DECISION item findable only inside a story's prose — it needs one place in the file a reader can scan without opening every story.
