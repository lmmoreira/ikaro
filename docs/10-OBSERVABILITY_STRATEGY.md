...
**Update, 2026-08-06: `batch`/`sending_queue` are back in `config.yaml`.** The 73%-loss measurement above was taken while the sampler bug was still active, making span arrival at the collector artificially sparse — not representative of steady real traffic. Once all the sampler/concurrency/metrics fixes let real traffic through, one-call-per-span started bursting past Cloud Trace's own per-project write-API quota (80 req/s) instead, losing spans with no CPU-throttling involved at all. A small, request-driven batch (`timeout: 1s`, `send_batch_size: 50` — nothing like the old unbounded 5s config) fixes that without reintroducing dependence on an idle-time timer. Full writeup: `docs/ENGINEERING_RULES.md` § Cloud Run CPU throttling, `infra/docker/otel-collector/README.md`.
