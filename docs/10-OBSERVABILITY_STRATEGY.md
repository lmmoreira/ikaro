...
- **Business dashboard** — M17-S54's 6 log-based business counters (bookings requested/approved/completed, notifications failed, customer/staff logins), plus M17-S56's `dashboardFilters` pinned filter on `tenant_id` — pick a tenant in the console toolbar to re-filter all 6 charts live, no Logs Explorer query needed
