...
// Extracted + unit-tested directly (2026-08-05, M17-S34 follow-up, cross-tool review finding on
// PR #326) — concurrencyLimit: 200 had no regression coverage of its own. Sized from the
// empirically measured rejection pattern at the old default of 30 (598 rejections in ~80
// minutes on staging, one burst of 500 in 29 seconds), not a "headroom above the 80-request
// Cloud Run cap" theory, which doesn't hold: the real bound is concurrent span exports, not
// concurrent requests, and a single request can fan out to 20-30 spans.
...
export function bootstrapTracing(
  ...
  const sdk = new NodeSDK({
    ...
    // buildOtlpExporterOptions() (extracted + unit-tested directly, 2026-08-05, M17-S34
    // follow-up, cross-tool review finding on PR #326): builds the `url` fallback and
    // `concurrencyLimit: 200` (sized from the empirically measured rejection pattern at the old
    // default of 30 — 598 rejections in ~80 minutes on staging, one burst of 500 in 29 seconds —
    // not from a "headroom above the 80-request Cloud Run cap" theory, which doesn't hold: the
    // real bound is concurrent *span exports*, not concurrent *requests*, and a single request
    // can fan out to 20-30 spans). Must be re-verified against live staging traffic after
    // deploy; if rejections recur at 200, the fix is a structural backpressure/queueing
    // redesign, not another arbitrary bump. Self-limiting regardless via each export's own
    // timeoutMillis (10s default).
    spanProcessors: [
...
**Update, 2026-08-06: `batch`/`sending_queue` are back in `config.yaml` — as a partial, empirically-validated mitigation, not a confirmed root-cause fix.** The 73%-loss measurement above was taken while the sampler bug was still active, making span arrival at the collector artificially sparse — not representative of steady real traffic. Once all the sampler/concurrency/metrics fixes let real traffic through, a new round of "context deadline exceeded" export failures appeared. The initial theory — Cloud Trace's per-project write-API quota (80 req/s) being burst past — was checked directly against Cloud Monitoring's own quota metrics and **refuted**: zero quota-exceeded events, actual usage under ~21% of the limit at peak. The real mechanism is still unconfirmed. Batching was re-tested live anyway and reduced, but did not eliminate, loss: it fixed the originally-observed multi-request-burst failure mode (71 spans lost in one shot under the old design, 0 lost across a comparable burst here), but a *separate*, low-volume single-request pattern (a lone `/pubsub/push` cron trigger, too small to hit the batch's size threshold) still lost spans via the same timer-dependency mechanism the batch removal was originally about — confirmed live, not just theorized. Extended observation (~2 hours, 363 requests) put the actual rate at 2 loss events total, both early, none since despite continued real traffic — low-frequency and intermittent, not persistent, and even a loss event leaves the trace itself visible (only some child spans go missing). Full writeup, including the corrected quota investigation and the honest rate data: `docs/ENGINEERING_RULES.md` § Cloud Run CPU throttling, `infra/docker/otel-collector/README.md`.
...
> **Historical section below (`### Superseded Grafana design`) — see the file-level banner above for why.** M17-S35 (`infra/terraform/modules/monitoring`, 2026-08-08) implements the real, live mechanism: **two** Cloud Monitoring dashboards per env — Engineering and Business, split in M17-S56 (2026-08-13) — provisioned as Terraform (`google_monitoring_dashboard`), not JSON files auto-loaded by a Grafana instance that doesn't exist.

**Deliberately not duplicated here as a hand-maintained table** — a dashboard-tile inventory drifts the moment someone edits the Terraform locals, the same failure mode already caught once in this repo for `docs/13-DATABASE_SCHEMA.md` (6 tables found stale during a `/docs-audit` sweep). The module's own `main.tf` locals (`service_widgets`, `latency_widgets`, `instance_count_widgets`, `sql_widgets`, `pubsub_widgets`, `business_counter_widgets`, `booking_creation_latency_widget`) are the single source of truth for exactly which tiles exist, and for which of the two dashboards (`engineering_dashboard_widgets`/`business_dashboard_widgets`) each one lands on. Current categories, by name only:

- **Engineering dashboard** — per Cloud Run service (backend/bff/web): request rate/5xx, p99 latency, instance count; Cloud SQL: connections, CPU (only once a database exists — count-gated on `database_instance_name`); Pub/Sub: oldest unacked message age (live queues), DLQ depth (dead-letter queues); M17-S55's GMP `POST /v1/bookings` p99 latency widget
- **Business dashboard** — M17-S54's 6 log-based business counters (bookings requested/approved/completed, notifications failed, customer/staff logins), plus M17-S56's `dashboardFilters` pinned filter on `tenant_id`, intended to let a viewer pick a tenant in the console toolbar and re-filter all 6 charts live instead of running a Logs Explorer query — **pending live staging verification** (cross-tool PR review finding, CodeRabbit, 2026-08-13; tracked in `plan/M17-CLOUD-DEPLOY.md`'s M17-S56 acceptance criteria), since Cloud Monitoring validates `dashboardFilters` server-side only

View live: Cloud Monitoring console → Dashboards, or via `terraform output dashboard_id_engineering` / `terraform output dashboard_id_business` in `envs/<env>` (see either output's own description for the console URL format).
