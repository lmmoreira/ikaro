...
- **Body:**
  ...
  - `durationMinutes` (M23-S02, UC-067) — **required** when the request's basket includes a service with `durationPolicy = CUSTOMER_SELECTED`; ignored otherwise (no effect on a fixed-duration booking).
  - `participantCount` (M23-S02, UC-067/UC-068) — an independent, always-accepted booking-level field, persisted on the booking regardless of whether the basket has a variable-duration or intake-bearing service; also serves as the required answer when the resolved intake schema's `participantCountRequired = true`. It is a capacity/attendee-count input only — it never overrides `ResourceRequirement.requiredQuantity`, which stays the service's static configured value.
...
- **Body:**
  ...
  - `beforeServicePhotoUrls` optional, defaults to `[]`.
  - `resourceSelections`, `durationMinutes`, `participantCount`, `intakeSchemaVersion`, `intakeAnswers`, `consentAccepted`, `attendees` — identical shape, semantics, and validation rules (including the single-variable-service-per-basket restriction) as the guest endpoint above.
...
- `GET /bookings` → List bookings. Query params (`M13-S03`): `status` (comma-separated), `date`, `from`, `to`, `page`, `limit`. **No `customerId` filter param** — for a CUSTOMER caller, scoping to their own bookings is role-derived server-side, not a query filter. Each list item includes `totalPrice`, `totalDurationMins`, and a compact `lineSummary: [{ lineId, serviceId, serviceNameAtBooking, durationMinsAtBooking, priceAtBooking }, …]`. STAFF/MANAGER-facing items (`StaffBookingCardResponse`) additionally include `assignedResources: [{ resourceId, resourceType, resourceName }, …]` — every resource assigned to the booking's lines, sourced from `booking_line_resource_assignments`, unconditional (not filtered by any client-side resource selection; `TD44` Story 2 round 3). Absent/empty when the booking has no resource assignments. Not present on `CustomerBookingListItem`.
