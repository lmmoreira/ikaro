...
- **Layout:** there is no single shared layout for `/dashboard/**` — `apps/web/app/dashboard/layout.tsx` is a bare passthrough (`return <>{children}</>`). Each top-level section owns its own `layout.tsx` (`bookings/`, `hotsite/`, `loyalty/`, `schedule/`, `services/`, `settings/`, `team/`, `leads/` as of `M20-S10`) that independently reads `{ tenantId, tenantSlug, tenantName, userName, role, locale }` from the JWT via `cookies()`/`getAccessToken()`, calls the shared `loadDashboardShellContext(token, payload)` (`apps/web/shells/dashboard/model/dashboard-shell-context.ts`), and renders `<DashboardShell>`. **Updated (`M13-S15`):** the JWT payload was enriched to carry `tenantName`/`userName`/`locale` directly (see `JwtIssuerService`) specifically so shells never need a separate profile/tenant-info fetch — `buildDashboardShellContext`/`loadDashboardShellContext` read these fields straight off the decoded token, with no extra API call. **Corrected (`M20-S10`, story-discovery 2026-08-27):** this doc previously (incorrectly) described a single shared `app/dashboard/layout.tsx` doing this work — it never did; the per-section duplication above is the real, longstanding pattern.
- **Key components (`apps/web/shells/dashboard/components/`):**
  ...
  - `Sidebar.tsx` — logo block, nav items (Agenda, Horários, Serviços, Fidelidade, plus Leads when the `LEAD_FORM` module is enabled — `leadFormEnabled`, `M20-S10`), and a "Somente Gerente" section (Equipe, Configurações, Hotsite) shown only when `role === 'MANAGER'`.
  ...
  - `MoreSheet.tsx` (renamed from `ManagerSheet.tsx`, `M20-S10`) — mobile "Mais" bottom-sheet. Renders a header-less "Leads" item for either role when `leadFormEnabled`, plus the manager-only nav items (Equipe, Configurações, Hotsite) when `role === 'MANAGER'`.
