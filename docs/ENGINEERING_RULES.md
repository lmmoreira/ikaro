...
**Follow-up (2026-08-05, same day): NodeSDK was silently running a metrics export loop against a collector with no metrics pipeline, logging a 404 as an ERROR every cycle, forever.** `bootstrapTracing()` is traces-only by design, but never explicitly disabled metrics — `@opentelemetry/sdk-node`'s `getMetricReadersFromEnv()` falls back to a default OTLP `PeriodicExportingMetricReader` whenever `OTEL_METRICS_EXPORTER` isn't `"none"` and neither `metricReaders` nor `metricReader` is passed. `infra/docker/otel-collector/config.yaml`'s `service.pipelines` only defines `traces:` (metrics deliberately deferred), so every periodic metrics export attempt hit a genuine `OTLPExporterError: Not Found` — the collector's HTTP router has no `/v1/metrics` route registered at all. **Fix:** pass `metricReaders: []` explicitly in `bootstrapTracing()`'s `NodeSDK` config — an empty array is truthy, so it deterministically short-circuits the env-var fallback in code, rather than relying on `OTEL_METRICS_EXPORTER=none` being set in every Cloud Run environment. This is not a stopgap: M17-S35 (Cloud Monitoring dashboards/alerts, not yet built) plans to use Cloud Run built-in metrics + log-based metrics derived from structured logs — "zero app code" per that story's own design — so the OTel SDK metrics path was never actually needed. See `plan/M17-CLOUD-DEPLOY.md`'s M17-S35 section for what would need to change if a future story chooses the Managed Prometheus path instead.

**Follow-up (2026-08-06, next day): a fourth bug appeared once the three fixes above let real, dense traffic finally reach the exporter — Cloud Trace's own per-project write-API quota, not CPU/network, and it reverses the collector-side `batch`/`sending_queue` removal earlier in this section.** Cloud Trace's write API has a documented, per-project quota of 4,800 requests per 60 seconds (80 req/s) — https://docs.cloud.google.com/trace/docs/quotas. With one synchronous export call per span (no batching) and real traffic flowing, an ordinary burst of ~4 dashboard HTTP requests (each fanning out to 15-20+ spans, per `SimpleSpanProcessor`'s design) produced 71 simultaneous export calls to Cloud Trace within ~300ms — an instantaneous rate of ~237 req/s, nearly 3x the quota — causing the exporter's own `timeout: 2s` to trip and all 71 spans to be permanently lost (no retry on a synchronous per-call design). This is a hard external ceiling; more CPU or memory on the collector would not have fixed it, and a coarse ~60s-resolution Cloud Run container CPU metric can't even confirm or rule out a sub-second contention spike as an alternative explanation — don't trust it for this class of investigation.

Crucially, **this doesn't contradict the earlier `batch`/`sending_queue` removal — it reveals that the 73%-loss measurement behind that removal was taken under conditions the sampler bug (fixed the same day as that removal) made unrepresentative.** With the sampler bug still active at the time, real span arrival at the collector was extremely sparse, with long idle/CPU-throttled gaps between spans — exactly the condition under which a timer-driven flush is most likely to land in a throttled gap. That measurement was real, but the traffic pattern behind it wasn't steady real traffic, and doesn't necessarily predict how the same design behaves once traffic is dense.

**Fix:** reintroduce `batch` (`timeout: 1s`, `send_batch_size: 50` — deliberately far below the old 5s timeout and unbounded default) and `sending_queue: enabled: true` in `infra/docker/otel-collector/config.yaml`, coalescing many small export calls into far fewer, larger ones that stay under the write-quota ceiling without depending on the timer ever firing during genuinely idle time — real traffic keeps hitting the batch's *size* threshold long before its `timeout` would matter. Verified live on staging with real traffic over ~34 minutes across two bursts: one dropped batch (32 spans) ~2 minutes after a fresh deploy (consistent with one-time connection/auth warm-up, not a steady-state pattern), then zero failures and 37/37 traces fully intact on a second burst ~20 minutes later, once warm — a clear, repeatable improvement over the no-batch design, which lost spans on two separate occasions under ordinary traffic with no warm-up excuse either time. **Not proven zero-loss forever** — `retry_on_failure` is genuinely rejected as an invalid key by this exporter version (re-confirmed live, 2026-08-06: `'googlecloudexporter.Config' has invalid keys: retry_on_failure`), so `sending_queue: enabled: true` batches exports without adding any actual resilience — a batch that hits the timeout is dropped permanently (logged as `"Exporting failed. Dropping data."` with a `dropped_items` count), never retried. If failures recur under steady traffic (not just after a fresh deploy), the next lever is a smaller `send_batch_size`, not assuming this configuration is safe by default. Full writeup and methodology: `infra/docker/otel-collector/README.md`.
