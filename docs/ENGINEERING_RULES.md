...
**Correction (2026-08-05, later same day): the fix above is real and validated, but does NOT explain the majority of trace loss actually observed in production — treat it as "closes one proven hole," not "solves trace reliability."** A follow-up live investigation, after PR #324 deployed, kept measuring **~75-85% of real traces still missing** from Cloud Trace — statistically unchanged from the ~73% baseline this section opened with. Six further live-production experiments, each reverted after testing, ruled out every remaining infra-level explanation:
- Collector CPU 0.1 → 1 vCPU, and memory 128Mi → 512Mi: no effect
- `cpu-throttling: false` (always-allocated CPU) on the whole `ikaro-backend` service, verified via the collector's own debug exporter showing backend's own child spans still never arriving even fully un-throttled: no effect
- `OTEL_TRACES_SAMPLER_ARG=1.0` forced explicitly on both `ikaro-backend` and `ikaro-bff` (ruling out an under-sampled default): no effect
- A live A/B using the collector's `debug` exporter (added via a runtime `--set` flag override, no rebuild needed) as ground truth, confirmed the collector genuinely never receives these spans — not a slow/failed export, a receive that never happens — with zero errors anywhere `diag.error`/`globalErrorHandler` would have logged one
- The `close`-event-on-keep-alive-connections hypothesis (a real, documented class of bug in `instrumentation-http`) was directly disproven twice: a raw Node `http` + real instrumentation repro (30/30 spans exported over one reused keep-alive connection) and, more importantly, **the full real app stack run locally — real NestJS/Express, real BFF→backend calls via `dev-login`, real keep-alive reuse — exported every single span, zero loss**, under the exact same traffic pattern that loses 75-85% in production

That last result is the important one: it proves the bug is not in this codebase's OTel usage, not in the Node/Express/NestJS request lifecycle, and not fixable by further collector-side tuning — it's something specific to the live Cloud Run environment that hasn't been isolated yet. Full investigation log, all six ruled-out hypotheses with their evidence, and the recommended next steps (build a minimal synthetic Cloud Run repro service; escalate to Google Cloud Support if that isolates it as Cloud-Run-generic) are in `infra/docker/otel-collector/README.md` § Known unresolved issue.
