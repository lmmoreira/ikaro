...
38. **Not fixed, flagged, 2026-08-23 — `resources` rows with `type = STAFF` have no described validation path for `ref_id`.** There's deliberately no DB FK from `booking.resources.ref_id` to `staff.staff` (correct — cross-context, per `CLAUDE.md` §2.4), but nothing in `CAND-01`'s main flow ("manager picks an existing Staff row to wrap") says how the backend confirms that `ref_id` is a real, active Staff row belonging to the same tenant before the `Resource` row commits. Checked directly against the live codebase: `apps/backend/src/contexts/booking/infrastructure/cross-context/` has `booking-customer.adapter.ts` and `booking-platform.adapter.ts`, but no Staff equivalent — so this isn't a "reuse an existing port" case, it's a genuinely new cross-context need. Per `CLAUDE.md` §7's cross-context priority order (events, then BFF orchestration, then Port+Adapter last), the likely right answer is BFF orchestration — the manager's resource-creation screen already has to call Staff Context to populate the "pick an existing Staff member" list, so the BFF can pass a pre-validated `staffId` rather than the Booking Context re-deriving it via a new port. Left as an open implementation-time decision, not resolved here.
39. **Not fixed, flagged, 2026-08-23 — the `class_schedule_templates.recurrence`/`recurring_booking_schedules.recurrence` hot-path risk noted in §9 was written before `RecurringBookingSchedule` existed and was never revisited for it.** §9's own risk note ("how many active templates can one resource realistically have" is an untested load-bearing assumption for JSONB recurrence evaluated on every availability check) predates 2026-08-22's addition of `RecurringBookingSchedule` to the same evaluation path (`CAND-29` step 5, §5's advisory-lock list). A popular shared resource can now accumulate both class-template *and* private-recurrence JSONB rules evaluated per check — the assumption's scope grew without its own re-estimate. Same disposition as §9's original note: not a correctness risk, a sizing assumption worth re-checking at implementation time.
40. **Underspecified, 2026-08-23 — `future_commitment_exceptions`' idempotency unique key (§10, CAND-47 A1: "the same unresolved impact already has an open worklist entry → update/reuse that entry") is asserted but never spelled out as actual columns**, unlike every other constraint in §10, which is documented as "complete persistence contracts... not optional notes." Closed here: `UNIQUE (tenant_id, source_type, source_id, affected_type, affected_id) WHERE status = 'OPEN'` — one open worklist entry per distinct (cause, affected-thing) pair; a second `CAND-47` trigger for the same pair while one is still `OPEN` updates the existing row (impact/deadline/alternatives) instead of inserting a duplicate, matching the worked `fce_1` example in §10.
