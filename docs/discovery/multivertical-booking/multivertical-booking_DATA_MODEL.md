...
A versioned, service-owned definition of booking questions, consent text/version, participant rules and typed field markers such as `PICKUP_ADDRESS`. A new version supersedes, never edits, the previous one, so a past booking's snapshot always resolves against the exact form it was submitted under (`CAND-43` A1). **[Superseded by M22-S04, 2026-09-19 — the shipped intake-schema contract has only `FREE_TEXT`/`BOOLEAN` question types: `PICKUP_ADDRESS` and `NAMED_ATTENDEES` were removed as redundant with the Details-tab pickup-address toggle and the Participantes card's `requiresNamedAttendees`/`participantCountRequired`, and no question type sets `requires_pickup_address` any more. See `docs/02-DOMAIN_MODEL.md` (intake schema) and `docs/04-USE_CASES.md` UC-054.]**
...
| questions | JSONB | NOT NULL — ordered `[{ fieldKey, label, type, required }]`; `type` covers generic input shapes (`FREE_TEXT`, `NAMED_ATTENDEES`) and business-specific typed markers the booking layer projects into an already-typed column — `PICKUP_ADDRESS` doesn't add a new column, it drives `bookings.pickup_address`, which already exists for car wash's "leva e traz" (`docs/13-DATABASE_SCHEMA.md:227`); this discovery generalizes that mechanism to any service, not just car wash **[Superseded by M22-S04, 2026-09-19 — the shipped intake-schema contract has only `FREE_TEXT`/`BOOLEAN` question types: `PICKUP_ADDRESS` and `NAMED_ATTENDEES` were removed as redundant with the Details-tab pickup-address toggle and the Participantes card's `requiresNamedAttendees`/`participantCountRequired`, and no question type sets `requires_pickup_address` any more. See `docs/02-DOMAIN_MODEL.md` (intake schema) and `docs/04-USE_CASES.md` UC-054.]** |
...
`questions` for this row: a `PARTICIPANT_COUNT` field (min 1, max 8 — "Sala Aurora comporta até 8 pessoas"), a `NAMED_ATTENDEES` field ("Participantes nomeados") and a `FREE_TEXT` field ("Necessidades de acesso"). This tenant requires *both* a count and named attendees on this one service — the domain doc's "only a count, or named rows" describes the range a service can configure, not that every service must pick exactly one end of it. **[Superseded by M22-S04, 2026-09-19 — the shipped intake-schema contract has only `FREE_TEXT`/`BOOLEAN` question types: `PICKUP_ADDRESS` and `NAMED_ATTENDEES` were removed as redundant with the Details-tab pickup-address toggle and the Participantes card's `requiresNamedAttendees`/`participantCountRequired`, and no question type sets `requires_pickup_address` any more. See `docs/02-DOMAIN_MODEL.md` (intake schema) and `docs/04-USE_CASES.md` UC-054.]**
...
36. **Two mechanisms for the same fact, reconciled 2026-08-22.** A `PICKUP_ADDRESS`-typed intake-schema question (§2 above) and the legacy `services.requires_pickup_address` boolean (already live for car wash, `docs/13-DATABASE_SCHEMA.md:207`) both express "this service needs a pickup address," and nothing said which one is authoritative. Resolved: `requires_pickup_address` stays the single source of truth that `bookings.pickup_address` population is validated against; declaring a `PICKUP_ADDRESS` question in a service's intake schema (via the new `CAND-09b`) sets that boolean in the same transaction rather than existing as a second, independently-driftable switch. **[Superseded by M22-S04, 2026-09-19 — the shipped intake-schema contract has only `FREE_TEXT`/`BOOLEAN` question types: `PICKUP_ADDRESS` and `NAMED_ATTENDEES` were removed as redundant with the Details-tab pickup-address toggle and the Participantes card's `requiresNamedAttendees`/`participantCountRequired`, and no question type sets `requires_pickup_address` any more. See `docs/02-DOMAIN_MODEL.md` (intake schema) and `docs/04-USE_CASES.md` UC-054.]**
...
> **Descoped at implementation (M21-S01 story discovery, 2026-09-01):** removed from Cluster 1 scope — `Resource.reactivate()` publishes no event for now, since nothing consumes it and an event drained into the outbox with zero real subscribers gets no Pub/Sub topic from the auto-generated catalog (see `docs/ENGINEERING_RULES_BACKEND.md` § Aggregate domain events → outbox, M20-S16 precedent). Kept here as the historical candidate shape; revisit if/when a real consumer need emerges.
