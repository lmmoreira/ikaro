...
> **Relocado 2026-08-21.** Este arquivo vivia em uma pasta `ux-handoff/` separada, ao lado de `plan/` e `docs/`. Consolidado em `docs/discovery/multivertical-booking/` para que toda a discovery viva num único lugar do repositório. Os HTMLs referenciados abaixo agora estão em `../prototype/` (não mais como irmãos deste arquivo) — os links relativos abaixo refletem a estrutura original e podem não resolver diretamente; use `../prototype/index.html` para navegar pelas telas reais. `trial_slots` (§5 abaixo) e a reposição de turma fixa (`RECORRENCIA_HANDOFF.md`) foram reconciliados contra `multivertical-booking.md`/`_DATA_MODEL.md`/`_USECASES.md` nessa mesma data — ver essas fontes para o estado final, não este documento histórico.
...
**O que é:** companion doc para o `multivertical-booking.md`. Não é roadmap — é discovery. Define como o onboarding pode abstrair os 13 modelos técnicos em 6 tipos de negócio que qualquer dono entende.
...
Esta decisão foi incorporada ao `multivertical-booking.md` e ao modelo de dados; este handoff registra sua origem em UX.
...
- **Pool de instrutores (opcional):** seção ao final do form com um multiselect de "Instrutores elegíveis". Padrão vazio — o usuário ignora se não quiser definir agora. Um instrutor selecionado = atribuição fixa. Dois ou mais = pool intercambiável, alinhado ao conceito de `eligibleResourceIds` no `multivertical-booking.md`.
