...
29. **Deep-review pass (2026-08-23) — `CAND-45`'s recurring-schedule approval bypass confirmed as an intentional business decision, not a gap.** Raised during a fresh four-lens review: every occurrence a `RecurringBookingSchedule` generates auto-confirms as `APPROVED` regardless of the service's own `default_approval_mode` (§9 item 33 above), and creating the schedule itself never routes through `MANUAL_APPROVAL` either — only a resource-conflict check (`CAND-45` A1). This means a customer can bypass a `MANUAL_APPROVAL` service's review gate entirely by requesting a recurring schedule instead of a one-off booking. **Confirmed as accepted product behavior:** the business is deliberately choosing this tradeoff — a customer opting into recurrence is opting into a faster flow, and skipping per-occurrence review for a standing commitment is the intended benefit, not an oversight. No guardrail (minimum occurrence count, review-on-create) is being added. `CAND-09c` (service booking policy) is updated to name this interaction explicitly, so a manager configuring a `MANUAL_APPROVAL` service understands what enabling recurrence eligibility on it means.
...
31. **Milestone-sequencing map, 2026-08-23 — four dependency-ordered clusters, recorded here for `/discovery-to-milestone`'s Phase B to apply directly rather than re-derive.** This is story/wave order (dependency sequencing), not a claim that each cluster ships independent end-user value — a backend/config cluster landing before the flow that consumes it is normal (`CLAUDE.md` §9: "sequence backend/BFF-only stories in an early wave before any frontend story that depends on them"). Every `CAND` is assigned to exactly one cluster except five that build incrementally across clusters and three never built under their own number — both noted below the table. Full audit against every `CAND-XX` heading in `multivertical-booking_USECASES.md` (68 headings, `01`–`56` plus lettered variants) confirmed complete coverage, 2026-08-23.
...
- **LGPD/PII retention and anonymization policy for this discovery's new surface** — the detailed policy is a separate platform concern, but ownership is not optional: the new PII-bearing tables must follow the existing Booking retention/anonymization policy, and the milestone must verify that policy exists before launch. No discovery-specific deletion mechanism is invented here.
