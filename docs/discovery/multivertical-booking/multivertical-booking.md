...
Private appointment no-show is in scope as a simple terminal `NO_SHOW` outcome after the scheduled end time. Customer reschedule is atomic, supports bundles/journeys, recalculates the quote before confirmation, and preserves an audit link/revision history.
...
| `ClassSessionBooking` | Reservation/contact and access/charge-intent snapshot. Guest states include verification/approval; attendee-level results live in child rows. |
...
3. **Resolved (2026-08-24): rolling-horizon generation uses a 15-minute idempotent worker and a 90-day platform default.** A service may configure a shorter horizon as an operational/cost choice; the worker recomputes missed windows safely, and customers can book only materialized sessions. Cross-family resource conflicts remain recurrence-rule checks and do not depend on a session row existing.
...
5. **Resolved — cancellation notifies customers.** No refund/credit workflow exists because Ikaro does not process payments; a closed session cannot be cancelled. Any manually reported charge is handled outside this discovery.
...
26. **Resolved (review): Ikaro does not process payments in this discovery.** Bookings snapshot quotes; rescheduling may reprice with an append-only quote revision. Staff may record a manually reported amount, method, collector and correction note at session close-out for operational visibility, but this is not a payment gateway, settlement ledger or reconciliation system. Online charges, refunds, credits and automatic financial penalties remain deferred.
...
29. **Deep-review pass (2026-08-23) — `CAND-45`'s recurring-schedule approval bypass initially confirmed as an intentional business decision, not a gap.** Raised during a fresh four-lens review: every occurrence a `RecurringBookingSchedule` generates auto-confirms as `APPROVED` regardless of the service's own `default_approval_mode` (§9 item 33 above), and creating the schedule itself never routes through `MANUAL_APPROVAL` either — only a resource-conflict check (`CAND-45` A1). This means a customer can bypass a `MANUAL_APPROVAL` service's review gate entirely by requesting a recurring schedule instead of a one-off booking. At the time, this was accepted as a deliberate tradeoff — a customer opting into recurrence opting into a faster flow — with no guardrail added. **Superseded 2026-08-28 — see item 32 below.** A follow-up review found the specific abuse path this pass never actually put to the business: create a schedule, let it generate one occurrence, then cancel the schedule (`CAND-45` A2) — a zero-friction way to obtain an `APPROVED` booking on a service explicitly configured to require review, repeatable at will. Closed via schedule-level approval, not accepted as-is.
30. **Deep-review pass (2026-08-23) — per-tenant staged-rollout feature-gating dropped from the migration plan.** `multivertical-booking_DATA_MODEL.md` §4 step 5 previously hedged "feature-gate multi-resource configuration... per-tenant if rollout is staged," which has no supporting mechanism (this platform's feature flags are env-var/deployment-wide, per `CLAUDE.md` §1 — there is no per-tenant flag system, and none is being built for this). **Confirmed:** the product is pre-production, so no staged/canary per-tenant rollout is needed — the migration is a single, all-tenants-together cutover once step 4's validation passes. See that document's §4/§6 for the corrected step.
31. **Milestone-sequencing map, 2026-08-23 — four dependency-ordered clusters, recorded here for `/discovery-to-milestone`'s Phase B to apply directly rather than re-derive.** This is story/wave order (dependency sequencing), not a claim that each cluster ships independent end-user value — a backend/config cluster landing before the flow that consumes it is normal (`CLAUDE.md` §9: "sequence backend/BFF-only stories in an early wave before any frontend story that depends on them"). Every `CAND` is assigned to exactly one cluster except five that build incrementally across clusters and three never built under their own number — both noted below the table. Full audit against every `CAND-XX` heading in `multivertical-booking_USECASES.md` (claimed 68 headings, `01`–`56` plus lettered variants) confirmed complete coverage, 2026-08-23. **Addendum 2026-08-28:** `CAND-45b` was added after this audit (item 32 above) and assigned directly to Cluster 3 alongside `CAND-45`, its parent flow.

**Correction, `/docs-audit` pass, same date — two real gaps in this note, not one.** (1) A live recount (`grep -c "^### \*\*CAND-"`) puts the total at **68 headings including `CAND-45b`** — meaning the 2026-08-23 baseline of "68" was already off by one before this addition (67 real headings existed then, not 68); the arithmetic in the original addendum above was wrong. (2) More substantively: programmatically expanding every literal ID and numeric range in the table above (including the `21`–`28`/`32`–`40` shorthand) against the actual heading list found `CAND-22b`, `CAND-23b`, and `CAND-25b` were never swept into Cluster 4 at all — the range shorthand only expands bare numbers, so a lettered variant sitting between two ranged endpoints (e.g. `22b` between `21` and `28`) silently falls through unless listed explicitly. All three are Group E session-booking CANDs, unambiguously Cluster 4 by content; added to that row's CAND list above. Re-verified after the fix: every one of the (now) 68 real headings is covered by exactly one of {a cluster's CAND list, the "builds incrementally" list, the "never built under their own number" list} — checked programmatically, not just re-read.

    | Cluster | Scope | CANDs |
    |---|---|---|
    | **1 — Foundation** | `Resource` aggregate: create/edit/deactivate/reactivate, Staff-as-`Resource` wrapping + its deactivation cascade, resource-scoped closures/openings, resource list view. No dependents yet — the migration's LOCATION-backfill (`multivertical-booking_DATA_MODEL.md` §4 step 2) rides along here even though it touches `services`/`service_resource_requirements` schema Cluster 2 owns, since every existing service needs its default LOCATION requirement inserted before Cluster 2's requirement CRUD is meaningful. | `CAND-01`, `02`, `03`, `03b`, `04`, `05`, `41`, `52` |
    | **2 — Service extensions + availability/exclusivity engine** | `Service` gains `bookingModel`, resource requirements, bundles, legs, buffer, intake schema, booking policy (APPOINTMENT branch only); the availability query and same-family overlap rejection that reads all of it. Neither half is independently verifiable without the other — a resource requirement with no availability engine reading it has no observable effect, and the engine has nothing to compute against without the schema. `CAND-13c` (combined multi-resource day grid) belongs here despite sitting under Group C by document position — its own text says it's "the same per-resource availability query as `CAND-29`, run across several resources," no class dependency at all. | `CAND-06`, `07`, `08`, `09`, `09b`, `09c`, `10` (APPOINTMENT branch only), `13c`, `29` (base), `30`, `31` (base, same-family) |
    | **3 — Customer/guest appointment booking + extensions** | The actual new booking capability customers/guests use, consuming 1–2: chosen/auto-assigned/fungible/bundled/legged booking, variable-duration reservations, intake submission, reschedule, private recurring reservations, availability alerts, future-commitment worklist (appointment-triggered), appointment no-show, tenant bootstrap for non-class presets. | `CAND-16`, `17`, `17b`, `18`, `19`, `20`, `42`, `43`, `44`, `45`, `45b`, `46`, `47` (base), `48`, `51` (Presets A/B/C/G), `53`, `56` (base) |
    | **4 — Classes/sessions** | The new materialized-session machinery, reusing 1–3: templates, sessions, bookings, attendees, waitlist offers, contracts, recurring enrollment, roster/attendance/close-out, guest verification/approval, class presets. Deliberately last, so cross-family exclusivity (model #13) is proven against real Cluster 1–3 traffic before session-family load is added on top. | `CAND-10` (SESSION branch), `10b`, `11`, `12`, `13`, `13b`, `14`, `15`, `21`–`28`, `22b`, `23b`, `25b`, `32`–`40`, `49`, `50`, `51` (Presets D/E/F), `54` |

    **Builds incrementally across clusters, not a gap:**
    - `CAND-10` — fork exists as a schema field from Cluster 2; SESSION branch only reachable once Cluster 4 ships `CAND-11`.
    - `CAND-29` — base in Cluster 2; gains the private-recurrence check when `CAND-45` ships (Cluster 3); gains the class-template check when `CAND-11` ships (Cluster 4).
    - `CAND-31` — same-family exclusion provable in Cluster 2; the cross-family assertion (model #13 — this discovery's central premise) isn't actually testable until Cluster 4 exists alongside 2/3.
    - `CAND-47`/`56` — base handles appointment-triggered causes in Cluster 3; gains class-template-triggered causes in Cluster 4.
    - `CAND-51` — same wizard shell; Presets A/B/C/G ship in Cluster 3, D/E/F attach in Cluster 4.

    **Never built under their own number:** `CAND-15b` (superseded by `CAND-37`), `CAND-55` (superseded by `CAND-38`). Appointment no-show is now included in `CAND-48` and must be sequenced with the appointment lifecycle.

    **Blocking bug found and fixed while validating this order:** `CAND-01`'s precondition previously required a `Service` already configured with a non-`LOCATION` resource requirement — circular against `CAND-06`'s own precondition, and exactly the kind of thing that would have silently broken "Foundation first" at implementation time. Fixed in `multivertical-booking_USECASES.md` — see that entry.

    **Batch-sequencing constraint, added 2026-08-28:** Cluster 1's LOCATION-backfill story and Cluster 2's first story can never share a `/run-batch` batch, despite sitting in different clusters — the backfill migration touches `services`/`service_resource_requirements`, schema Cluster 2 also writes to (see Cluster 1's own row above). `/run-batch` requires zero file overlap between every pair of stories in a batch (`CLAUDE.md` §9), checked live against this plan, not assumed from cluster/wave membership alone — different-cluster is necessary but not sufficient for batch-safety here.

    **LGPD/PII retention** — explicitly deferred, not addressed by this sequencing. See §11.

32. **Resolved 2026-08-28 — `CAND-45`'s recurring-schedule approval bypass (item 29) closed via schedule-level approval, not accepted as-is.** The create-then-cancel loophole (create a schedule purely to obtain one auto-confirmed occurrence past a `MANUAL_APPROVAL` gate, then cancel it) is closed at its root: creating the schedule is now itself the review gate, so repeating create-then-cancel just re-triggers that same gate every time — there is no way to launder around it. Mechanism: when the service's effective approval mode is `MANUAL_APPROVAL`, a new `RecurringBookingSchedule` is created `PENDING_APPROVAL` (still resource-conflict-checked per `CAND-45` A1) and generates **no occurrences** until a staff member approves or rejects it in one action (`CAND-45b`, mirroring `CAND-34`'s shape but scoped to the schedule, not a session). Approval → `ACTIVE`, generation begins from approval time. Rejection → `CANCELLED`, reason `APPROVAL_REJECTED`. An unresolved request past the service's `manual_hold_minutes` auto-cancels with reason `APPROVAL_EXPIRED`, the same hold-expiry mechanic `Booking` already has. `AUTO_CONFIRM` services are unchanged — the schedule still creates `ACTIVE` directly. Once a schedule is `ACTIVE`, every occurrence it generates still auto-confirms without further review, exactly as item 33 already established — only the one-time creation gate is new. `CAND-09c` step 3's manager-facing explanation is updated to describe one-time schedule-level review instead of no review at all. See `multivertical-booking_DATA_MODEL.md` §2/§6 item 42 for the schema change and `multivertical-booking_USECASES.md`'s `CAND-45`/`CAND-45b` for the full flow.
...
- **Online billing/subscription management** — deferred, same as credit-passes above. `CONTRACT`, `GUEST_TRIAL` and pay-per-class are eligibility/access decisions only; Ikaro does not charge, settle, refund or reconcile money here. A manually reported amount may be recorded during close-out for operational visibility, but it is not a payment gateway or financial ledger.
- **LGPD/PII retention and anonymization for this discovery's new surface** — fully out of scope for this discovery, not a launch gate this document owns. **Corrected 2026-08-28** — an earlier version of this bullet said the milestone must verify an existing retention/anonymization policy before launch; checked directly, no such policy currently exists for `bookings`' contact PII at all. `docs/26-DATA_RETENTION_AND_PRIVACY.md` covers only booking photos and hotsite assets. Subject-rights export/deletion for that existing PII, and for every new PII-bearing table this discovery adds (`class_session_bookings`, `class_session_booking_attendees`, `recurring_booking_schedules`, etc.), is entirely `M17-S51`'s responsibility (`plan/M17-CLOUD-DEPLOY.md`) — itself gated on go-live (`M17-S37`), still pending. This discovery does not attempt to close that gap or gate its own promotion on it.
...
4. **Manual charge-record boundary:** online billing remains deferred. Close-out must record an append-only externally reported charge outcome (`PAID`, `UNPAID` or `WAIVED`) for every payable guest or authenticated pay-per-class booking, with amount when applicable, method, collector, time, and correction/reversal reason; Ikaro does not process the charge.
...
13. **Appointment no-show:** an appointment may transition to terminal `NO_SHOW` only after its scheduled end time. STAFF and MANAGER may mark it; MANAGER may correct it with an append-only audit transition. No-show emits a dedicated event, sends a retryable notification, awards no loyalty points and does not emit completion. Class no-show remains an attendee-level attendance outcome.

14. **Resolved (2026-08-24): session generation policy.** A retryable, idempotent worker runs every 15 minutes and materializes sessions 90 days ahead. The platform owns the default; a service may configure a shorter horizon. Missed windows are recovered on the next run, and customers can book only within the materialized horizon.
15. **Resolved (2026-08-24): waitlist offer duration.** The tenant configures the offer duration, defaulting to 24 hours, with a hard platform maximum of 48 hours. An offer always expires at session start at the latest.
16. **Resolved (2026-08-24): Staff resource validation.** Booking performs final validation through a narrow Staff lookup adapter/use-case contract: same tenant, existing, active and schedulable. The BFF supplies the picker/read experience but is not the authoritative write validator. Staff remains unaware of Booking; `StaffDeactivated` is propagated through the event bus.
17. **Resolved (2026-08-24): session event architecture.** `ClassSession` and `ClassSessionBooking` are full aggregate roots whose events are drained through the transactional outbox. Notification and loyalty consumers react asynchronously; delivery failure never rolls back the committed booking state.
18. **Resolved (2026-08-24): recurring local time.** Recurrence rules are authored in the tenant timezone, occurrences are persisted as UTC, and display converts back to the tenant/customer locale. A nonexistent local time is skipped and logged; an ambiguous repeated local time uses the earlier occurrence.
19. **Resolved (2026-08-24): payment boundary.** Ikaro does not process payments. A payable reservation must be explicitly recorded at close-out as paid, unpaid or waived; staff may enter the reported amount and method, but gateway charging, refunds, credits, settlement and reconciliation are deferred.
20. **Resolved (2026-08-24): canonical vocabulary.** The domain and API source of truth uses `Service`, `ClassScheduleTemplate`, `ClassSession`, `ClassSessionBooking` and `RecurringEnrollment`. `ClassType` and `Enrollment` may be read-model/presentation labels only and must not become persistence aggregates.
