...
**Companion doc:** `multivertical-booking.md` — os modelos de agendamento que este documento mapeia para linguagem de negócio.
...
O modelo de agendamento descrito em `multivertical-booking.md` suporta 13 configurações distintas. Expor qualquer parte dessa taxonomia diretamente ao usuário no onboarding produziria uma experiência de planilha, não de produto.
...
| **Sala / Coworking / Locação por Tempo** | Salas de reunião, coworking, quadras, vagas | Modelos 4, 7, 9 + reserva de duração variável |

> Os números de modelo referem-se à tabela §2 de `multivertical-booking.md`. Modelos 7 (bundle) e 8 (etapas sequenciais) são configuração avançada disponível em qualquer preset, não ativados por padrão.
...
**Protótipo de descoberta:** `prototype/manager-14-onboarding-preset.html` cobre a escolha, as perguntas mínimas e a revisão do bootstrap. É uma validação de UX, não uma tela comprometida com implementação.
