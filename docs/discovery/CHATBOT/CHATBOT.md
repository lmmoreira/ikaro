...
§8.9's global spend circuit breaker already accommodates this with no rework needed — it groups its cost calculation by `model_id`, not a flat platform-wide rate, so a tenant using a pricier overridden model already contributes its *actual* cost to the platform total correctly (§8.9's `MODEL_PRICING` lookup already keys by model, not by "the one active provider").
...
6. Anthropic direct as a third adapter, or keep the initial adapter set to just OpenRouter (primary) + Vertex (fallback)? **Now has a concrete driver, not just a hypothetical**: §4/§5's per-tenant `llmProvider`/`llmModel` override (e.g. testing Claude Fable for one tenant) isn't actually usable without this adapter existing — the registry can only offer providers that are actually built.
...
8. **OpenRouter prompt-caching support for this model — confirmed via OpenRouter's own docs, not yet empirically re-verified (partial resolution).** Their docs state caching is automatic for DeepSeek models, no configuration needed, cache hits discounted to 0.1x input rate and visible via `usage.prompt_tokens_details.cached_tokens` in the response. An attempted live test (send the identical system prompt twice, check for `cached_tokens > 0`) was blocked by an API-key auth hiccup mid-discovery and not retried — so this is one step short of this discovery's own standard for everything else (real API calls, not just docs). §3's cost estimates deliberately assume *no* caching (the conservative case) until this gets the same empirical treatment as the reasoning-effort setting did.
...
**Status: prototype built (`CHATBOT/prototype/`, 7 screens + index) and eval run (`CHATBOT/eval/`, 19 questions + a 7-question adversarial pass, both against the real API).** Remaining before this is fully ready for `/discovery-to-milestone`: §10's still-open items — `chatbot_messages` retention policy (§10.3), the cap-reached notification email decision (§10.4), sign-off on the third-adapter question (§10.6), confirmation of `CHATBOT_MIN_PROVIDER_BALANCE_USD`'s proposed $2 value (§8.10), an empirical (not just docs-based) prompt-caching check (§10.8), and deciding an owner/cadence for re-running the adversarial eval (§10.9). None of these block understanding the design — they're implementation-detail decisions, not open architectural questions.
