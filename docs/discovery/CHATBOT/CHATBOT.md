...
**Rough monthly cost estimate**, 50 tenants × 20 conversations/day × 5 turns, ~2,000 input + 200 output tokens/message, `low` effort forced: **~$35–45/month at list price across every tenant combined**, likely lower once the static per-tenant knowledge block is cached (§6) — provider-side prompt caching for OpenRouter models is adapter-specific and not yet confirmed available the same way Vertex/Anthropic caching is; a §10 item, not assumed here. Either way this is noise next to the existing Cloud Run + Cloud SQL cost tables.
...
- `anthropic-llm.adapter.ts` — candidate, since this codebase already has an Anthropic relationship

Selected via env var (`CHATBOT_LLM_PROVIDER=openrouter|vertex-ai|anthropic`), bound in the module with `useClass` — **never `useExisting`**, which the anti-pattern table already flags as a trap that silently instantiates the wrong class when a token is overridden (relevant here specifically because tests will want to swap in a fake `ILlmProvider`).
...
2. **Cost/UX defaults, not self-service-editable but each individually overridable per tenant** — the seven caps (§8: `maxConversationsPerDay`, `maxConversationsPerIpPerDay`, `maxConcurrentConversations`, `maxMessagesPerConversation`, `maxMessageLengthChars`, `maxHistoryMessagesSentToLlm`, `maxOutputTokensPerResponse`) plus `maxKnowledgeTextLength` (§6) — all eight follow the identical `tenant.settings.chatbot?.X ?? DEFAULT_X` pattern: a code constant default, with an optional explicit override in a specific tenant's row when Ikaro grants one (e.g. a premium contract). The default lives in code; the *override*, when one exists, lives in the database — both true at once, not competing answers.
3. **Model/provider technical parameters, tied to the adapter implementation** — the `reasoning.effort = "none"` setting in `openrouter-llm.adapter.ts` (§4) and the `MODEL_PRICING` lookup (§8.9). Code constants; doesn't make sense as anything looser since they only mean something in the context of the code that interprets them.
4. **Operational break-glass — the one exception, and deliberately not in the same file.** `CHATBOT_GLOBAL_DAILY_SPEND_LIMIT_USD` (§8.9) stays an env var, not a code constant, for the opposite reason #1 needs friction: if costs are genuinely spiraling in production, ops needs to lower this *fast* — an env var (Cloud Run config) changes in minutes, a code constant needs a full deploy cycle. Same "platform-wide constant" idea, but two different risk profiles pointing to two different mechanisms on purpose, not an inconsistency.
...
1. **Not available** — the widget renders nothing at all. Two different mechanisms produce this same outcome, worth keeping distinct:
   ...
   - **Module enabled, but currently unavailable** — any of: the tenant's daily or concurrency cap (§8) already exhausted before this visitor even opened the widget, the LLM provider failing a health check, **or the platform-wide daily spend circuit breaker (§8.9) already tripped** — the last one meaning every tenant's widget shows "not available" simultaneously, platform-wide, not just the one tenant that happened to push spend over the line. None of this can be known from the cached manifest (same reasoning as keeping `knowledgeText`/caps out of it), so it needs a small, uncached, always-fresh pre-flight call the widget makes on mount (`GET /public/platform/chatbot/status?tenantSlug=...` → `{ available: boolean }`) — the same endpoint checks all four conditions, one consistent mechanism rather than the global breaker producing a different, messier failure experience than the tenant-level ones.
...
**Platform-wide backstop (not a per-tenant setting):**
...
**How today's platform-wide spend is actually computed.** Price isn't uniform across models (§3), so this can't be a flat `SUM()` — it groups by model first, then applies each model's rate in application code:
```sql
SELECT model_id, SUM(input_tokens) AS total_in, SUM(output_tokens) AS total_out
FROM chatbot_messages
WHERE created_at >= CURRENT_DATE
GROUP BY model_id;
```
Each `model_id`'s totals get multiplied by that model's `MODEL_PRICING` rate and summed across models — that's today's real estimated spend, across every tenant combined. This runs before any new session is created anywhere, so it's on the hot path, platform-wide, not an occasional background check.
...
**Every message logs real token usage** (`chatbot_messages.input_tokens`/`output_tokens`/`model_id`, straight from the adapter's `ChatCompletionResult`) — this is the audit trail for actual per-tenant spend, not just a usage-count proxy, and is the foundation for a future "spend by tenant" report (ties into the BI-layer ambition in CLAUDE.md's business context).
...
3. **`chatbot_messages` retention policy** — it stores real visitor/bot conversation text (§5), not just metadata. Keep indefinitely, auto-purge after N days, or truncate `content` after some window while keeping token counts for the cost audit trail? Needs a real decision before this ships, not a default assumption.
4. `ChatbotDailyCapReached` → tenant-admin notification email — worth building in the same milestone as the cap itself, or a fast-follow?
...
6. Anthropic direct as a third adapter, or keep the initial adapter set to just OpenRouter (primary) + Vertex (fallback)?
...
8. **OpenRouter prompt-caching support for this model** — not yet confirmed one way or the other (§3, §4). Needs checking before the "likely lower once cached" cost estimate in §3 can be trusted; if unsupported for this model/provider route, the realistic monthly estimate is the uncached one.
9. **Adversarial testing needs a recurring cadence, not a one-time pass (§9).** `CHATBOT/eval/` currently reflects a single point-in-time result — re-run it whenever the model, provider, or system prompt structure changes, and periodically regardless (new jailbreak techniques emerge continuously; a 2026-08-07 pass says nothing about 2027). No owner or cadence decided yet — worth deciding before this becomes "tested once, trusted forever" by default.
...
**Next step:** once §10's open questions are resolved, build `CHATBOT/prototype/` — illustrative hotsite screens with the widget in all three states from §7's "Widget States & Failure UX," in both `bubble` and `inline` variants — reusing `plan/journey/manager/prototypes/hotsite` as the visual/structural base rather than starting from scratch, before this gets promoted into a real milestone via `/discovery-to-milestone`.
