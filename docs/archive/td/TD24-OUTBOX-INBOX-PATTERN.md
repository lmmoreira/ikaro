...
| D17 | **Refines D7/D15 further: notification's multi-recipient dispatch (`dispatchTemplatesToMany`) claims per-recipient, not per-batch** — `consumer_name` composition becomes `${notificationType}:${channel}:${recipientEmail}` for this path (AUD-004 item 3, 2026-07-17). `consumer_name` sized `VARCHAR(400)` **from `CreateSharedInbox`'s original migration** (squashed again, same pre-production rationale as D16 — no environment had run the narrower `VARCHAR(150)` version) rather than a follow-up `ALTER TABLE`. | The original per-batch claim meant one failing recipient's `unclaim` forced a full-batch retry, re-sending to every recipient including ones that already succeeded. Per-recipient claiming makes an already-succeeded recipient a cheap `tryClaim`-false skip on redelivery — no re-render, no re-dispatch — while the genuinely-failed recipient(s) retry normally. Dispatch within the batch is sequential and continue-on-error (attempt every recipient in the pass; throw once at the end only if ≥1 failed), so healthy recipients aren't held hostage by one bad address. Proved against real Postgres in `send-booking-requested-notification.use-case.integration.spec.ts`. |
...
D6's original design injected `EVENT_BUS` directly — **superseded by D14** the same day: every repository actually drains through `OUTBOX_PUBLISHER` via the shared helper, as the last step of `save()` (inside the ambient transaction):

```ts
// shared/infrastructure/outbox/drain-domain-events.ts — extracted to avoid one copy per
// repository (SonarCloud duplication gate)
await drainDomainEvents(aggregate, this.outboxPublisher);
```

- The event-emitting aggregates today: `Booking`, `Staff`, `Tenant`, and `LeadFormSubmission` (added M20-S16) — the pattern is part of the "new aggregate" recipe in `docs/ENGINEERING_RULES.md`, which stays the up-to-date count as more aggregates are added.
...
- Events constructed outside aggregates (§A2) keep calling `outboxPublisher.publish()` explicitly — which is now an outbox write.
...
- [ ] Outbox/inbox lag observable via structured logs (unpublished count, oldest-unpublished age, GC deletions, publish failures) — the alert path is `M17-S35`'s `outbox_backlog` policy (`infra/terraform/modules/monitoring/main.tf`), mostly live in staging as of 2026-08-08 (see `docs/18-RELEASE_LIFECYCLE_OPERATIONS.md` for exact status; see S05 notes above on `M17-S33`/`M17-S35`).
