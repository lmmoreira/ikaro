...
## Status
- **State**: Resolved (2026-07-14) — Stories 1-17 all done. Story 17 (final wave) shipped the CI-enforced catalog↔translation exhaustiveness test (`apps/web/shared/lib/i18n/error-codes-exhaustiveness.spec.ts`), replaced the `ANTI_PATTERNS.md` `violations[]`-only-branching entry with the raw-error-text rule, synced `docs/ENGINEERING_RULES.md` (new § Exception handling & i18n pattern), `docs/24-BFF_ARCHITECTURE.md` (Error Passthrough Contract), and `docs/14-API_CONTRACTS.md`, and fully rewrote `docs/25-ERROR_CATALOG.md` (it previously documented a contradicting, never-implemented `type`-URI-fragment scheme). The closing `bad-smell-audit` pass across all three apps found 45 raw findings; each was individually verified against the actual code before acting — 34 (25 backend `makeXxx()` test-mock helpers, 9 web fetcher-naming) were confirmed false positives from checks miscalibrated against this codebase's actual patterns/architecture and were left as-is; the remaining real findings were fixed: a dead BFF `uploads/` controller (already flagged as dead in `td/TD22-ORPHANED-UPLOAD-CLEANUP.md` and never removed) was deleted, `bookings.controller.ts`'s date-range/pagination query building and duplicated role check were extracted to a small pure-function util, a trivial backend string-trim duplication was fixed, and 3 redundant `app/dashboard/loyalty/**` unit specs (E2E already covers that route) were removed. **Story 18 (2026-07-14, follow-up):** also done — discovery found no OTel SDK exists yet (`M17-S33` still pending), so the story was rescoped to ship `error.code` as a structured `AppLogger` field on every non-2xx response in both apps via a matching `ErrorInterceptor` in each. Post-implementation review found and fixed a real asymmetry: the BFF's interceptor already converted truly-unhandled errors into an RFC 9457 `ProblemDetail`, but the backend had no equivalent mechanism at all and fell through to NestJS's default non-compliant 500 shape — backend's `ErrorInterceptor` now does the same conversion, closing a standing gap against CLAUDE.md §1's error-shape invariant rather than just logging around it. That fix then made the two apps' interceptors near-identical, which SonarCloud correctly flagged as new-code duplication (11.7%, required ≤3%) — resolved by extracting the shared logic into `packages/nestjs-http`. CodeRabbit's review then caught a structural bug in the interceptor-based design itself: NestJS runs Guards *before* Interceptors, so an interceptor-based `catchError` can never see exceptions thrown by a Guard (confirmed real — BFF alone has 4 global guards throwing code-bearing exceptions) — fixed by converting the whole mechanism from an `Interceptor` to an `ExceptionFilter` (`BaseErrorFilter`/`ErrorFilter`, `@Catch()` + `APP_FILTER`), the correct NestJS primitive for a true pipeline-wide catch-all. The true OTel span attribute is deferred to `M17-S33`, which now carries a cross-reference note. All 18 stories are now done.
...
**Backend/BFF asymmetry found and fixed (2026-07-14, post-implementation review):** the initial version only logged errors that were already `HttpException`s with a `code`. The BFF's existing `ErrorInterceptor` already had a second branch — converting any truly unhandled (non-`HttpException`) error into a synthetic 500 `ProblemDetail` — but the backend had no equivalent at all (confirmed zero `@Catch`/`ExceptionFilter`/`APP_FILTER` anywhere), so an unhandled backend exception fell through to NestJS's default, non-RFC-9457 500 shape (`{"statusCode":500,"message":"..."}`, no `code`) — a standing gap against CLAUDE.md §1's "Errors: RFC 9457 Problem Details on all non-2xx" invariant, and invisible to this story's own logging. `mapXxxError`'s own documented fallback (`if (err instanceof Error) throw err;`) confirms this path is real, not hypothetical. Backend's interceptor was given the same unhandled-error branch: convert to a proper `ProblemDetail` (`code: AuthErrorCode.INTERNAL_ERROR`, the same shared framework-fallback code the BFF already uses — no new code minted) and log it via `AppLogger.error()`.

**Cross-app duplication found and fixed (2026-07-14, SonarCloud):** writing the same interceptor logic twice (once per app) tripped SonarCloud's new-code-duplication gate (11.7% on new code, required ≤3%) — correctly, since the two files were now near-identical. Extracted the shared logic into `packages/nestjs-http` (already the designated "shared NestJS HTTP infrastructure for backend and bff" package), typed against `BaseAppLogger` from `@ikaro/observability` (added as a new dependency of `@ikaro/nestjs-http`, following the same abstract-base/thin-per-app-subclass pattern `BaseAppLogger`/`AppLogger` itself already established). Each app's error handler is now a 3-line subclass that only wires in its own `AppLogger` instance; the full branch-coverage test suite lives once in the shared package, and each app's own spec is a one-line wiring smoke test. No behavior change — same logic, one physical copy instead of two.

**Interceptor-cannot-see-Guard-exceptions bug found and fixed (2026-07-14, CodeRabbit review on PR #139):** the shared logic above was originally implemented as a NestJS `Interceptor` (`catchError` on the RxJS response pipe). CodeRabbit correctly flagged that NestJS's request pipeline runs `Middleware → Guards → Interceptors → Pipes → Controller` — Guards execute *before* any Interceptor's `intercept()` is even invoked, so an interceptor-based `catchError` structurally cannot see exceptions thrown by a Guard. Verified against this codebase directly: the BFF alone registers 4 global guards (`JwtAuthGuard`, `TenantGuard`, `RolesGuard`, `ActiveStaffGuard`, all in `apps/bff/src/app.module.ts`), each of which throws code-bearing `HttpException`s via `throwProblemDetail` — every one of those responses (a very common error category: auth/role/tenant-mismatch rejections) was silently invisible to this story's whole logging mechanism, directly undermining its own first acceptance criterion. Fixed by converting the shared class from an `Interceptor` to a NestJS `ExceptionFilter` (`@Catch()`, registered via `APP_FILTER` instead of `APP_INTERCEPTOR`) — filters are the actual terminal catch-all for the entire pipeline (Middleware, Guards, Interceptors, Pipes, Controllers), which is what "every error response" requires. Renamed throughout for accuracy: `BaseErrorInterceptor` → `BaseErrorFilter` (`packages/nestjs-http`), `ErrorInterceptor` → `ErrorFilter` in both apps, moved out of each app's `shared/interceptors/` into a new `shared/filters/` directory. Verified end-to-end via the BFF's real component-test pipeline (real guards, real `TestingModule`, supertest) — confirmed a genuine structured JSON log line (`context: "ErrorFilter"`, carrying `code`) is now produced through the actual HTTP stack, not just an isolated unit test. Two other CodeRabbit findings on the same review were checked and are not applicable: using `SystemErrorCode` instead of `AuthErrorCode.INTERNAL_ERROR` for the generic fallback — that type doesn't exist in `packages/types/src/error-codes.ts`, and `AuthErrorCode.INTERNAL_ERROR` is this codebase's own documented "framework/generic fallback" convention (§ Exception handling & i18n pattern, `docs/ENGINEERING_RULES.md`), already used by the BFF's pre-existing code before this story touched it; and a doc-edit-permission nudge that CodeRabbit couldn't know was already satisfied through the actual conversation.

**Acceptance criteria:**
- [x] Every error response with a `code` produces a structured log entry carrying that code — `ErrorFilter` in both apps logs `code`/`path`/`method` via `AppLogger`, `warn` for 4xx and `error` for 5xx, and (unlike the original interceptor-based design) genuinely covers every layer of the request pipeline including Guards
- [x] No sensitive `params` values are logged — only `code`, `path`, `method` are logged; `params`/`detail` are never included
- [x] Truly unhandled (non-`HttpException`) errors are also observable and RFC 9457-compliant in both apps — backend previously had no conversion mechanism at all; now matches BFF's existing pattern
- [x] No cross-app code duplication — shared logic lives once in `packages/nestjs-http`'s `BaseErrorFilter`, verified by SonarCloud's new-code-duplication gate
- [x] Errors thrown by Guards (auth/role/tenant rejections) are logged and RFC 9457-compliant, not just errors from Interceptors/Controllers — verified via a dedicated unit test and a real end-to-end component test through the actual guard chain

**DoD:** Additive observability plus two real fixes: backend's unhandled-error response shape now matches the RFC 9457 `ProblemDetail` contract everywhere else in the codebase, and the mechanism itself is now structurally capable of observing every error in the pipeline (not just the subset that happened to reach an Interceptor) — both closing standing gaps rather than working around them. True OTel span attribute for `error.code` deferred to `M17-S33`.
