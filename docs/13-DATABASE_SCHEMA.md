...
- **Race closed via advisory lock (M21-S03, PR #460 round 4):** the delete-vs-create race the rule above targets — `RemoveScheduleOpeningUseCase` reading "no dependents yet" concurrently with `OpenScheduleUseCase` reading "tenant-wide opening exists" — is closed by both use cases acquiring the same per-`(tenant_id, date)` `pg_advisory_xact_lock` (`ITenantDayLockPort`) before their respective check, inside the write transaction. Whichever side wins the lock fully determines what the other one sees once it proceeds: a concurrent delete either sees the not-yet-committed create (dependents check finds it, blocks) or the create sees the already-committed delete (`TenantOpeningRequiredError`) — never a stale read of both.
- **Race closed via a second, tenant-scoped advisory lock (M21-S03, PR #460 round 7):** the tenant-window bound could still be invalidated by a concurrent `PATCH /tenants/settings` narrowing `businessHours[day]` — `businessHours` was read once by the controller (from `RequestContext`, sourced from the `platform` context's `Tenant` aggregate) before `OpenScheduleUseCase` ever ran, so a settings change mid-request could leave the window-bound check validating against a stale value. Closed by extracting the advisory-lock primitive into a generic, cross-context-neutral `ITenantLockPort` (`apps/backend/src/shared/ports/tenant-lock.port.ts`, promoted out of `booking`'s own `application/ports/` once a second context needed it — mirrors `ITransactionManager`'s placement in `shared/`, with neither `booking` nor `platform` depending on the other) with two methods: `lockTenantDay(tenantId, date)` (the per-date lock above) and `lockTenantSettings(tenantId)` (new — not date-scoped, since `businessHours` isn't tied to one calendar date). `UpdateTenantSettingsUseCase` (platform) acquires `lockTenantSettings` before persisting; `OpenScheduleUseCase` acquires the same lock and re-fetches `businessHours` fresh (via the existing `IBookingPlatformPort.getBusinessHoursAndLocale`, already used by `CreateTenantLocationResourceUseCase`) before its authoritative day-closed and window-bound checks — the original `businessHours` snapshot only powers a fast, non-authoritative pre-check now. (Originally declined at round 4 as disproportionate cross-context coordination for a rare, same-tenant, self-correctable race; reconsidered and built at round 7 once the shared-port extraction made it a small addition rather than new machinery.)
