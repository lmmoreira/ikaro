...
- **Known, accepted risk (not fixed): the tenant-window bound itself can still be invalidated by a concurrent `PATCH /tenants/settings` narrowing `businessHours[day]`.** `businessHours` is read once by the controller (from `RequestContext`, itself sourced from the `platform` context's `Tenant` aggregate) before `OpenScheduleUseCase` ever runs — re-validating it under the advisory lock above would require `platform`'s tenant-settings-update code to also acquire the same lock before writing, a genuine cross-bounded-context coordination this story does not add. Declined deliberately: both sides of this race are rare, manual, same-tenant staff/manager actions (not a customer-facing hot path), and the worst case is a resource-scoped opening whose window no longer matches the tenant's just-changed hours — a soft, easily-correctable inconsistency, not data loss or a cross-tenant issue. (Noted M21-S03, PR #460 round 4 — Codex's review raised this as an Important finding alongside the dependents-race above; that half was fixed via the lock, this half was declined with this reasoning.)
