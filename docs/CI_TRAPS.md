...
| New env var added to `env.validation.ts` passes unit tests but fails a job that boots the **real** app with `Invalid input: expected string, received undefined` (or a boot-smoke-test crash) | The real sync point is not always `pr-tests.yml`. BFF component tests boot the real `AppModule` via `apps/bff/src/test/component-test.helpers.ts`'s `createTestApp()`, which sets every required var itself (`TEST_ENV_KEYS`) — there is deliberately no `env:` block for that job in `pr-tests.yml`, since it would always be shadowed (verified by unsetting all of them locally: 12/12 suites still pass). Backend integration tests build a `TestingModule` directly and never call `validateEnv()`, so only vars a given spec's code path actually reads need setting (`integration-global-setup.ts` for the cross-cutting ones, individual specs for narrower ones like `PLATFORM_ADMIN_KEY`). `pr-security.yml`'s boot-smoke-test step is the one place that *does* read the workflow's literal `env:` — a real `docker run` has no test code to override it | Add the new var to whichever is real for your case: `component-test.helpers.ts`'s `TEST_ENV_KEYS` (bff component tests), the relevant integration spec or `integration-global-setup.ts` (backend integration, only if actually read on the path under test), or `pr-security.yml`'s smoke-test step `env:` (backend/bff boot) |
