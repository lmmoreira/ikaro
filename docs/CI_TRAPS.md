...
| A required check name (e.g. `Trivy Image Scan (backend)`) never satisfies even though the job visibly ran/skipped in the Actions UI | Job-level `if:` combined with `strategy.matrix` — GitHub evaluates the job-level `if:` *before* matrix expansion, so a skipped matrix job collapses into ONE synthetic check reported under the raw, un-templated `name:` string (e.g. `Trivy Image Scan (${{ matrix.service }})`) instead of the N per-entry named checks branch protection is waiting for | Never combine a job-level `if:` with a `strategy.matrix` whose expanded names are individually required checks. Move the skip condition to each *step* instead — the `checkov` job earlier in this same file already does this for a non-matrix job (no job-level `if:`, a step-level `dorny/paths-filter` gate on the one real work step). The job always runs, so the matrix always properly expands; skipped-work steps just finish in seconds reporting success under the correct name. Real incident: TD27 PR #185, 2026-07-22 — `trivy-scan`'s job-level `if:` made the PR permanently `BLOCKED` (`mergeStateStatus`) despite every other check green. |
