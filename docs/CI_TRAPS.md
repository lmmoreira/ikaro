...
| `POST /cron/loyalty-expiry` intermittently returns 500 (`QueryFailedError`, FK violation on `FK_balance_expiry_log_entry`) | `ExpirePointsJob` (renamed from `ExpirePointsUseCase`) reads via global `findExpiringBefore()` (by design, no tenant filter — CLAUDE.md §3); a test in another spec file can transiently create a `loyalty_entries` row with a near-now past `expiresAt` that's globally visible. If that row is deleted (`afterEach`) before the cron's later `markProcessed` INSERT runs, the FK constraint fails | Any "read-now, write-later" global cron use case must re-verify each row still exists (`existsById()`) immediately before its dependent write — see `ExpirePointsJob.processGroup()` |
| `cron-loyalty.controller.integration.spec.ts` fails CI-only (reproduces in CI, not locally even after 6+ local attempts — isolated and full-suite), asserting a `LoyaltyBalanceEntity` row is `undefined` that the test itself just `.save()`d | Same architectural root as the row above (global, unscoped `findExpiringBefore()` scanning the whole shared integration DB) but a different symptom: this file's 4 top-level `it()` blocks shared one hardcoded `CUSTOMER_ID` constant across every test, so any two tests' balance/entry rows collided on the exact same `(tenant_id, customer_id)` identity — the FK-violation fix above (`existsById()`) only guards the entry-existence check, not this. Investigated twice (M17-S34, 2026-08-05, diagnostics added and removed without a full fix; then 2026-08-26) — the precise interleaving was never pinned down either time, only ruled out as a `maxWorkers: 1`-violating race, a thrown/swallowed exception (job logs showed zero errors both times), or a builder/ID collision | Give every test in a file exercising a global cron job its own unique id (`uuidv7()`) for any row identity the job's global scan could touch — never share a hardcoded customer/entity id across `it()` blocks in the same file, even under one shared tenant. The nested `describe('POST /cron/loyalty-expiry-warning', ...)` block in this same file already did this correctly (`const inlineCustomer = uuidv7();` per test) before the top-level tests were fixed to match |
...
| `docker/init-db.sh` (or any psql bootstrap script) fails with `syntax error at or near ":"` after switching a hardcoded password to a psql variable | psql's client-side `:'var'` interpolation is suppressed inside dollar-quoted (`$$ ... $$`) regions — treated as verbatim text, the same way it skips interpolation inside `''`/`E''` string literals. A `CREATE USER ... WITH PASSWORD :'pw'` line embedded inside a `DO $$ BEGIN ... END $$;` block never gets substituted; the literal `:'pw'` text reaches Postgres and fails to parse | Move the interpolation to a plain top-level statement instead, outside any dollar-quoted block: `SELECT format('CREATE USER x WITH PASSWORD %L', :'pw') WHERE NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'x') \gexec` — `\gexec` executes the generated SQL text as a separate statement, sidestepping the DO-block restriction entirely (M17-S27 precedent, 2026-07-24: the first attempt embedding `:'migrator_pw'` directly inside a `DO $$ $$` block broke `integration-global-setup.ts`'s bootstrap step, caught by re-running the integration suite) |
| A Cloud Run job/service log line reads `injected env (N) from .env // tip: <something> [some-domain.com]` or similar, styled like an odd/suspicious injected instruction | This is `dotenv` v17's own real, unmodified source (`lib/main.js`'s `TIPS` array) — a hardcoded list of self-promotional one-liners it randomly appends to its own startup log line, including real entries advertising the maintainer's other products (`dotenvx.com`, `vestauth.com`). Not tampering, not a compromised dependency, not prompt injection — verified against the actual installed package source, not just the log text (M17-S27 precedent, 2026-07-24: initially treated as a possible injection attack until traced to `dotenv@17.4.2`'s literal `_getRandomTip()` function) | Don't spend time auditing the dependency tree over this specific string shape — grep `node_modules/dotenv/lib/main.js` for `TIPS` to confirm it's the same list before treating any variant of this line as suspicious. Passing `{ quiet: true }` to `dotenv.config()` suppresses this (and all other) dotenv log output — every call site in this repo does this to keep production/CI logs clean |
...
**This is a deliberate, budget-driven tradeoff, not an oversight.** Trivy Image Scan already provides substantial per-PR overlap for the same class of finding — it independently caught the nodemailer CVE that first surfaced this gap. A dependency CVE merged via a PR now surfaces at most nearly 7 days later (the next Monday 08:00 UTC run — `weekly-jobs.yml`'s `cron: '0 8 * * 1'`) instead of blocking that PR outright.
...
| An override targets a sub-dependency pinned by a *direct* dependency (e.g. `next>sharp`) | The direct dependency may pin below the fix deliberately, not just conservatively — don't assume a same-major-version bump is automatically safe | Check whether the direct dependency's own declared range already permits the fix (e.g. `^5.3.4` allows `5.10.1`) — if so, it's just a stale lockfile resolution and a plain override is safe. If the range excludes the fix (0.x caret semantics: `^0.34.5` excludes `0.35.0`), read the fix version's actual changelog for breaking changes, then grep the direct dependency's own installed `dist/` for usage of anything removed/changed before trusting the override — don't reason from the CVE description alone. Real incident: TD27 PR #185, 2026-07-22 (`fast-xml-parser` was a safe stale-lockfile case; `sharp` needed the full verification before overriding past Next's own pin) |
...
| A `-r <preload-file>` flag documented in `package.json`'s own `"start"` script silently never runs in the real deployed container, with zero error anywhere | The Dockerfile's own `CMD` is a **separate, independent** command from `package.json`'s scripts — `CMD ["node", "dist/main.js"]` never invokes `"start"` at all, so any preload flag that script documents is pure documentation unless the Dockerfile's `CMD` *also* includes it. The boot smoke test above does **not** catch this: the process still starts and stays up fine either way, since the missing preload just means a module never gets patched/initialized, not a crash | `CMD` must explicitly include every preload flag `package.json`'s `"start"` describes, e.g. `CMD ["node", "-r", "./dist/tracing.js", "dist/main.js"]` — never assume the two are in sync just because one documents the other. Real incident: M17-S34, 2026-08-04 — `apps/{backend,bff}/Dockerfile` had `CMD ["node", "dist/main.js"]` since M17-S33 (no `-r ./dist/tracing.js`), so the OTel SDK never started in any real deployment; `docker build`+Trivy+the boot smoke test all passed the entire time because the container booted and served traffic normally, just without tracing. Only surfaced once a real collector sidecar existed to reveal the silence (M17-S34) — verify any `-r`/preload-flag Dockerfile change by actually `docker run`-ing the built image and confirming the preloaded module's *effect* (not just that the process stays up), not by trusting `package.json` and the Dockerfile agree. |
...
**Moved code can be misreported as "new" duplication:** SonarCloud's PR-diff `new_duplicated_lines_density` metric is computed against the diff, not against whether the duplicated *content* is actually new. Extracting an existing inline block into a brand-new file (e.g. splitting a controller's inline Zod schemas into a sibling `*.schemas.ts` file) makes every line of that block show up as "added" in the diff, even though the text itself already existed pre-move and was already duplicated against the backend's equivalent schema before the extraction. Don't trust the PR-scoped percentage alone when a finding coincides with a pure move/rename — verify with the SonarCloud API (`/api/duplications/show` for the file, `/api/measures/component_tree` comparing the PR branch vs. `main`) whether the flagged lines were duplicated on `main` already. If they were, apply the cross-app exception above deliberately; do not add a broad exclusion or treat the move itself as the source of the duplication.
...
**A stale local branch can make a bot review flag files as wrongly "deleted" that were never touched.** If another session merges new files to `main` while your feature branch is still based on an older commit, a cross-tool reviewer (Codex/CodeRabbit) diffing your branch against the *current* `main` can report those files as removed by your PR — they're not; your branch's base just predates their creation. The check isn't "does the finding look plausible" (it will) — it's whether your branch has actually merged the latest `main` yet. Run `git fetch --unshallow origin` first if the worktree is a shallow clone (`git merge origin/main` otherwise fails with `fatal: refusing to merge unrelated histories`), then `git merge origin/main` and re-push before re-triaging the finding.

**A bot review's claim that a required doc update is "missing from this PR" only checked this PR's own diff — it did not check whether the doc is already correct on the current `main`.** A doc update that landed earlier (e.g. during story-discovery, committed and merged before the feature branch's implementation work even began) shows zero diff in the implementation PR, because there's nothing left to change — not because the doc was skipped. Verify against `main`'s real current content, not the PR diff: `gh api repos/<org>/<repo>/compare/main...<commit-sha>` returns `{"status": "identical"}` when that earlier commit (and everything in it) is already part of `main`, which answers the question directly and doesn't require a full `git fetch --unshallow` just to check whether a specific commit is already there. M22-S01 precedent, PR #479 round 9, 2026-09-15: Codex flagged a migration adding 5 new tables as missing a matching `docs/13-DATABASE_SCHEMA.md` update "in the same PR" — the doc already fully documented every table and column the migration created, column-for-column, landed in an earlier story-discovery commit confirmed identical to `main`'s tip via the API compare above. Declined with that evidence rather than adding a no-op doc change to satisfy the finding.
...
If a future story adds a *second*, differently-configured Turnstile widget (a real production sitekey exercised against a different environment), re-verify this assumption for that specific sitekey before reusing the hidden-input wait — this behavior is a documented property of the test key, not necessarily every key. (Relocated from `docs/ENGINEERING_RULES.md`, 2026-09-20, TD41 — this is a test-execution nuance, not a code anti-pattern.)
...
| Trap | Root cause | Fix |
|------|-----------|-----|
| SonarCloud job re-runs `test:cov` for all packages, doubling PR build time | A `sonarcloud` job generates its own coverage instead of downloading artifacts that sibling test jobs already uploaded | Declare `needs: [backend-unit, bff-unit, web-unit]`, download their artifacts via `actions/download-artifact`, and only run `test:cov` for packages not covered by those jobs (`@ikaro/observability`, `@ikaro/env-validation`, `@ikaro/validation`). See `pr-tests.yml` for the reference pattern. |
| Trivy permanently blocks PRs on a CVE that has no upstream fix | Removing `ignore-unfixed: true` means un-patchable CVEs fail every PR — authors can do nothing to resolve them | Keep `ignore-unfixed: true` in the PR-blocking Trivy scan. Track unfixed CVEs separately in `weekly-jobs.yml`'s `trivy-unfixed` job (Monday cron, `ignore-unfixed: false`, `exit-code: '0'`, SARIF uploaded to GitHub Security tab). |
| A CI step uses `npx -y <tool>` — different jobs on the same PR can silently get different tool versions | `npx -y` downloads the latest version at runtime — not pinned, not cached, not in the lockfile | Add the CLI as a root `devDependency` (pinned in `pnpm-lock.yaml`, cached by `actions/setup-node`) and use `pnpm exec <tool>` in the workflow step. See `wait-on` in `pr-tests.yml` as the reference. |
...
| "Optimizing" the Checkov job's path condition from a step-level `if:` to a workflow/job-level `paths:` filter — or assuming "no job-level `if:`" alone is enough to keep a job unconditional when it has a `needs:` chain | The `checkov` job must *run and report success* on every PR (it's shape-compatible with a required status check). With `on.pull_request.paths` the job never starts on non-terraform PRs — the check stays "Expected — waiting for status" forever and blocks merge (same failure class as the S08 infra-only PRs that needed `--admin` bypass, from the other direction). **Corrected 2026-07-23:** "job unconditional" (zero job-level `if:`) turned out NOT sufficient by itself. `checkov`'s `needs: [test-suite-passed]` has no explicit `if:` either, so it falls back to the *implicit default* `if: success()` — which doesn't just check the direct need's conclusion, it propagates a "skipped" status transitively from further upstream (`backend-unit` etc., skipped on an infra-only PR) even though `test-suite-passed` itself explicitly used `if: always()` and genuinely resolved to `success`. Confirmed live on PR #192: `checkov` never actually scanned Terraform on an infra-only PR at all — the whole job silently skipped before its own `dorny/paths-filter` step ever ran, despite matching this row's "keep it unconditional" guidance exactly. | Keep the step-level design (below) **and** add an explicit job-level `if: always() && needs.test-suite-passed.result == 'success'` — the `always()` specifically breaks transitive skip-propagation from jobs this one doesn't even directly depend on; the explicit result check still correctly refuses to run if the direct need genuinely failed. Then: `dorny/paths-filter` step computes the filter, scan + SARIF-upload *steps* gated on `steps.filter.outputs.terraform == 'true'`. See `gitleaks`/`snyk-sca`/`checkov`/`trivy-scan` in `pr-tests.yml` for the corrected pattern. Filter path is `infra/terraform/**` (fixed in M17-S11 — the original `infrastructure/terraform/**` was stale and the gate silently never fired) |
...
| A required check name (e.g. `Trivy Image Scan (backend)`) never satisfies even though the job visibly ran/skipped in the Actions UI | **Corrected 2026-07-23 — the original diagnosis here was wrong, verify empirically before trusting a theory this table itself once recorded.** First, the fact that made both the original AND this session's own initial diagnosis wrong: GitHub branch protection treats a required check that genuinely *reports* "skipped" as satisfying the requirement, same as "success" (documented GitHub behavior — search "GitHub required status check skipped" if this needs re-confirming later). A check that merely reports skipped is NOT the bug here. The actual bug is narrower and different in kind: a check name that never reports *at all* (not even as skipped) stays "Expected — waiting for status" forever, and THAT blocks merge. It's NOT about a job-level `if:` being present. `trivy-scan` had *zero* job-level `if:` for a full day (the 2026-07-22 "fix" below) and the exact same collapse into the raw, un-templated `Trivy Image Scan (${{ matrix.service }})` reproduced live on PR #192. The real mechanism is the same transitive skip-propagation as the Checkov row above: with no explicit `if:`, the job falls back to implicit `if: success()`, which propagates "skipped" from jobs several levels upstream (`backend-unit` etc.) even though the job's *direct* need (`test-suite-passed`) genuinely succeeded — so the job never starts at all, and a job that never starts can't expand its matrix, regardless of *why* it never started. | Add an EXPLICIT job-level `if: always() && needs.test-suite-passed.result == 'success'` (not "remove the `if:` entirely") — `always()` breaks the transitive skip-propagation; the explicit result check still refuses to run if the direct need genuinely failed. Per-step conditions (e.g. `needs.detect-changes.outputs.app == 'true'`) still correctly gate the real build/scan work once the job is allowed to start. Real incidents: TD27 PR #185 (2026-07-22, misdiagnosed as "job-level `if:` + matrix" — that fix removed the `if:` entirely and didn't actually resolve it) and a fresh CI-gate PR (2026-07-23, root-caused for real against live PR #192 evidence). |
| PR's `mergeStateStatus` is `BLOCKED` even though `gh pr checks` shows every check passing or correctly skipping | A required-status-check name in branch protection has silently drifted from what the producing workflow's job actually reports (renamed at some point without updating branch protection) — GitHub treats an unmatched required name as "never reported," not "failed," so it never shows up in the visible failure list | Compare `gh api repos/<owner>/<repo>/branches/main/protection/required_status_checks --jq '.checks[].context'` against the actual reported names on the PR's head commit: `gh api --paginate repos/<owner>/<repo>/commits/<sha>/check-runs --jq '.check_runs[].name'` (the `--paginate` matters — the API's default page size truncates before alphabetically-late names like `SonarCloud`/`TypeScript`, which will look "missing" even when they ran fine). Any required name with no matching reported name is the actual blocker. Real incident: TD27 PR #185, 2026-07-22 — `SonarCloud Code Analysis` was required, the job reports `SonarCloud Analysis`; this exact class of gap was already flagged as an open mystery in `docs/BOOTSTRAP_LOG.md`'s M17-S08 entry (2026-07-15: "several required checks never triggered... worth investigating separately") but never root-caused until this session. |
| `docker buildx build --cache-from`/`--cache-to type=gha` silently never caches — every build is a full rebuild, no error, no warning | `ACTIONS_RUNTIME_TOKEN`/cache endpoint vars exist on the runner but aren't exposed to plain `run:` steps by default — only `docker/build-push-action` auto-detects and injects them; a raw `docker buildx build` in a `run:` step gets nothing | Add `crazy-max/ghaction-github-runtime` as a step before any `docker buildx build` using `type=gha` cache. Real incident, M17-S25/TD30, 2026-07-22: found missing in three separate places at once (`pr-tests.yml`, both of `deploy-staging.yml`'s build jobs, `weekly-jobs.yml`) — the gap had existed silently since whichever story first added Trivy scanning. |
| Assuming a repo's PR review is mandatory because `required_status_checks` has many entries configured | `required_status_checks` only gates the PR *merge button* — it doesn't, by itself, block a direct push to the branch. Whether direct pushes are blocked at all is a **separate** setting (`required_pull_request_reviews`/`restrictions`) | Before relying on "a PR is required" as a security assumption, check `gh api repos/<owner>/<repo>/branches/<branch>/protection --jq '{required_pull_request_reviews, restrictions}'` directly. Real finding, TD30 story-discovery, 2026-07-22: this repo has extensive required status checks but no enforced "require PR before merging" setting at all. |
| `gh run rerun <run-id>` on a workflow gated by a "still current" staleness check (compares `HEAD` of the target branch against the run's own triggering SHA) keeps failing that check forever, even on a fresh rerun | A rerun reuses the *original* run's `GITHUB_SHA` — it never updates to the branch's current tip. Once the branch moves past that commit, the staleness check can never pass again for that run object, no matter how many times it's rerun | Trigger a genuinely new run instead: `workflow_dispatch` (if available) targets whatever the branch's tip is *at trigger time*, giving it a fresh `GITHUB_SHA` every time. Real incident: M17-S27, 2026-07-23 — `deploy-staging.yml`'s `check-current` job correctly detected staleness on every rerun of a run triggered before a docs-only commit landed on `main`, since that commit didn't match the workflow's own `paths:` filter and so never produced a fresh run of its own. |
| A path-filtered `push`-triggered workflow has no way to manually redeploy the branch's current tip without a real, matching code change | An empty commit (`git commit --allow-empty`) doesn't help — GitHub's `paths:` filter matches *changed files*, and a truly empty commit changes none, so it never triggers a path-filtered workflow at all, contrary to what "trigger via empty commit" instructions sometimes assume | Add `workflow_dispatch: {}` to the workflow's `on:` block, matching whatever sibling workflow in the same repo already has it (e.g. a promote/deploy pipeline for another environment). Real incident: M17-S27, 2026-07-23 — `deploy-staging.yml` had no `workflow_dispatch` while `deploy-production.yml` already did; discovered only when a real redeploy was needed and neither an empty commit nor `gh run rerun` worked. |
| WIF verification fails with `gcloud auth list --filter=status:ACTIVE` even though `google-github-actions/auth` succeeded | The auth action supplies an external-account credential file; `gcloud auth list` does not expose the expected active-account field in that mode | Verify the generated credential file's `service_account_impersonation_url` against the expected IAM Credentials URL without echoing either value. This proves the authenticated target without logging credential content (TD34, 2026-07-24). |
| `gcloud run deploy` with multi-container `--container=NAME` flags fails with `ERROR: (gcloud.run.deploy) unrecognized arguments: --region, --project` — the error text misleadingly suggests trying `gcloud alpha run deploy`/`gcloud beta run deploy` | The **stable** track already supports `--container`/`--depends-on` (confirmed via `gcloud run deploy --help`) — the real, documented rule is that non-container-specific flags (`--region`, `--project`, etc.) must be specified **before** the first `--container` flag; anything placed after gets parsed as belonging to that container's own flag group instead, and the generic "try alpha/beta" suggestion in the error is a red herring, not the actual fix | Put every service-level flag (`--region`, `--project`, ...) before the first `--container=...` group in the command. Real incident: M17-S34, 2026-08-04 — the first real staging deploy after the collector sidecar story merged failed with exactly this error; reproduced locally against a live `gcloud` CLI (old ordering reproduces the failure verbatim, corrected ordering gets past arg-parsing) before fixing `deploy-staging.yml`/`deploy-production.yml`. |
| A CI gate meant to enforce "prerequisite Y was actually done" is built as a path-diff check (`dorny/paths-filter`, or native `on.push.paths`) instead of a live-state check | A diff only answers "did *this push* touch path X" — `gh run rerun` reuses the same already-computed diff and can never clear it (a different, worse failure mode than the staleness row above: here no rerun clears it, fresh or not), AND any later, unrelated push also has an empty diff against X regardless of whether Y was ever satisfied, so the gate silently passes even when Y was never done at all | Check live state instead of a diff: e.g. `git merge-base --is-ancestor <last-relevant-commit> <candidate-commit>` combined with `gh api repos/<owner>/<repo>/actions/workflows/<other-workflow>.yml/runs` to confirm a qualifying run of the *other* workflow actually succeeded at or after the last relevant change — not "did this commit's diff mention it." Real incident: M19-S02, 2026-08-11 — a first-drafted `foundation-apply-required` gate in `infra-deploy.yml` (meant to block automatic applies until Foundation's separately-gated IAM apply had actually run) shipped with exactly this flaw, plus a wrong first-attempted remediation message ("just re-run this workflow"), before a live-state redesign replaced it entirely. |
| A new CI check needs to verify state owned by a separately-privileged pipeline (e.g. a protected Foundation/IAM apply) | Reaching for that pipeline's own service-account credential — even a read-only one — widens the *checking* workflow's blast radius into a boundary it was deliberately never granted access to; the check only needs to know whether the other pipeline already ran, not to act as its identity | Prefer a zero-new-trust signal already available to the checking workflow's existing permissions — here, the GitHub Actions API (`gh api .../actions/workflows/<name>/runs`, default `GITHUB_TOKEN`, `permissions: { actions: read }`) to ask "did workflow W succeed," instead of impersonating the other side's Terraform planner/deployer identity to ask the infrastructure directly. Real incident: M19-S02, 2026-08-11 — granting `infra-deploy.yml` read-only impersonation of Foundation's planner SA to check IAM state was under consideration, caught before implementation by the question "does tf-planner have the same access as foundation — could that be a privilege-escalation problem?" — TD34's whole point is that these identities stay separate. |
