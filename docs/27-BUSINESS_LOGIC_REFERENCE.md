...
flowchart TD
  ...
  H --> I["countActiveByResource(tenantId, ids, tenant-local day bounds of windowStart)"]
...
- **`resourceSelections`** (`POST /bookings` body, `ResourceSelectionInput[]`) is keyed by `(serviceId, legIndex, resourceType)` — `legIndex: null` for a flat requirement. An entry that doesn't match any `CUSTOMER_CHOICE` requirement on the booking is simply unused, never an error — this is what makes replay-based re-resolution (next bullet) safe.
- **Approval/reschedule re-resolution replay:** `ApproveBookingUseCase`/`RescheduleBookingUseCase` re-run the *entire* resolution (not just a re-check) every time, to stay the single source of truth for "what resources does this booking occupy" — but neither has an HTTP body carrying a fresh `resourceSelections` at that point. `deriveResourceSelectionsFromAssignments()` (`resource-occupancy.helpers.ts`) reads the booking's own already-persisted, immutable `booking_line_resource_assignments` rows and replays them as `resourceSelections`, so a `CUSTOMER_CHOICE` pick survives re-resolution unchanged (the customer's confirmed staff member doesn't silently change at approval time) while `AUTO_ANY`/`AUTO_FUNGIBLE_POOL` requirements simply ignore the replayed entries and re-derive fresh, exactly as before M23-S01.
...
- **Conflict-error granularity:** `BookingSlotConflictService.assertSlotFree()` classifies a genuine submit-time conflict (UC-064 A2, UC-065 A1) by inspecting the flattened candidate list it already receives — any conflicting candidate with a non-null `legIndex` → `BookingLegUnavailableError`; otherwise, more than one flat candidate anywhere in the booking → `BookingBundlePartiallyUnavailableError`; otherwise the original generic `BookingSlotUnavailableError`. This is an approximation (a multi-service, non-bundle basket where two unrelated single-resource services both contend also gets the "bundle" message) rather than tracking true per-line requirement-grouping through the conflict check — accepted as good enough since the customer-facing message is equally true either way.
