...
- [x] `--debug` baseline (both modes) reviewed; the one confirmed tool limitation (`dependency-cruiser` invoked only via `spawnSync` against a resolved binary path, never a JS import — knip structurally can't trace that) is allowlisted in `architecture-policy.json`'s `exceptions` array with reason, owner, and expiry. Remaining real findings (`uuid` unused in backend, `ms` unused in bff, `express` used in backend production code but undeclared — the exact #79 pattern this story exists to catch) are left unresolved, per this AC's own "triage before promoting to blocking" — not fixed in this story
