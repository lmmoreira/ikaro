...
```json
{
  ...
  "rationale": "packages/types/src/enums.ts is web-consumable and must never gain a module type ahead of packages/validation's canonical, backend/BFF-only source. It historically lagged behind during LEAD_FORM's staged rollout (M20-S01 → M20-S07, both sides caught up as of 2026-09-01) — this entry is a permanent guardrail against the same staged-rollout gap recurring for the next module type, not a currently-open lag.",
  "owner": "...", "reviewBy": "..."
}
```
Extract each side's literal string-member set (`constArray` walks the array literal; `union` walks a `TypeAliasDeclaration`'s union members). The rule is **superset-only, not equality**: every member of `mayLeadAhead` must exist in `canonical` (nothing web-facing may exist that the canonical source doesn't know about — this direction is a real bug, not a deferred rollout); the reverse (canonical having a member `mayLeadAhead` lacks) is allowed and produces no finding — currently vacuous (both sides equal today, per discovery note point 1), exercised for real the next time a module type ships in stages again. Note why this is not the same shape as Story 9's `aggregateValueObjectRegistry`/Story 11's `ikaro-types-drift` (both of those check pure duplication/mismatch, not a deliberately-allowed one-directional lead).
...
- [ ] `docs/15-HOTSITE_DYNAMIC_ARCHITECTURE.md` §7 "Adding a New Module" Step 1 updated to name `packages/validation/src/hotsite.ts`'s `HOTSITE_MODULE_TYPES` as the canonical edit point (backend derives automatically) and clarify `packages/types/src/hotsite.ts`'s copy stays a deliberately separate, detector-guarded web-facing entry
