...
**Acceptance criteria**:
- [ ] `knip.config.ts` defines entry/project/plugin settings per app/package (all 3 apps + 9 packages) plus a root-level `scripts/**/*.cjs` entry — none rely on ignored top-level monorepo defaults
- [ ] Both `knip` (default) and `knip --production --strict` run in CI; production entry patterns (`!` suffix) are set per workspace so the strict run actually restricts checked dependencies to `dependencies`
- [ ] `--debug` baseline (both modes) is reviewed, and every intentional dynamic/framework/reflection export or dependency is allowlisted in `architecture-policy.json`'s `exceptions` array with reason, owner, and expiry — not in a separate native-config allowlist
- [ ] Ships **non-blocking** first via `continue-on-error` in a new `pr-quality.yml` job, reporting to `$GITHUB_STEP_SUMMARY` plus an uploaded JSON artifact — expect real findings on first run, triage before promoting to blocking
- [ ] `scripts/knip-config.spec.cjs` asserts the configured workspace/entry registry matches reality (mirrors `dependency-cruiser-config.spec.cjs`)
