...
**PR split (structural, not optional — AC #5/#6 require it):**
- **PR1:** Story 22 (wire `pnpm architecture-check` into `ci:fast`) bundled with this story's AC #1–4 and #7 — burn-in verification, fixture audit, command-equivalence confirmation, promoting `.github/workflows/pr-quality.yml`'s `architecture-check` job to blocking (remove `continue-on-error: true`, rename the job from "Architecture validation (report-only)" to "Architecture validation"), plus `packages/architecture-check/tsconfig.json`'s `incremental: true` build optimization (see Story 22's note) and the rollback procedure below.
- **PR2 (separate, later — cannot land until a fresh burn-in window elapses on the now-blocking job):** AC #5's second burn-in observation + AC #6's branch-protection required-status-check addition. AC #6 explicitly forbids bundling this with the promotion change, and AC #5 requires real elapsed time on `main` first — neither can be compressed into PR1.

**AC #1 (burn-in) — verified 2026-09-01:** The `architecture-check` CI job's real origin is `feat(ci): add architecture validation foundation (TD37)` (commit `53f3fd44a8`, 2026-08-10) — not `pr-quality.yml`'s own file-creation date (2026-08-31), a later consolidation that a naive `git log --follow -A` search is misled by. Sampled job-level (not workflow-level — `continue-on-error: true` masks a job failure from the workflow's own conclusion) conclusions across 5 runs spanning 2026-08-10 → 2026-08-28: all 5 `"conclusion":"success"`. A live run today (2026-09-01) against current `main` also shows zero findings across all 19 detectors, exit code 0. Sampled, not an exhaustive audit of every run in the window — treated as sufficient evidence given the tool is deterministic static analysis over committed source (no flakiness vector), not environment-dependent.

**AC #2 (fixture/message/zero-target/exception completeness) — verified 2026-09-01:** All 19 `architecture-check` detector rules map to a dedicated `.spec.ts` (own file, or grouped with sibling rules from the same source file — `di-alias.spec.ts` covers `unsafe-use-existing`/`reverse-di-alias`/`global-module-export-pairing`; `test-hygiene.spec.ts` covers `test-builder-coverage`/`entity-builder-pk-uuidv7-default`/`test-harness-registration`; `architecture-check.spec.ts` covers Story 0's original 6: `transactional-io`/`transactional-save`/`error-mapper-coverage`/`error-prototype-chain`/`vo-create-no-bare-error`/`shared-vo-error-mapper-coverage`), each with multiple positive/negative/edge-case `it()` blocks. Remediation messages spot-checked across every detector source file — all substantive and actionable, citing the exact file/pattern/fix and (where relevant) a doc section, never a bare rule name. Zero-target enforcement is a shared, structural guarantee (`cli.ts`'s `zeroTargetResults` check applies uniformly to every detector's `ScanResult`, established once in Story 0 and exercised by `architecture-check.spec.ts`'s "fails the zero-target contract for an empty scan" test) rather than 19 duplicated per-detector tests — read as satisfying the AC's intent, not a gap requiring 18 new tests. `architecture-policy.json`'s `exceptions` registry is real and actively used by the 3 of 19 detectors that currently need it (`aggregate-primitive-vo`, `error-mapper-coverage`, `shared-vo-error-mapper-coverage`; 38 entries total across the whole architecture-check suite including its ESLint/knip-based rules); the other 16 currently need none.

**AC #3 (local/CI equivalence) — verified 2026-09-01:** Both run the identical `pnpm architecture-check` command (root `package.json` alias → `pnpm --filter @ikaro/architecture-check run check` → `tsc -p tsconfig.json && node dist/cli.js`); CI's job runs `pnpm install --frozen-lockfile` then the same command. Byte-for-byte identical, confirmed by direct comparison of `.github/workflows/pr-quality.yml` against the root `package.json`.

**AC #7 (rollback procedure):** If the blocking `architecture-check` job (CI or local `ci:fast`) produces a confirmed false positive that blocks legitimate work, an authorized reviewer restores `continue-on-error: true` to the CI job and removes `pnpm architecture-check` from `ci:fast`, in one explicit reviewed PR stating: (1) the exact false-positive finding, (2) why it's a false positive and not a real violation, (3) a follow-up/expiry date to re-promote once the detector is fixed — mirroring `architecture-policy.json`'s existing exception-registry discipline rather than a silent, undocumented revert.

**Baseline for PR2 (verified live, 2026-09-01):** `main`'s branch protection currently requires exactly 16 status checks (`ESLint`, `Prettier`, `TypeScript`, `Backend Unit Tests`, `Backend Integration Tests`, `Gitleaks Secret Scan`, `Snyk SCA`, 3x `Trivy Image Scan`, `Checkov IaC Scan`, `Infra PR Checks Passed`, `actionlint`, `zizmor`, `SonarCloud Analysis`, `Terraform env-var contract`). `Architecture validation` is not among them — PR2 adds it.
...
**Discovery note (TD37-S21, 2026-09-01) — 4 corrections locked in before implementation, each verified live against current `main`, not assumed:**

1. **The "deferred web rollout" premise above is now historical, not current.** M20-S07 (✅ Done) already shipped real `LEAD_FORM` entries into every `apps/web` exhaustive map (`default-layout.ts`, `hotsite-editor-lazy-panels.tsx`, `module-schemas.ts`) — confirmed all 3 pre-unification copies are, right now, still byte-for-byte identical (10 members, same order). Part B's registry entry is kept — it's a forward-looking guardrail against the *next* module type repeating this exact staged-rollout gap, not a detector for an active lag. The `rationale` field below reflects this.
2. **A 4th, previously unmentioned duplicate found during discovery:** `hotsite-config.aggregate.ts:55-66`'s own runtime `MODULE_TYPES: ReadonlySet<HotsiteModuleType>`, hand-maintained and used by `validateLayout()`'s membership check — distinct from the *type* alias Part A originally scoped. Folded into Part A below so "collapse the avoidable copies" is actually complete, not partial.
3. **The type-only-import hedge and `@ikaro/types` re-export fallback are dropped — confirmed unnecessary, not just untested.** Read `scripts/dependency-cruiser.config.cjs`'s `no-domain-framework-or-infrastructure-imports` rule directly: its `to.path` list restricts only `@nestjs/*`, `typeorm|axios|express`, and same-context `application`/`infrastructure` — `@ikaro/validation` isn't in it, for either a type-only or a real import. Stronger: `apps/backend/src/shared/value-objects/email.vo.ts` (+6 sibling VOs — `country-code`, `hex-color`, `phone-number`, `seo-description`, `seo-title`, `time-of-day`, `timezone`) already do a **real, non-type-only** runtime import from `@ikaro/validation` today, in framework-free domain-adjacent code. `@ikaro/validation`'s compiled `dist/index.js` re-exports every submodule unconditionally (`export * from './x'` chains), so importing anything from it already pulls in `zod` transitively, today, independent of this story. Part A now does a real value import for the `MODULE_TYPES` Set with no exception entry needed in `architecture-policy.json`.
4. **"Report-only (Phase 1)" AC dropped — the mechanism it depended on no longer exists.** Stories 19 and 22 (✅ Done) already promoted the *entire* `pnpm architecture-check` command to one blocking gate in `ci:fast`/pre-push; `cli.ts`'s exit code is all-or-nothing across every detector's combined findings, with no per-detector report-only switch. Story 20 — the immediately-preceding detector story, also landed after that promotion — already set the precedent: no report-only AC, ship once fixtures pass and a live `pnpm architecture-check` run against `main` shows zero findings (same live-verification discipline Story 22 itself documented). This story follows the same path.

**Update (PR #456 round 3, 2026-09-01) — the registry is strict bidirectional equality, not a one-directional lag allowance:** Codex's review flagged, with escalating severity across rounds (Important → Critical), that the one-directional `mayLeadAhead` design left a genuine two-way-divergence case unguarded — canonical gaining a member the registered copy never picked up produced no finding, which reads as failing this story's own "plain 2-way divergence for a non-lead/lag registry entry" AC line. Put directly to the user: `HotsiteModuleType`'s original "may lag" design existed specifically to accommodate `LEAD_FORM`'s real, historical staged rollout (M20-S01 → M20-S07) — keep that allowance (build a genuine opt-in strict/lenient mode, with this entry staying lenient), or close it outright for this entry too. **Decision: close it outright.** `closedEnumRegistry` is now unconditional strict bidirectional equality for every registered entry — the `mayLeadAhead` field is renamed `mirror` (a name that no longer implies any one-directional allowance), and a member missing from either side is a finding. **Real consequence, not just a rule change:** a future module-type addition (or removal) must land in `packages/validation/src/hotsite.ts` and `packages/types/src/enums.ts` in the same commit — the staged, multi-PR rollout `LEAD_FORM` originally used is no longer possible for this enum without failing `pnpm architecture-check`.

**Part A — Unify the backend mirror, the validation schema, and the aggregate's runtime module-type Set (real code change, done as part of this story):**
- `packages/validation/src/hotsite.ts` becomes the single source: export a canonical `as const` tuple `HOTSITE_MODULE_TYPES = ['HERO', 'SERVICE_LIST', 'GALLERY', 'TESTIMONIALS', 'BOOKING_CTA', 'ABOUT', 'CONTACT', 'FOOTER', 'CHATBOT', 'LEAD_FORM'] as const`, derive both `HotsiteModuleSchema`'s `type: z.enum(HOTSITE_MODULE_TYPES)` and an exported `export type HotsiteModuleType = (typeof HOTSITE_MODULE_TYPES)[number]` from it.
- `apps/backend/src/contexts/platform/domain/hotsite-config.types.ts` drops its own `HotsiteModuleType` union entirely and instead does a **type-only** import: `import type { HotsiteModuleType } from '@ikaro/validation'`. Confirmed safe (discovery note point 3) — no exception entry needed.
- `apps/backend/src/contexts/platform/domain/hotsite-config.aggregate.ts`'s own `MODULE_TYPES: ReadonlySet<HotsiteModuleType>` (line 55) drops its hand-typed literal array in favor of a **real** import: `import { HOTSITE_MODULE_TYPES } from '@ikaro/validation'; const MODULE_TYPES: ReadonlySet<HotsiteModuleType> = new Set(HOTSITE_MODULE_TYPES);` — same confirmed-safe basis as above.
- `hotsite-config.aggregate.ts`'s existing `export *` from `hotsite-config.types.ts` keeps re-exporting `HotsiteModuleType` under the same name, so no call site elsewhere in backend needs to change its own import path.

**Part B — Detector enforcing that the one remaining split family never drifts:** a `ts-morph` detector reads a `closedEnumRegistry` array from `architecture-policy.json`. Each entry names a conceptual enum, its canonical (unified) source, and a `mirror` that must always carry the exact same member set — in both directions:
```json
{
  "name": "HotsiteModuleType",
  "canonical": { "path": "packages/validation/src/hotsite.ts", "kind": "constArray", "exportName": "HOTSITE_MODULE_TYPES" },
  "mirror": { "path": "packages/types/src/enums.ts", "kind": "union", "exportName": "HotsiteModuleType" },
  "rationale": "packages/types/src/enums.ts must always exactly mirror packages/validation's canonical, backend/BFF-only HOTSITE_MODULE_TYPES — no permitted lag in either direction. It historically lagged behind during LEAD_FORM's staged rollout (M20-S01 → M20-S07); this entry closes that exact gap going forward — a new module type must land in both files in the same change.",
  "owner": "...", "reviewBy": "..."
}
```
Extract each side's literal string-member set (`constArray` walks the array literal; `union` walks a `TypeAliasDeclaration`'s union members). The rule is **strict equality, not a one-directional allowance**: any member present on one side but not the other — in either direction — is a finding. `packages/types` is not permitted to lag behind `packages/validation` any more than it's permitted to lead ahead of it; a future module-type addition must update `canonical` and every registered `mirror` in the same commit. Mechanically distinct from Story 9's `aggregateValueObjectRegistry` (aggregate-prop-vs-VO typing) and Story 11's `ikaro-types-drift` (structural interface/type-alias field-shape diffing) — this detector extracts and diffs literal string-member sets from a `constArray`/`union` declaration pair.

**What it catches**: `packages/types/src/enums.ts`'s `HotsiteModuleType` diverging from the canonical `@ikaro/validation` source in either direction — gaining a member the canonical source lacks (the M20-S01 incident shape), or lacking a member the canonical source has (the class of drift a future staged rollout would otherwise reintroduce).
...
- [ ] `apps/backend/src/contexts/platform/domain/hotsite-config.types.ts`'s own `HotsiteModuleType` union is deleted, replaced by a type-only import from `@ikaro/validation`
- [ ] `hotsite-config.aggregate.ts`'s `MODULE_TYPES` Set derives from the same canonical `HOTSITE_MODULE_TYPES` import (real, value-level) instead of its own hand-typed literal array
...
- [ ] Detector flags a member present in `mirror` but not `canonical`, a member present in `canonical` but not `mirror`, and a fixture where both directions diverge simultaneously (one finding naming both); passes only on exact equality
...
- [ ] Detector registered in `cli.ts` alongside the existing check list (near `checkIkaroTypesDrift(...)`), using the already-registered-but-previously-unused `packages/validation/tsconfig.json` project
- [ ] A live `pnpm architecture-check` run against `main`-equivalent state shows zero findings before merge (Story 20/22 precedent) — no separate report-only rollout phase for this detector
- [ ] Permanent valid/invalid/zero-target fixtures, per `detectorContract`
- [ ] `docs/15-HOTSITE_DYNAMIC_ARCHITECTURE.md` §7 "Adding a New Module" Step 1 updated to name `packages/validation/src/hotsite.ts`'s `HOTSITE_MODULE_TYPES` as the canonical edit point (backend derives automatically) and state that `packages/types/src/enums.ts` must be updated in the same commit, not a later, staged one
...
Selector shapes locked in at discovery time (below), then hardened across 4 rounds of Codex PR review — see the round-4 update further down for the final shapes actually shipped:
...
**Update (TD37-S23 PR #450, Codex review, 2026-08-31): final selectors, hardened beyond the discovery-time shapes above.** Each round below found a real bypass the original selector missed (rounds 1-2), or found that the harder selector's own baseline needed a source fix, not just a scope exception (round 2's `pill-select.tsx`):
- E2E-1 (round 1 + round 2): also matches computed-literal member access (`page['getByText'](...)`) and a bare destructured call (`const { getByText } = page; getByText(...)`) — final: `CallExpression:matches([callee.property.name=/^(getByLabel|getByText)$/], [callee.computed=true][callee.property.type='Literal'][callee.property.value=/^(getByLabel|getByText)$/], [callee.type='Identifier'][callee.name=/^(getByLabel|getByText)$/])`.
- E2E-2 (round 3): also matches a string literal wrapped in a `JSXExpressionContainer` (`data-testid={'row-2026-08-31'}`), not just the plain `JSXAttribute` string-literal form — final: `JSXAttribute[name.name='data-testid']:matches([value.type='Literal'][value.value=/\d{4}-\d{2}-\d{2}/], [value.type='JSXExpressionContainer'][value.expression.type='Literal'][value.expression.value=/\d{4}-\d{2}-\d{2}/])`.
- E2E-3 (round 1): descendant combinator, not direct-child, so a template literal nested inside a conditional/logical expression is still caught — final: `JSXAttribute[name.name='data-testid'] JSXExpressionContainer TemplateLiteral`. This broadened match caught a real, pre-existing production violation in `apps/web/shared/components/ui/pill-select.tsx` (`data-testid={\`${testId}-${option.value}\`}`), fixed to a static `data-testid` + separate `data-value` attribute (this repo's documented pattern) — with 11 dependent e2e/component-spec call sites updated to match.
- AC3 (rounds 1-4): a `Linter.verify()`-based regression spec alone wasn't accepted as sufficient evidence that a real violation fails at `ci:fast`/push time — added a permanent test (`e2e-quality.eslint.spec.ts`'s "AC3: a real fixture violation..." block) that spawns the actual `pnpm lint` CLI as a subprocess against on-disk fixture files, mirroring TD37-S18's own `eslint-next-plugin.eslint.spec.ts` precedent exactly.
...
| Both-direction `isActive` guard (#94) | Business-logic completeness — whether a given use case *should* guard both directions is a domain judgment, not a structural shape |
| Duplicate cross-context ports / duplicate read endpoints / duplicate use cases (#80, #91, #93) | "Is this really the same responsibility" is a design judgment; a naming/shape heuristic here already produced false positives in your own bad-smell-audit iteration history |
...
| "Too much orchestration in web vs BFF" (BFF-1's fuzzier half) | A call-count/branching heuristic is a proxy metric, not a real detector; risks the same "expanding a blocking gate to chase an exploratory audit's coverage" mistake row #142 already rejected |
...
| Interceptor-vs-`ExceptionFilter` placement for Guard-visible concerns (#137) | The specific past instance was narrow and bespoke (structured error-logging); a generic "no `catchError` in any Interceptor" rule would have too many legitimate exceptions to be worth encoding |
...
- [x] Retired manual bad-smell-audit checks — confirmed verbatim in `.claude/commands/bad-smell-audit.md`: BE-1 (line 60, "RETIRED — mechanized by `pnpm architecture-check` (TD37-S09)"), BE-4 (line 82, TD37-S07), WEB-9 (lines 187-189, "Now CI-enforced by... `ikaro-types-drift` detector (TD37 Story 11)")
- [x] `docs/ANTI_PATTERNS.md` updated — 14 rows across Stories 2, 3, 6, 8, 9, 10, 11, 13 now note CI-enforcement (2026-09-01 sweep). Not exhaustive: Story 1's full `dependency-cruiser` boundary rule set wasn't cross-referenced row-by-row beyond 2 representative rows
- [x] This TD's own "Out of Scope" table double-checked against `docs/ANTI_PATTERNS.md` (2026-09-01) — no incident since 2026-07-27 changes the calculus on any row, but 4 row-number citations had drifted (rows get renumbered as new rows are added) and were corrected in place
- [x] Every static detector has permanent valid/invalid fixtures and a zero-target assertion; every exception is in the reviewed registry with rationale, owner, and review/expiry — already verified above, see Story 19's own "AC #2 (fixture/message/zero-target/exception completeness) — verified 2026-09-01" note
