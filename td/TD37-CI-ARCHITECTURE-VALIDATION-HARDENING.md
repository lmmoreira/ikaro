...
2. **Two `max-lines`-only exceptions added** to `packages/architecture-check/architecture-policy.json` (`max-lines-aggregate` rule, 2 entries) — `booking.aggregate.ts` (641 lines) and `hotsite-config.aggregate.ts` (433 lines) are DDD aggregates whose file length is inherent to owning many small cohesive methods, not a complexity smell a file split would fix; both are far over even the raised 250-line threshold, so the exception (not the threshold change) is what resolves them. Exempts `max-lines` only; `max-lines-per-function` is enforced and fixed on both files. This does not reopen the "no bulk exceptions" finding from discovery — two individually-reasoned, dated (`reviewBy: 2026-11-30`) entries is exactly the registry's intended shape, distinct from the ~900-file blanket grandfather that was rejected.
...
- [x] All ~37 `.ts` function-length violations and ~21 file-length-only violations fixed — zero violations on `main` for all 3 rules once Story 5A also lands (2 reviewed, dated `max-lines-aggregate` exceptions added during implementation — see Implementation note)
