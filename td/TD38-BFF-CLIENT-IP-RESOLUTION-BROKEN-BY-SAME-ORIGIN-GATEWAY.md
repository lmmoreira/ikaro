...
- PR not yet re-verified green on CI since this review-response round (pushed, awaiting a fresh run — the Playwright/health-probe/prod-Terraform/Authorization-stripping bugs above were all found via real CI failures on earlier pushes, not just local review).
- `web-internal-key`'s real value: populated out-of-band via `gcloud secrets versions add` (done for staging as part of this round, since a version-less secret referenced in `secret_env_vars` fails `terraform apply` outright per `modules/cloudrun-service/main.tf`'s own documented constraint — a `terraform import` block adopts the manually-created container). Prod's real value is not populated (prod's `bootstrap_mode=true` omits `secret_env_vars` entirely, so it's inert there for now).
...
- **Story B (prod)** — not started (beyond the coherence fix above).
...
- [ ] The `e2e` job in `pr-tests.yml` (local/docker-compose E2E) passes unmodified — **initially wrong**: a real CI run on this branch caught `WebOnlyGuard` 401ing the 9 direct-BFF Playwright helpers (see Phase 5's correction note above); fixed via `playwright.config.ts`'s `extraHTTPHeaders`, pending a CI run to confirm green
