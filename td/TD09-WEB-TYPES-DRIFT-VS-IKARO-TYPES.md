...
**Update (2026-07-23)**: re-checked — `scripts/pre-pr.sh`'s `WEB-7` check already points at `apps/web/features/.*/api` (someone fixed the above follow-up since 2026-07-14 without updating this note). It's diff-scoped (only checks files changed in the current PR), so it can't catch drift in files nobody has touched since — which is exactly how `LoyaltyEntryItem`/`LoyaltyRedemptionItem` drifted undetected (see `td/TD31-BAD-SMELL-AUDIT-COVERAGE-SNAPSHOT.md` items 2.1/2.2). `bad-smell-audit.md`'s new `WEB-9` check (added the same day) covers the full-codebase sweep this diff-scoped check structurally can't. Unrelated stale-path bug actually found this session: `scripts/pre-pr.sh` checks `WEB-4`, `23`, and `27` all hardcoded the dead `apps/web/components/` path (same class of bug, different checks) — fixed in the same commit as this note.

**Update (2026-07-27, TD31 Story 2/20 fix)**: `LoyaltyEntryItem`/`LoyaltyRedemptionItem`/`LoyaltyBalanceResponse` in `apps/web/features/loyalty/api.ts` turned out not to need a type-only fix. Tracing every consumer found the whole set of read functions (`getLoyaltyBalance`, `getLoyaltyEntries`, `getLoyaltyRedemptions`, `getCustomerLoyaltyBalance`, `getCustomerLoyaltyEntries`, `getCustomerLoyaltyRedemptions`) and their `useLoyalty.ts` hooks have zero real (non-spec) consumers — they're `M13-S01` scaffolding, fully superseded by `features/customer/api.server.ts` (customer-facing, `bffServerFetch`) and `features/loyalty/dashboard-api.ts` (staff-facing, same function names, already correct types). Resolved by deleting the 6 duplicate functions/hooks instead of retyping them — see `td/TD31-BAD-SMELL-AUDIT-COVERAGE-SNAPSHOT.md` Story 2's "Discovery update" for the full trace. Only `redeemPoints`/`useRedeemPoints` remains in both files (no `@ikaro/types` equivalent exists, no drift, just an unbuilt feature).
