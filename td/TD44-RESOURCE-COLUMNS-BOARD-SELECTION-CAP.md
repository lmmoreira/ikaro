...
2. **Minimum-granularity content variant.** A booking whose duration equals the schedule's own `slotGranularityMinutes` (today driven by real service-duration data, not hardcoded — confirmed dynamic per tenant/service config) currently renders identical content to a 2-hour booking: title/subtitle/status row, then (from TD44-S2) the resource-summary line if assigned, then a time-range line. For a booking at this minimum granularity, drop the time-range line — title, subtitle (services), and status badge only, relying on the block's own vertical position in the grid (now readable again once item 1 lands) instead of restating the time as text. A booking longer than the minimum granularity keeps every existing line, including the resource-summary line from TD44-S2 and the time-range line. This branch lives in `renderBookingTimelineEvent` (`ScheduleTimelineEventRenderer.tsx`), keyed off `event.booking.totalDurationMins` compared against the active `slotGranularityMinutes` — not a new prop, both values already flow into the component today.
3. **Scroll-to-current-time on load (new functionality — confirmed nothing like this exists today, see verification below).** When the Day view (and Week view, for the same information-density reason) loads showing a date that includes the current moment (i.e. today), the timeline should auto-scroll once to bring the current-time row into view on initial load/date-change, instead of always starting at the active window's opening time. Concrete rules to lock in during `/story-discovery`:
   - Only fires when the viewed date actually includes "now" (Day view: selected date === today; Week view: today falls within the visible week) — a past/future date keeps today's exact behavior (starts at the top of the active window), no regression.
   - Fires once per date change (mount or date-key change), never on a subsequent re-render from data refetch/polling — must not fight a user's own manual scroll mid-session.
   - The actual scroll *container* needs to be identified during discovery (an Explore pass over `ScheduleMainView.tsx`/`ScheduleTimelineBoard.tsx`/the page shell found no existing scroll-container ref to reuse — this is new, not a modification of existing scroll logic) — confirm whether the whole page scrolls or a dedicated timeline `<div>` does, since the effect's target and its cleanup differ depending on which.
   - Decide the target offset: scrolling so "now" lands at the very top of the viewport vs. a bit of lookback (e.g. 1 hour of context above "now," closer to Google Calendar's actual behavior) — pin this explicitly rather than picking implicitly in code.
4. **Remove two redundant labels.** Verified against the exact class strings in the user's own DOM inspection: the pasted `<span class="text-[0.625rem] font-medium uppercase tracking-[0.08em] text-gray-500">` matches `ScheduleTimelineBoard.tsx:84-88` (the compact board's "N agendamento(s) neste dia" count) **exactly**, and the pasted status Badge's classes match that same file's status badge (`:75-81`) — confirming the screenshot driving this request is **Week view's day-card header** (`TimelineCompactBoard`), not Day view. Both are conditionally-and-unconditionally rendered restatements of information the grid itself already shows visually (which day-cards have blocks, and roughly how many). Remove both from `ScheduleTimelineBoard.tsx`'s compact board. **Whether Day view's parallel elements should also go, for consistency** (`ScheduleMainView.tsx:94-101`'s status badge — 3-way `statusRegularOpen`/`specialOpeningBadge`/`statusClosed`; `ScheduleDayHeader.tsx:91-97`'s count badge, `bookingsOnDay` key) **is an open scope question, not assumed** — confirm explicitly with the user during `/story-discovery` rather than silently scoping to Week view alone or silently expanding to Day view. The underlying i18n keys (`dashboard.schedule.statusRegularOpen`, `specialOpeningBadge`, `statusClosed`, `bookingsOnDay`) stay in both locale files regardless — they're shared across Day and Week consumers, so removing one board's usage doesn't make a key dead unless the Day-view scope question above also resolves to "remove."

**Files likely touched (confirm exact list during `/story-discovery`, once the scroll-container question is resolved):**
- `apps/web/features/booking/schedule/schedule-timeline-formatting.ts` (+ `.spec.ts`) (modify — split the conflated `minHeightPx` param into a small grid-row coordinate unit + a separate per-block content-fit floor)
- `apps/web/features/booking/schedule/schedule-timeline.ts` (+ `.spec.ts`) (modify — `buildBlockStyle`/`getSlotHeight` call sites, applying the per-block floor without feeding it back into `slotHeight`/`top` for other blocks)
- `apps/web/features/booking/components/dashboard/schedule/ScheduleTimelineEventRenderer.tsx` (+ `.spec.tsx`) (modify — minimum-granularity content branch, dropping the time-range line)
...
- `apps/web/features/booking/components/dashboard/schedule/ScheduleMainView.tsx` (+ `.spec.tsx`) (modify, if Day-view scope confirmed — remove the equivalent status badge)
- `apps/web/features/booking/components/dashboard/schedule/ScheduleDayHeader.tsx` (+ `.spec.tsx`) (modify, if Day-view scope confirmed — remove the count badge)
- New scroll-to-now module, exact name/location TBD during discovery (likely a small pure "compute scroll offset for now vs. active window" function + a hook wiring it to whichever container discovery identifies), with its own `.spec.ts`
- `apps/web/e2e/schedule.spec.ts` / `schedule-resource-columns.spec.ts` (modify — assert reduced content on a minimum-granularity booking, assert scroll-to-now on today's date, assert no auto-scroll on a non-today date, assert the removed labels are gone from Week view's day-card)
...
**Acceptance criteria — product:**
- [ ] A full business day's worth of bookings requires meaningfully less scrolling than today to review, on a standard laptop viewport — concrete target pinned during `/story-discovery`, not left as "smaller."
- [ ] A booking at the schedule's minimum granularity renders only title, subtitle (services), and status badge — no time-range line — in every board it can appear in (Day single timeline, Day resource-columns board, Week day-cards).
- [ ] A booking longer than the minimum granularity keeps all of today's content, including TD44-S2's resource-summary line when assigned and the time-range line.
- [ ] Loading the Day view for today auto-scrolls once to bring the current-time position into view.
- [ ] Loading the Day/Week view for a date that does not include the current moment is unaffected — no auto-scroll, identical to today's behavior.
- [ ] The auto-scroll fires once per date change, not on every re-render/refetch — a user's own manual scroll during the session is never overridden.
- [ ] Week view's day-card header (`ScheduleTimelineBoard.tsx`'s compact board) no longer shows the status badge or the "N agendamento(s) neste dia" count.
- [ ] Whether Day view's equivalent elements are also removed is explicitly confirmed with the user during `/story-discovery`, not silently decided either way.

**Acceptance criteria — technical:**
- Unit:
  - [ ] `getSlotHeight` (or its post-discovery replacement) produces the newly-agreed value(s) for the recalibrated constant(s)
  ...
  - [ ] The scroll-to-now computation returns a defined offset when the viewed date includes now, and `null`/no-op when it doesn't
  - [ ] `ScheduleTimelineBoard`'s compact board no longer renders the status badge or count span (existing DOM/testing-library assertions updated, not just deleted)
- Integration: n/a — no `.integration.spec.ts` tier for `apps/web`
- Tenant isolation: n/a — client-side only
- E2E: a minimum-granularity booking's block shows no time-range text while a longer one still does; Day view opened on today's date ends up scrolled near the current-time row; Day view opened on a future date stays at the top of the active window; Week view's day-card no longer shows the status badge/count text
- [ ] Coverage ≥80% on changed code
- [ ] `tsc --noEmit` clean, lint clean
