...
| D17 | **Refines D7/D15 further: notification's multi-recipient dispatch (`dispatchTemplatesToMany`) claims per-recipient, not per-batch** — `consumer_name` composition becomes `${notificationType}:${channel}:${recipientEmail}` for this path; column widened `VARCHAR(150)` → `VARCHAR(400)` (AUD-004 item 3, 2026-07-17). | The original per-batch claim meant one failing recipient's `unclaim` forced a full-batch retry, re-sending to every recipient including ones that already succeeded. Per-recipient claiming makes an already-succeeded recipient a cheap `tryClaim`-false skip on redelivery — no re-render, no re-dispatch — while the genuinely-failed recipient(s) retry normally. Dispatch within the batch is sequential and continue-on-error (attempt every recipient in the pass; throw once at the end only if ≥1 failed), so healthy recipients aren't held hostage by one bad address. Proved against real Postgres in `send-booking-requested-notification.use-case.integration.spec.ts`. |
