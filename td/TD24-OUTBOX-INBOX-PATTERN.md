...
The atomic story: split the publish/subscribe token, rebind, drain, delete the 16 publish loops, migrate the 16 subscribing handlers, fix the test bus — these ship together because each half breaks without the others.

**Modify — DI + new port (2 files, per D12):**
- New `shared/ports/event-subscriber.port.ts` — `IEventSubscriber { subscribe<T extends Envelope>(eventName, handler, consumerName): void }`, `EVENT_SUBSCRIBER` token. Permanently `useExisting: GcpPubSubEventBusAdapter` in `event-bus.module.ts` — never touched again by any future rebind.
- `event-bus.module.ts`: `EVENT_BUS → useClass: OutboxPublisher` (was `useClass`/`useExisting: GcpPubSubEventBusAdapter` per S01's D12 wiring) — this is now genuinely the only line this story changes in the module. `TRIGGER_BUS`/`PUSHABLE_EVENT_BUS` already point directly at `GcpPubSubEventBusAdapter` (D12, shipped in S01) and are unaffected.

**Modify — event handlers, the *subscribing* 16 (distinct from the *publishing* 16 below):** all 16 notification/loyalty/staff handler files (§B's consumer list) switch `@Inject(EVENT_BUS) private readonly eventBus: IEventBus` + `.subscribe(...)` to `@Inject(EVENT_SUBSCRIBER) private readonly eventBus: IEventSubscriber` + `.subscribe(...)`. Mechanical (same call, different token/interface); their specs only need the type import updated where they reference `IEventBus` explicitly.

**Modify — integration test wiring (per the subscriber-token split):** integration app helpers currently override `EVENT_BUS` alone with the routing bus so a flow test's publish and subscribe sides observably connect. Once subscribe moves to `EVENT_SUBSCRIBER`, overrides must bind **both** `EVENT_BUS` and `EVENT_SUBSCRIBER` to the same routing-bus instance.
...
**Modify — use cases, the *publishing* 16 (§A1 list — a different 16 files than the subscribing handlers above):** delete the `clearDomainEvents()` publish loop (and the now-unused `EVENT_BUS` injection where nothing else uses it) from all 11 booking + 4 staff + 1 platform use cases. Each use case's event emission is now implicit in `repo.save()` inside its existing `txManager.run()`.
...
- **Full-topology test (one, new):** an integration app where `EVENT_BUS` is the real `OutboxPublisher` and `EVENT_SUBSCRIBER` is the routing bus, with `OutboxRelayService`'s inner adapter also pointed at the routing bus so a relayed message reaches the subscribed handler — driving one booking approval end-to-end: use case → outbox row → relay → dispatch → notification handler → email log. This is the only test that exercises the *production* pipeline shape (flow suites bypass the outbox; S01 tests it in isolation) — it catches envelope-serialization drift through the JSONB round-trip that neither covers.
...
**Modify — enforce the ambient-transaction invariant (2 files):** once the jobs bullet above lands, *every* `publish()` call site in the codebase runs inside `txManager.run()` — the 3 jobs for batch atomicity (this story), the 3 aggregate repos already (S02's D6 drain), so the "standalone" fallback `OutboxPublisher`/`TypeOrmOutboxRepository` support since S01 (D13; see the "commits the outbox row standalone when published outside any transaction" test) stops being a legitimate path and becomes a signal that a future call site forgot to wrap itself. Add a hard check to `TypeOrmOutboxRepository.insert()`: throw (e.g. `OutboxPublishedOutsideTransactionError` or similar — a startup/programmer-error class, not a domain error) when `getActiveEntityManager()` is undefined, and delete the standalone `this.repo.query(...)` fallback branch entirely. **Do not add this check before this story** — S02 ships before S03, so between S02's rebind and this story's job-wrapping, the 3 jobs would still be calling `publish()` unwrapped and the check would break them.
...
- [ ] No application code calls Pub/Sub publish directly for domain events — `GcpPubSubEventBusAdapter.publish` is referenced only by `OutboxRelayService`; `EVENT_BUS` resolves to `OutboxPublisher`.
