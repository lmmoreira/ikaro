...
5. **New env var needed:** `DB_INSTANCE_CONNECTION_NAME` (format `project:region:instance`, e.g. `ikaro-staging:southamerica-east1:ikaro-db-staging`) — the connector needs this, not the raw private IP `DB_HOST`. Thread through Terraform: `envs/staging/main.tf` and `envs/prod/main.tf`, both the Cloud Run service module calls (backend) and `migrate_job`. Decide whether `DB_HOST` stays (for the `APP_ENV=local` fallback path, which won't use the connector) or is fully replaced — local dev/CI Testcontainers Postgres has no Cloud SQL instance and must keep working exactly as today.
   - **Correction (story-discovery, 2026-07-24):** the source value already exists — `modules/database/outputs.tf` exposes `output "instance_connection_name"` (`google_sql_database_instance.main.connection_name`). It is currently unconsumed by any `env_vars` block in either env's `main.tf`. No new Terraform output is needed — only wiring the existing output into the backend and `migrate_job` `env_vars` blocks in both envs.

6. **IAM — verify only, not a grant (corrected via story-discovery, 2026-07-24):** the connector calls the Cloud SQL Admin API to mint ephemeral certs, which requires the calling identity to hold `roles/cloudsql.client`. This TD originally assumed the runtime service accounts (`ikaro-backend@`, `ikaro-migrate@`) would need a new binding for this. **Live-checked and already false**: both already hold `roles/cloudsql.client` at the project level — `modules/iam/main.tf:52` (`ikaro-backend@`) and `modules/iam/main.tf:60` (`ikaro-migrate@`) — most likely provisioned in M17-S17 ahead of this need. No new Terraform IAM change is required; this step is now a one-line verification, not implementation work.
...
- [x] `@google-cloud/cloud-sql-connector` added; wrapped in an `infrastructure/`-layer adapter (anti-lock-in guardrail respected) — `apps/backend/src/shared/infrastructure/database/cloud-sql-connector.adapter.ts`
- [x] `data-source.ts` and `app.module.ts` both connect via the connector-backed adapter for `APP_ENV !== 'local'`; local/CI Testcontainers path unchanged — both now use an async factory (verified `typeorm@1.1.0`'s CLI awaits an exported `Promise<DataSource>`, and `@nestjs/typeorm`'s `forRootAsync.useFactory` supports `Promise<TypeOrmModuleOptions>`)
