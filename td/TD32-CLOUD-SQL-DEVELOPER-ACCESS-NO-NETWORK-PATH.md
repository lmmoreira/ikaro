...
- `google_project_iam_audit_config` × 2 — Data Access audit logging (`ADMIN_READ`/`DATA_READ`/`DATA_WRITE`) for `iap.googleapis.com`, and `DATA_READ` for `secretmanager.googleapis.com` (`AccessSecretVersion` is also Data Access-classified and opt-in — a gap the first PR round missed, caught in cross-tool review). Both are IAP's/Secret Manager's own docs being explicit these aren't on by default.
...
- [ ] `cloud-sql-proxy` is already running (systemd, auto-started on boot) by the time SSH login completes; connecting via DBeaver (`127.0.0.1:5433` through an SSH port-forward, username `ikaro-relay-vm@ikaro-staging.iam`, empty password) or a direct `psql` to `10.27.0.3` succeeds — closes the original Cloud SQL half of this TD.
...
- [ ] IAP tunnel connections to the relay VM, and every Google Cloud API call made from inside it (Secret Manager reads, identity-token minting — all attributed to the relay VM's own service account, correlatable with the IAP log entry for which human SSH'd in), appear in Cloud Audit Logs — verified by inspecting logs after a real session. (In-VM shell/`psql`/DBeaver activity is out of scope for this criterion — Cloud Audit Logs cannot capture it; that's `pgAudit`'s job, already enabled.)
