...
- IAP tunnel connections land in Cloud Audit Logs — this requires explicitly enabling Data Access audit logging for `iap.googleapis.com` (`google_project_iam_audit_config`; IAP's own docs are explicit this is opt-in, not on by default), same guarantee §2 already claims for backend access, actually delivered this time. Arbitrary commands run inside the VM (raw shell, `psql` queries) are **not** covered — Cloud Audit Logs only records Google Cloud API calls, not in-VM shell activity; query-level Postgres activity is `pgAudit`'s job instead (already enabled by `modules/database`), a separate log.
...
- `google_secret_manager_secret_iam_member` granting `roles/secretmanager.secretAccessor` on the `platform-admin-key` secret to the admin identity — currently only the `backend` runtime SA has this (`modules/iam/main.tf`); without it, the tenant-provisioning acceptance criterion has no way to read the value it needs to send as `X-Platform-Admin-Key`.
- `google_project_iam_audit_config` enabling Data Access audit logging (`ADMIN_READ`/`DATA_READ`/`DATA_WRITE`) for `iap.googleapis.com` — IAP's own docs are explicit that tunnel-access logging is opt-in, not on by default; without this, the Cloud-Audit-Logs acceptance criterion has nothing actually turning it on.
- Wire into `envs/staging/main.tf` (and `envs/prod/main.tf` once needed) with `create = false` by default.
- `tests/*.tftest.hcl` covering the count-gate (`create=true` plans the instance, `create=false` plans zero) and the firewall's source range, per `infra/terraform/README.md`'s unit-test convention.

**Identity used inside the VM: the human operator's own credentials, not a new VM service account.** `google_sql_user.iam_admin` (`modules/database/main.tf`) already registers `iam_admin_user` as a Cloud SQL IAM user, and `admin@ikaro.online` already has `roles/run.invoker` on the backend directly. Re-authenticating as yourself inside the relay VM (`gcloud auth login --update-adc` — sets both the gcloud CLI's active account and Application Default Credentials in one step; `gcloud auth application-default login` alone only does the latter and leaves `gcloud auth print-identity-token`/`gcloud secrets versions access` with no active account, a real gap caught in cross-tool PR review) reuses both grants with zero new IAM surface — the only gap is the `platform-admin-key` accessor grant above. Granting the VM's own service account instead would mean duplicating every one of these grants onto a new principal for no benefit.

**Terraform is authoritative for all resources above — never a raw `gcloud compute instances create` script, and never a local `terraform apply`.** The firewall rule, the three IAM grants, and the audit-log config are permanent, always-applied config (inert, not billable, no reason to ever toggle them); only the VM resource itself is `count`-gated. A standalone script that created the VM outside Terraform would drift from tracked state and risk an orphaned (and quietly billed) instance if the matching teardown step is ever skipped — exactly the ad-hoc-infra pattern this project has consistently moved away from (e.g. the `enable_database`/`bootstrap_mode` flag removals). Toggling `create_relay_vm` happens exclusively through the PR-per-toggle flow described above — a thin wrapper script (e.g. `scripts/relay-vm-up.sh` / `scripts/relay-vm-down.sh`) may automate flipping the tfvars value, committing, and opening the PR, but it must never call `terraform apply`/`terraform destroy` directly.
...
- [ ] `admin@ikaro.online`, re-authenticated inside the relay VM via `gcloud auth login --update-adc`, can read `platform-admin-key` (`gcloud secrets versions access latest --secret=platform-admin-key`) — proves the new secret-accessor grant is correctly wired.
...
- [ ] The staging demo tenant (`{"name":"Ikaro","slug":"ikaro","adminEmail":"lm.moreira@gmail.com","country_code":"BR"}`) is provisioned end-to-end through the relay VM against `POST /internal/tenants`, closing M17-S27's own tenant-provisioning AC item. (`country_code` added here — `ProvisionTenantSchema`, `apps/backend/.../provision-tenant.dto.ts`, requires it; the original example body would have 400'd, caught while writing `modules/relay-vm/README.md`'s own usage example.)
...
- [ ] IAP tunnel connections to the relay VM, and every Google Cloud API call made from inside it under the re-authenticated identity (secret reads, identity-token minting), appear in Cloud Audit Logs — verified by inspecting logs after a real session. (In-VM shell/`psql` activity is out of scope for this criterion — Cloud Audit Logs cannot capture it; that's `pgAudit`'s job, already enabled.)
