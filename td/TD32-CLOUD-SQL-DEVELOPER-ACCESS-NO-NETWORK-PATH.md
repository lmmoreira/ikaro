...
- [ ] `admin@ikaro.online`, re-authenticated inside the relay VM via `gcloud auth application-default login`, can read `platform-admin-key` (`gcloud secrets versions access latest --secret=platform-admin-key`) — proves the new secret-accessor grant is correctly wired.
