...
- **Priority**: Medium — resolved for the current Cloud SQL, internal Cloud Run, and tenant-provisioning use cases by the IAP relay VM; future relay capabilities still need their own review
...
- M17-S27's own Step 1 AC (verifying dev-machine proxy login to Cloud SQL) and Step 4 AC (tenant provisioning via `gcloud run services proxy`) could not be completed as originally documented.
- The IAP relay VM now supplies the required VPC network path. Future direct-access use cases still need to follow the relay's scoped IAM and on-demand lifecycle.

## Implemented approach — IAP relay VM
...
The implemented relay extends the zero-trust pattern with a VPC-internal, IAP-reachable VM. It is the supported path for the internal backend and tenant provisioning; `gcloud run services proxy` remains historical and does not bypass internal ingress.
...
### Implemented Terraform shape (`modules/relay-vm/`)

- `google_compute_instance "relay"` — `count = var.create ? 1 : 0`, `machine_type = "e2-micro"`, network interface on the existing `subnet_id` (no `access_config` block → no external IP). Startup installs the pinned `cloud-sql-proxy` systemd service and the bundled `/usr/local/bin/provision-tenant.sh` script; the latter calls the internal backend directly with a metadata-server identity token.
...
- `google_service_account "relay"` + `google_project_iam_member` (`roles/cloudsql.client`, `roles/cloudsql.instanceUser`) + `google_sql_user "relay"` (`CLOUD_IAM_SERVICE_ACCOUNT`, `.gserviceaccount.com` suffix trimmed per Google's IAM database auth docs) — registers the relay VM's **own** service account as a passwordless Cloud SQL IAM user whenever a real database instance is configured (`var.db_instance_name != ""`), independent of the on-demand VM toggle. This keeps the database identity and its migration-managed read grants stable across VM up/down cycles. Production has no instance yet; when it is created, add the real instance name and connection name to `infra/terraform/foundation/envs/prod/terraform.tfvars`, apply Foundation first, and only then run application migrations.
- `google_secret_manager_secret_iam_member` grants `roles/secretmanager.secretAccessor` on both `platform-admin-key` and `internal-api-key` to the relay VM's **own** service account. The bundled provisioning script reads both values at execution time.
...
- [ ] From inside a relay VM created from this automation (no manual auth step — the relay SA authenticates automatically via the metadata server), the bundled provisioning script reads both `platform-admin-key` and `internal-api-key` through Secret Manager — Terraform tests prove the grants are wired to the relay SA, but the new self-fetch path still needs a live exercise.
...
- [x] From inside the relay VM, the bundled script uses a metadata-server-minted identity token against the backend and reaches the app's own JSON health response (not Google's generic ingress-block 404) — closes the Cloud Run half of this TD.
- [x] A staging tenant was provisioned end-to-end through the relay VM against `POST /internal/tenants`, using a designated authorized staging mailbox supplied at execution time. (`country_code` is required by `ProvisionTenantSchema`.)
...
- [x] `plan/M17-CLOUD-DEPLOY.md` §2's "Developer → Cloud SQL" row and D4 describe the actual, built mechanism; the operator procedure is in `docs/RUNBOOKS.md`.

## Impact on M17-S27

Both AC items were tracked here instead of being hotfixed inside S27's runbook (per S27's own Step 5: "document every gap found as a follow-up issue — do not hotfix silently"). The IAP relay VM now closes the network-path gap for both Cloud SQL and internal Cloud Run access; Cloud SQL Studio remains available for database inspection when the relay is not enabled.
