...
- [x] From inside the relay VM (no manual auth step — the relay SA authenticates automatically via the metadata server), the bundled provisioning script reads both `platform-admin-key` and `internal-api-key` through Secret Manager — proves the secret-accessor grants are wired to the relay SA, not the human.
