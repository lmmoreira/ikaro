...
**Cross-app duplication found and fixed (2026-07-14, SonarCloud):** writing the same interceptor logic twice (once per app) tripped SonarCloud's new-code-duplication gate (11.7% on new code, required ≤3%) — correctly, since the two files were now near-identical. Extracted the shared logic into `packages/nestjs-http` (already the designated "shared NestJS HTTP infrastructure for backend and bff" package) as `BaseErrorInterceptor`, typed against `BaseAppLogger` from `@ikaro/observability` (added as a new dependency of `@ikaro/nestjs-http`, following the same abstract-base/thin-per-app-subclass pattern `BaseAppLogger`/`AppLogger` itself already established). Each app's `ErrorInterceptor` is now a 3-line subclass that only wires in its own `AppLogger` instance; the full branch-coverage test suite lives once in `packages/nestjs-http/src/base-error.interceptor.spec.ts`, and each app's own spec is a one-line wiring smoke test. No behavior change — same logic, one physical copy instead of two.

**Acceptance criteria:**
- [x] Every error response with a `code` produces a structured log entry carrying that code — `ErrorInterceptor` in both apps logs `code`/`path`/`method` via `AppLogger`, `warn` for 4xx and `error` for 5xx
...
- [x] No cross-app code duplication — shared logic lives once in `packages/nestjs-http`'s `BaseErrorInterceptor`, verified by SonarCloud's new-code-duplication gate

**DoD:** Additive observability plus one real fix: backend's unhandled-error response shape now matches the RFC 9457 `ProblemDetail` contract everywhere else in the codebase, closing a standing gap rather than working around it. True OTel span attribute for `error.code` deferred to `M17-S33`.
