...
#### Story 5 — Staff context: codes + fix the missing VO-error branch (real bug)
...
#### Story 7 — Platform context: codes (heaviest single-context load) + dead-class decisions
...
**Scope:** all ~15 BFF-originated error sites cataloged above, `active-staff.guard.ts`, `uploads.controller.ts`, `customers.controller.ts:119`, `auth-controller-flow.service.ts:131`.

**Work required:**
1. Convert every ad-hoc BFF-originated error object to the canonical envelope with a `BFF_*` or `AUTH_*` code.
2. Fix `uploads.controller.ts`'s Nest-default `BadRequestException(string)` to use the canonical shape instead (3rd incompatible shape eliminated).
...
4. **Bug fix**: `active-staff.guard.ts` currently discards the backend's real error and substitutes a generic 503 because it can't use the request-scoped `BackendHttpService` (documented DI-scope reason). Align its error handling with `BackendHttpService.call()`'s passthrough behavior as closely as the scope constraint allows — at minimum, preserve the backend's `code`/`detail` when available instead of always substituting a generic message; keep the existing fail-open-on-404 behavior unless the user wants that revisited (flag it, don't silently change security-relevant fail-open logic in this story).
5. **Security review (§10)**: guest-token validation errors (`bookings.controller.ts:250-258,488-524`) and the tenant-not-registered checks (`auth-controller-flow.service.ts:152-159,191-198`) each reveal something about token/account state. Evaluate each for enumeration risk the same way as Story 5; collapse to a generic code where the specific reason shouldn't be observable to the caller.
6. **Verification**: grep both `apps/backend/src` and `apps/bff/src` for built-in NestJS pipes (`ParseUUIDPipe`, `ParseIntPipe`, etc.) used on route params across all controllers. `uploads.controller.ts`'s Nest-default shape proves at least one such gap exists; confirm whether there are others and fold any found into this story's scope rather than leaving a 5th uncatalogued shape.

**Acceptance criteria:**
- [ ] All BFF-originated errors use the canonical envelope with a `code`
...
- [ ] `ActiveStaffGuard` preserves backend error information where DI scope allows; any change to its fail-open-on-404 behavior is called out explicitly and confirmed with the user, not silently altered
- [ ] Guest-token and tenant-registration error codes' specificity vs. enumeration-risk tradeoff is documented as a deliberate decision
- [ ] Every built-in NestJS pipe usage across both apps is confirmed to either already use the canonical shape or is fixed to use it in this story
...
3. Sync `docs/ENGINEERING_RULES.md` (new § Exception handling & i18n pattern), `docs/24-BFF_ARCHITECTURE.md` (BFF error passthrough contract), and `docs/14-API_CONTRACTS.md` (canonical `ProblemDetail` shape) to the final pattern. The `docs/ENGINEERING_RULES.md` section must include a concrete "adding a new error" checklist: (1) add the code to the relevant literal union in `error-codes.ts` — the compiler rejects step (3) below until this is done, per §9; (2) add a translation entry to both locale files — the Story 17 exhaustiveness test rejects a missing one; (3) construct/throw the error with the typed constructor from Story 2.
...
- [ ] `docs/ENGINEERING_RULES.md`, `docs/24-BFF_ARCHITECTURE.md`, `docs/14-API_CONTRACTS.md` reflect the final pattern, including the explicit "adding a new error" checklist
