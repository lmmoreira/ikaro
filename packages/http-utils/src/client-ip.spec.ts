import { getClientIp, getTrustedClientIp } from './client-ip';
...
describe('getTrustedClientIp()', () => {
  it('reads the default x-real-client-ip header', () => {
    const ip = getTrustedClientIp({ headers: { 'x-real-client-ip': '203.0.113.10' } });
    expect(ip).toBe('203.0.113.10');
  });
  ...
  it('falls back to req.ip when the header is absent', () => {
    const ip = getTrustedClientIp({ headers: {}, ip: '10.0.0.1' });
    expect(ip).toBe('10.0.0.1');
  });
  ...
  it('supports a custom header name', () => {
    const ip = getTrustedClientIp({ headers: { 'x-custom-ip': '198.51.100.5' } }, 'x-custom-ip');
    expect(ip).toBe('198.51.100.5');
  });

  it('ignores unrelated headers such as x-forwarded-for — it never re-derives, only trusts', () => {
    const ip = getTrustedClientIp({
