...
export class AddOutboxLease1748500000001 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "shared"."outbox" ADD COLUMN "lease_expires_at" TIMESTAMPTZ NULL`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_shared_outbox_lease" ON "shared"."outbox" ("lease_expires_at") WHERE "published_at" IS NULL`,
    );
  }
