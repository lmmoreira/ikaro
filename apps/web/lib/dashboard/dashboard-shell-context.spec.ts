...
vi.mock('@/lib/i18n/get-messages', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@/lib/i18n/get-messages')>();
  return {
    ...actual,
    getMessages: vi.fn(),
  };
...
vi.mock('@/lib/api/dashboard/tenants', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@/lib/api/dashboard/tenants')>();
  return {
    ...actual,
