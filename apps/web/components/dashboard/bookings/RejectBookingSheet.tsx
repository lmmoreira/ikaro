...
export function RejectBookingSheet({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(err instanceof Error && err.message ? err.message : t('rejectError'));
    }
  }
    ...
    <div className="fixed inset-0 z-30 flex items-end justify-center bg-black/40 p-0 sm:items-center sm:p-4">
      ...
      >
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <p className="text-sm font-semibold text-gray-900">{t('rejectSheetTitle')}</p>
            <p className="mt-1 text-sm text-gray-500">{t('rejectSheetDescription')}</p>
          </div>
