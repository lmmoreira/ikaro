...
- **There is no `InsertQueryBuilder.onConflict()`** — use `.orUpdate(overwrite, conflictTarget, { overwriteCondition })` with real DB column names, not entity property names. And because `useDefineForClassFields` makes an unassigned class field a real own-property, assert partial-upsert exclusion on the value (`.toBeUndefined()`), never on `in` presence. → `docs/ANTI_PATTERNS.md` § InsertQueryBuilder.onConflict
- **Adding a `CHECK` constraint to an existing table with live rows via a plain `ADD CONSTRAINT` takes `ACCESS EXCLUSIVE` for a full-table validation scan** — split into `ADD CONSTRAINT ... NOT VALID` + a separate `VALIDATE CONSTRAINT` (the weaker `SHARE UPDATE EXCLUSIVE`) instead, even when the checked columns are new in the same migration. → `docs/ANTI_PATTERNS.md` § A plain `ALTER TABLE ... ADD CONSTRAINT ... CHECK
- **A domain event drained into the outbox needs at least one real `eventBus.subscribe()`/`triggerBus.registerTrigger()` consumer before it ships** — the Pub/Sub topic auto-generator can't see an event that's only constructed and published, so every outbox sweep fails permanently once deployed wherever the topic doesn't exist. A logger-only handler is fine if there's no real business consumer yet. → `docs/ANTI_PATTERNS.md` § A domain event is drained into the outbox
...
- **Cloudflare Turnstile's test sitekey never renders an interactive challenge iframe** — it writes a dummy token straight into the hidden `cf-turnstile-response` input; wait on that input's value in E2E, not an iframe selector. → `docs/CI_TRAPS.md` § Cloudflare Turnstile's test sitekey never renders an interactive iframe
...
- **Any user-supplied search term wrapped in a `%...%` LIKE/ILIKE pattern must be escaped first** (`escapeLikePattern()`) — a caller-controlled `%`/`_` is itself a wildcard and can defeat a length-based guard. → `docs/ANTI_PATTERNS.md` § A user-supplied search term is wrapped in a `%...%` LIKE/ILIKE pattern
