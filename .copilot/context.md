...
**5. Monitor CI; triage bot reviews** — run `/pr-land`, which loads its own full mechanics (including the mandatory bot-finding discipline: read → check against real codebase practice → check against the real business scenario → fix or explain why not, with immediate escalation for business/design-only or undeterminable findings) as a skill invocation: `.claude/commands/pr-land.md` § Step 3. It posts the CodeRabbit trigger, dispatches Codex, and loops batching every round's fixes into one commit + one push, re-dispatching Codex only for the next round, until CI is green and Codex reports 0 unresolved Critical/Important.
