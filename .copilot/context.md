...
- **Race conditions — 3 primitives, picked by shape, not by "add a lock":** (1) DB exclusion constraint — rows already exist, invariant is "no two can overlap"; (2) `findByIdForUpdate()` real row lock — a row already exists and must be read-then-written consistently, and it bypasses any read cache in front of the normal `findById()`; (3) `pg_advisory_xact_lock` via a dedicated lock port — no row exists yet to lock. A lock port starts bounded-context-local; promote to `shared/` only once a second consumer needs the *same primitive*, not merely "also has a race" (M21-S03 precedent — `ITenantLockPort` was promoted to `shared/` for a settings race that actually needed primitive 2, not primitive 3, then walked back). Full decision framework: `docs/ENGINEERING_RULES.md` § Choosing a race-condition primitive, and where its lock port should live.
...
- **A domain event drained into the outbox with zero real `eventBus.subscribe()`/`triggerBus.registerTrigger()` consumers gets no Pub/Sub topic from the auto-generated catalog** (`packages/infra-scripts/src/pubsub-catalog.ts` only discovers a topic from a real subscribe/register call site — it has no way to see an event that's merely constructed and published). The aggregate still publishes it correctly per the outbox auto-flush invariant, so every sweep tick fails permanently once deployed anywhere the topic doesn't exist, with no automatic recovery. Every event a repository drains into the outbox needs at least one real subscriber before it ships — a logger-only handler is sufficient if there's no real business consumer yet (M20-S16 precedent, 2026-09-01: `LeadFormSubmissionReceived` shipped with a deliberate, documented "no consumers yet" design — correct on its own terms — but the missing topic this produced went unnoticed for ~4 days on `ikaro-staging`, surfacing only as the `Ikaro staging — outbox backlog age` alert on 4 permanently-stuck rows — full incident: `docs/ENGINEERING_RULES.md` § Aggregate domain events → outbox (repo auto-flush), `plan/M20-LEAD-FORM-MODULE.md` § M20-S16).
...
- **A shared test builder's default field representing a point in time (e.g. `expiresAt`, `startedAt`) must be computed relative to `Date.now()`, never a hardcoded calendar date.** A fixed date silently drifts from "safely far in the future" into "already expired" as real time passes, and stays invisible until something actually queries for staleness — most commonly a new global/cross-tenant retention-purge job scanning the whole shared integration DB. Confirmed to recur: `ChatbotSessionEntityBuilder`'s hardcoded `startedAt`/`lastMessageAt` first caused this (a leftover session swept up by `ChatbotRetentionPurgeJob`'s cross-file scan, worked around ad hoc in one call site rather than fixed at the builder), then `LeadFormSubmissionBuilder`'s hardcoded `expiresAt` caused the identical failure mode against `LeadFormRetentionPurgeJob` (M20-S04 precedent, 2026-08-25 — full incident: `docs/ENGINEERING_RULES.md` § Shared test-builder date defaults).
- **A new cross-tenant, unscoped system job (a daily retention purge, a global sweep with no `tenant_id` predicate) needs its own standalone index matching its own filter column — an existing `(tenant_id, X)` composite index can't be seeked once the query drops the `tenant_id` filter.** `docs/13-DATABASE_SCHEMA.md`'s Indexing Strategy section documents this as an explicit, narrow exception to "every index MUST start with tenant_id," but the underlying design gap has already recurred twice: `chatbot_messages.IDX_chatbot_messages_created_at` (`AddStartedAtIndexToChatbotSessions`, M19-S07) and `lead_form_submissions.IDX_platform_lead_form_submissions_expires_at` (M20-S04, 2026-08-25, Codex review finding on PR #422 — the story's own draft, which named the composite index by habit, missed it too). When a new story's job mirrors an existing cross-tenant job's *shape*, also check whether it needs to mirror that job's *standalone index*, not just reuse whatever composite index the table already has (full incident: `docs/ENGINEERING_RULES.md` § Standalone index for a cross-tenant system job).
- **After a `git merge`/pull brings in changes to a workspace package's own exports (`packages/types`, `packages/validation`, etc.), rebuild that package before trusting a local type-check or `ci:fast`'s pre-push hook.** These packages are consumed by others via their built `dist/` output, not live source — `ci:fast` (`pnpm lint && ... && pnpm type-check && ...`) never rebuilds dependencies itself, it assumes `dist/` is already current. A stale local `dist/` produces a `TS2339: Property 'X' does not exist` error that looks exactly like a real bug in the freshly-merged source, when the source is actually fine. This is purely a local sandbox risk, never a real-CI one — a fresh GitHub Actions checkout always installs and builds from scratch — but a long local session that merges in upstream changes partway through will hit it (M20-S05 precedent, 2026-08-25: both `packages/types` and `packages/validation` had stale `dist/` in the same push attempt, back to back, each producing a different confusing type error before `pnpm --filter <pkg> run build` — or a full `pnpm build` — resolved it).
- **A worktree's own copied `.env` file can go stale relative to sibling stories merged into `main` after the worktree was created — the same class of risk as the stale-`dist/` bullet above, but for env vars instead of build output.** A test/dev server reading `process.env` at runtime has no way to know a required var was added by a later story; the resulting failure looks exactly like a real regression from whatever was just merged in, not an environment gap. Purely a local sandbox risk, never a real-CI one — a fresh GitHub Actions checkout always gets secrets injected fresh, never a stale local `.env` (M20-S12 precedent, 2026-08-27: a worktree created before M20-S05/S09 landed `TURNSTILE_SECRET_KEY` was missing it from `apps/bff/.env`; every Turnstile-gated BFF test returned 400 instead of its expected 200/404/429/401, looking exactly like the `origin/main` merge had broken error propagation, until `getOrThrow('TURNSTILE_SECRET_KEY')`'s own fail-closed `catch` block was traced and `diff <(grep -oE '^[A-Z_]+=' .env.example) <(grep -oE '^[A-Z_]+=' .env)`-style comparison against `apps/bff/.env.example` found the gap). After merging sibling-story changes into a long-lived worktree, check each touched app's `.env.example` for vars its own `.env` is missing, the same way you'd rebuild a stale `dist/`.
- **Cloudflare Turnstile's test sitekey (`1x00000000000000000000AA`, the only one this repo configures) never renders an interactive challenge iframe — it writes a dummy token straight into its own hidden `cf-turnstile-response` input the moment `render()` resolves.** An E2E wait on `iframe[src*="challenges.cloudflare.com"]` will time out forever regardless of how correct the surrounding widget/CSP code is; wait on the hidden input's value instead. Cost 3 review rounds of otherwise-correct, unrelated fixes before being found via live `page.evaluate()` debugging (M20-S09 PR #433 precedent, 2026-08-26 — full incident: `docs/ENGINEERING_RULES.md` § Cloudflare Turnstile's test sitekey never renders an interactive iframe).
- **A CSP allowance for a new external UI resource (a script, iframe, `fetch`/`connect-src` target) must be scoped to every page a user could soft-navigate *from* into the page that needs it — not just to that one page.** CSP is a document-response header the browser only re-reads on a fresh top-level navigation, never on a Next.js client-side (`next/link`) transition; a route reachable via `<Link>` from a page whose own CSP lacks the allowance stays silently blocked with no console-visible error, no matter how correct the destination page's own CSP computation is. Scope the allowance to the whole route subtree reachable via soft navigation (mirroring `needsMapsFrameSrc`'s existing tree-wide scoping), not to the single page that actually consumes the resource (M20-S15 precedent, 2026-08-28: `needsTurnstileSrc()` scoped Turnstile's CSP allowance to `/[slug]/lead-form` only; the hotsite home page's CTA `<Link>` into it is a soft navigation, so a guest who loaded the home page fresh kept its Turnstile-less CSP the whole time and the widget silently never rendered, masked in every existing E2E spec because they all used `page.goto()` directly instead of a real soft navigation — full incident: `docs/ENGINEERING_RULES.md` § CSP allowances for a new external UI resource must be scoped to what a fresh document load can carry, not to the one page that uses it).
- **A new full-page hotsite component must explicitly paint `backgroundColor: 'var(--ba-background)'` on a `min-h-screen` wrapper — `applyBranding()` only defines `--ba-*` custom properties on the root element, it never sets an actual background.** Every existing full-page view (login, booking, `InformationCompletionPrompt`, `Unavailable`, booking's `SubmitInfoForm`) already does this explicitly; a component that skips it falls through to the browser's default white regardless of the tenant's real branding — invisible in CI (jsdom-based axe-core doesn't compute real rendered contrast) and only surfaces against an actual dark-themed tenant in a real browser (M20-S09 PR #433 precedent, 2026-08-26: all 5 lead-form states shipped without it, passing 41/41 unit tests, and were unreadable white-on-white in production for any dark-themed tenant — full incident: `docs/ENGINEERING_RULES.md` § Hotsite full-page components must explicitly paint `--ba-background`).
- **A documented invariant that field X must never appear inside field Y is not actually enforced unless something validates it at the request boundary — a doc comment plus a client-side "strip before sending" helper is a UI courtesy, not an API contract.** This bites hardest when Y's own schema is an unconstrained record (`z.record(string, unknown())`, common for per-module-type `data` fields whose shape isn't statically derivable) — nothing then stops a caller from embedding X's field names directly inside Y instead of at the top level (M20-S08 precedent, 2026-08-26: `audienceMode`/`questions` could be embedded inside a `LEAD_FORM` module's own `layout[].data`, bypassing `LeadFormConfig`'s validation and reaching the public-cached manifest — full incident + fix: `docs/ENGINEERING_RULES.md` § Schema-level enforcement of "never persisted here" invariants).
- **Before skipping a migration's backfill because "no source data can exist yet," check whether the *endpoint/controller* that writes to the source table has already merged to `main` — not whether a dedicated front-end page for it has shipped.** A backend endpoint is a live, callable traffic path the moment it merges and deploys, independent of whether any UI page calls it yet (a direct API call, a smoke test, or another integration can reach it first). This exact mistake happened and was reversed within the same PR: a backfill was dropped reasoning "the guest-facing page hasn't shipped," when the actual public submission *endpoint* had already merged in an earlier story — real data could already exist (M20-S08 precedent, 2026-08-26 — full incident: `docs/ENGINEERING_RULES.md` § Migration backfills).
- **Any user-supplied search term wrapped in a `%...%` LIKE/ILIKE pattern must be escaped first — a caller-controlled `%` or `_` is itself a LIKE wildcard, not literal text, and can defeat a length-based guard that assumes the term is real search content.** A minimum-length gate meant to guarantee a `pg_trgm` index has an extractable trigram (e.g. "reject search terms under 3 characters") does not protect against a wildcard-only term like `%%%` — 3 characters, passes the gate, matches every row with zero real selectivity, silently degrading the query to a full scan the index exists specifically to avoid. Use `escapeLikePattern()` (`apps/backend/src/shared/utils/escape-like-pattern.ts` — backslash-escapes `\`, `%`, `_`) on the raw term *before* wrapping it in `%...%`, not after (M20-S12 precedent, PR #434 round 3, 2026-08-27 — Codex review finding: `search`/`filters[].value` on the new `lead_form_answers` trigram-search feature had no escaping, letting `%%%` bypass the 3-char guard entirely).
- **A Cloud Run service's `vpc_egress` mode determines whether its outbound calls to public destinations even reach the VPC's firewall/NAT layer at all — `PRIVATE_RANGES_ONLY` routes public-IP-destined traffic over Cloud Run's own default internet path, bypassing the VPC (and any NAT/firewall configured there) entirely; `ALL_TRAFFIC` forces every outbound call through the VPC, where it needs a real Cloud NAT to reach the internet at all.** Before adding new network infrastructure (Cloud NAT, an egress firewall rule, a forward proxy) to let a restrictively-egressing service make a new third-party call, check whether a service with a more permissive egress mode already in this codebase (the backend's `PRIVATE_RANGES_ONLY`, already used for the OpenRouter/LLM adapters) can host the call instead — relocating the call is very often simpler and safer than building new network infrastructure to widen the restrictive service's blast radius. Also worth knowing: an IP-CIDR-based egress firewall allow-list is a weak restriction against a shared-edge third party like Cloudflare — its published IP ranges front a huge number of unrelated domains, so "allow Cloudflare's ranges" is much closer to "allow most of the internet" than "allow this one specific hostname"; a genuine hostname-level restriction needs Cloud NGFW FQDN-based firewall objects (Enterprise tier) or a forward proxy (Secure Web Proxy), not a plain IP-based rule (M20-S14 precedent, 2026-08-27: a staging lead-form submission failed `BFF_TURNSTILE_VERIFICATION_FAILED` on every attempt because `TurnstileService.verify()`'s call to Cloudflare's siteverify endpoint had no route out of the BFF's `ALL_TRAFFIC`-egressing, NAT-less VPC — full incident + investigation: `docs/ENGINEERING_RULES.md` § Cloud Run `vpc_egress` mode determines third-party outbound reachability, `plan/M20-LEAD-FORM-MODULE.md` § M20-S14).
- **A new `no-restricted-syntax` selector in `apps/web/eslint.config.js` (or its backend/BFF equivalents) must be checked against the file's own 3 already-documented bypass classes — computed-literal member access, a bare non-member call, and a value nested inside a `JSXExpressionContainer`/conditional rather than directly on the JSX attribute — before considering it complete, not discovered one at a time across review rounds.** All 3 have an existing example selector in the same file to copy from (see `RAW_FETCH_SELECTOR` for the first) (TD37-S23 precedent, PR #450, 2026-08-31: all 3 classes were rediscovered one at a time across 4 separate Codex review rounds while adding 3 new selectors, despite the first class's own fix already sitting in the same file being edited — full incident: `docs/ENGINEERING_RULES.md` § `no-restricted-syntax` selectors must be checked against every already-documented bypass shape).
- **Before a blind `Write` on a file believed to be new, grep for its expected exported symbols first — a file's absence from the specific area you're working on is not proof of its absence from the repo.** The `Write` tool's own "must `Read` an existing file first" safeguard only tracks files *this session* has read, not actual on-disk state, so an existing, already-consumed file can be silently overwritten with no warning (M21-S04 precedent, 2026-09-02: `resource-route.ts` was blindly `Write`-created believing it to be new; it already existed from the section's original implementation and was already imported by `BottomNav.tsx` — harmless by luck since the rewritten logic was equivalent, but the sibling `.spec.ts` file's overwrite silently dropped a test case, caught only by manually diffing against `git log --follow` — full incident: `docs/ENGINEERING_RULES.md` § Before a blind `Write` on a file believed to be new, grep for its expected exported symbols first).
- **Adding one new simple test to a spec file can retroactively make SonarCloud's `S5976` flag a *new* 3+ duplicate-test group among pre-existing tests it hadn't flagged before** — the rule's threshold is 3-or-more structurally-similar test bodies, and a single addition can tip an already-existing, previously-unflagged pair over that threshold. Grep the file for other tests sharing the same simple shape before adding a new one; parameterize proactively rather than discovering it in a second bot-review round (M21-S04 precedent, 2026-09-02: fixing one flagged trio by parameterizing it, then separately adding one new simple test, formed a brand-new flaggable trio out of that test plus two unrelated, pre-existing tests that had coexisted unflagged for months — full incident: `docs/ENGINEERING_RULES.md` § SonarCloud's duplicate-test rule (`S5976`) can retroactively flag pre-existing tests once a new similarly-shaped test is added).
- **A lock (advisory or row-level) only orders callers who both explicitly acquire it — it guarantees nothing about whether a "fresh" re-read after acquiring it is actually fresh, if that read's normal method goes through an independent cache the lock has no relationship to.** Before trusting a lock to protect a read against a concurrent write, check whether that read's normal code path is backed by a `CachingXxxRepository` — if so, pair the lock with a cache-bypassing read method (this codebase's `findByIdForUpdate()` pattern), not just correct ordering between callers (M21-S03 precedent, PR #460 round 7, 2026-09-04: an advisory-lock design around a tenant-settings TOCTOU check correctly serialized two concurrent callers, but the "fresh" re-read inside the winning transaction still went through `CachingTenantRepository`'s 60s-TTL cache, so the lock provided zero actual freshness guarantee — full incident: `docs/ENGINEERING_RULES.md` § A lock only orders callers who both acquire it — it does not bypass an independent cache sitting behind the read it's protecting). **The identical failure mode also applies to a same-request in-memory read taken before the lock was acquired, not just a cache** — an aggregate loaded pre-lock and blindly `save()`d post-lock can silently clobber a concurrently-committed write even with zero cache involved; re-read fresh (e.g. `findById()`) *after* acquiring the lock whenever the use case's post-lock write depends on state that could have changed concurrently (M21-S06 precedent, PR #461 round 1, 2026-09-04 — `UpdateResourceUseCase`'s blind `save()` on a stale in-memory `isActive` could silently undo a concurrent cascade deactivation, caught by CodeRabbit review — full incident: `plan/M21-MULTIVERTICAL-FOUNDATION_IMPLEMENTATION_DETAILS_DEVELOPER.md` § 7).
- **An advisory lock's key format (or any other computed key that must match exactly across concurrently-running instances for the same logical entity — a cache key, a hash-routing key) cannot be changed once it has ever protected real production traffic, without accounting for a rolling/blue-green deploy running old and new code side by side.** Old and new instances would compute *different* keys for the identical real-world entity during the deploy window, so the lock never actually serializes them against each other — silently reopening whatever race it exists to close, with no error anywhere. A brand-new key with nothing live to desynchronize against can be namespaced freely from day one (M21-S06 precedent, PR #461 round 1, 2026-09-04: a new `lockTenantStaff` key safely got a `tenantstaff:` namespace prefix; applying the identical "cleanup" to the already-live `lockTenantDay` key — caught before merging by Codex review — would not have been safe. `lockTenantDay`'s key stays byte-for-byte unchanged — full incident: `docs/ENGINEERING_RULES.md` § Choosing a race-condition primitive).
- **A new event handler's class name must be unique across the whole codebase, not just within its own context — `packages/infra-scripts/src/pubsub-catalog.ts`'s topic/subscription generator keys every `static readonly` class property by bare class name, with no file/module qualifier.** Two independent handlers both named e.g. `TenantProvisionedHandler` in different contexts throws a generator conflict at CI's catalog-generation step. Qualify the class name with its owning context from the start whenever a second context needs "the same shaped handler" for the same event (e.g. `TenantProvisionedNotificationHandler`, `TenantProvisionedBookingHandler`) (M21-S02 precedent, 2026-09-02 — full incident: `docs/ENGINEERING_RULES.md` § Event Handlers).
- **When a bot review (or any mid-PR discovery) prompts extending an existing algorithm to a genuinely new dimension — not just fixing the one gap flagged — re-derive the new code against every invariant already documented for that feature area in the same doc file, not only the specific gap that triggered the change.** A general-rule sentence sitting near the algorithm being extended is a checklist item to verify against, not ambient context that can be skimmed past because it predates the current change (M21-S03 precedent, PR #460 rounds 8-9, 2026-09-04: `docs/02-DOMAIN_MODEL.md` already stated, before any resource-scoping work began, that "a resource opening never bypasses a tenant-wide closure" — the round-8 fix that made `AvailabilityService` resource-aware violated that exact invariant, caught one round later as a fresh Critical finding on code that had existed for exactly one round — full incident: `docs/ENGINEERING_RULES.md` § Re-check a same-file documented invariant when extending an existing algorithm to a new dimension mid-PR).
- **`packages/architecture-check`'s `transactional-save` detector requires a repository `save()` call to be textually, directly inside the `txManager.run(async () => {...})` callback — not merely reachable through a separately-defined method the callback calls.** A `save()` call that's transactionally correct at runtime still gets flagged if it's defined in a helper method called from the callback rather than written inline. When extracting a use case's post-lock logic into a validation helper, keep `save()` literally inline in the `txManager.run()` callback and have the helper do validation only (M21-S03 precedent, PR #460 round 7, 2026-09-04: `OpenScheduleUseCase`'s post-lock logic was first extracted into one `validateAndSave()` helper that also called `save()`, flagged by the detector; fixed by renaming to `validateUnderLock()` (validation only) and keeping `save()` inline in the callback — full incident: `docs/ENGINEERING_RULES.md` § `architecture-check`'s `transactional-save` detector requires `save()` to be textually inside `txManager.run()` — not merely reachable through it).
...
- **A Terraform variable with `type = string` and no `default` is "required" only in the sense that *some* value must be supplied — it does not reject an empty string.** `TF_VAR_x=""` (e.g. an unset GitHub Actions repository `vars.X` interpolating to an empty string in `${{ vars.X }}`) satisfies "a value was provided" and `terraform plan` proceeds normally; only an explicit `validation` block with a real format check (e.g. `can(regex("^[0-9a-f]{32}$", var.x))`, matching `cloudflare_zone_id`'s own pattern) actually fails closed on empty. Caught live, twice in the same story: `turnstile_site_key` and (immediately after, same fix applied) `cloudflare_account_id` both passed a "green" `Terraform plan (prod)` CI run with the real env var resolving to `""` — confirmed only by reading the actual job log's `TF_VAR_x:` line, not by trusting the checkmark (M20-S05 PR #423 precedent, 2026-08-25).
- **When a bot review flags "this PR deletes/removes X" and X should obviously still exist (a recently-merged feature, a documented rule), check whether the branch is simply stale relative to `main` before assuming a real regression.** A branch forked before sibling stories merged makes an honest diff *look* like deletion — the PR's own file state never had X, so GitHub's diff renders it as removed. Confirm with `git merge-base --is-ancestor <suspected-missing-commit> HEAD` (or `git log HEAD..main` for the full list of what's missing). **If `git merge-base` returns nothing and exit 1 with no error text at all** (not "no common ancestor" or any other message), check `git rev-parse --is-shallow-repository` — a shallow worktree clone makes `merge-base` fail silently instead of reporting the real reason, easy to misread as "these branches share no history." The same shallow clone can also make `git merge origin/main` itself fail outright with `fatal: refusing to merge unrelated histories` — a far more misleading message than silence, since it actively suggests a real force-push/history-rewrite rather than "just fetch --unshallow" (M20-S12 precedent, 2026-08-27: the worktree's own root commit was its single most recent commit — history had been truncated right at the tip — confirmed via `git log --oneline --max-parents=0 HEAD` showing that same commit as its own root; `git fetch --unshallow origin` resolved both the `merge-base` and the `merge` failure identically). Fix: `git fetch --unshallow`. **A third symptom, same root cause:** `git show --stat`/`git diff` on a commit sitting at the shallow boundary (no locally-fetched parent) displays the diff against an *empty tree* — every file the branch has ever touched renders as a fresh addition, which can look exactly like an enormous, unintended commit (thousands of files, zero deletions) when the real commit was small and correct. `git fetch --unshallow` resolves this identically to the other two symptoms (M21-S04 precedent, 2026-09-02: a 4-file doc commit briefly looked like a 3416-file, 468K-line commit this way, before unshallowing showed the real, correct diff). The correct remedy for genuine staleness is `git merge main` (never rebase, per the existing branch-conflict rule below), not re-implementing whatever the bot thinks is missing (M20-S05 PR #423 precedent, 2026-08-25: Codex correctly, independently flagged 3 separate "this PR removes X" Criticals — a retention-purge cron, tenant-settings validation codes, and their own doc precedents — all resolved by one `git merge origin/main`, none were real regressions).
- **Before adding any Terraform `output` block, check whether its value derives from a secret/sensitive-marked resource attribute — `infra/terraform/README.md`'s own "Public-repository security" section already forbids this outright** ("Outputs are operational metadata, not secrets. Do not add secret-derived values to outputs; marking an output `sensitive` only masks terminal display and does not remove the value from Terraform state"). This applies even when the goal is a genuinely good one (avoiding an operator ever typing/pasting a raw secret by hand) — an output derived from `cloudflare_turnstile_widget.site.secret` was implemented, opened as its own PR, and reverted unmerged after a live Codex review correctly caught it as Critical, citing this exact doc line. When a resource Terraform manages generates a secret as a side effect of creating something else (here: a Cloudflare Turnstile widget, created mainly to automate its own non-secret sitekey), the secret half still follows this repo's one universal pattern for every other secret — the operator retrieves the value from the vendor's own console/dashboard and runs `gcloud secrets versions add` manually — never a Terraform-mediated extraction path, no matter how that path is dressed up (M20-S05 precedent, 2026-08-25).
...
| A new module type needs admin-only config fields that must never reach the public-cached `layout[].data`, and a generic content-update endpoint for the same aggregate already exists | Add the fields as optional top-level fields on the existing generic endpoint (with a schema-level check blocking them from a module's own `data`) — not a parallel module-specific endpoint. The parallel endpoint ends up duplicating the generic one's entire persist/side-effect machinery (image promotion, cross-aggregate validation) just to accept the same base fields a second time (M20-S01→S08 precedent, 2026-08-26: `UpdateLeadFormModuleUseCase` shipped as a near-complete copy of `UpdateHotsiteContentUseCase`, deleted a few stories later once caught) |
| A use case's `execute()` calls another use case's `execute()` as a thin orchestration step | Check the callee's real caller count first. One caller with no independent reuse (e.g. an original `SubmitLeadFormUseCase` that only ever delegated to `CreateLeadFormSubmissionUseCase`, with nothing else calling either "submit" or "create" independently) means merge them — the split is an artificial seam, not a real boundary. Only keep the delegation when the callee has genuinely independent callers (e.g. `GetLeadFormPublicConfigUseCase`, called by both the submission flow *and* the controller's own `GET config` endpoint) (M20-S05 PR #423 precedent, 2026-08-25) |
| Defaulting a new externally-managed resource (Terraform-created or otherwise) to "one per consuming feature" without checking its own authorization model first | Check whether the resource authenticates/authorizes at a domain or account level rather than a per-feature level — e.g. a Cloudflare Turnstile widget's sitekey authorizes a *domain* to render the challenge, not a specific form. When it does, provision one shared resource and have every current/future feature reuse it; a second resource per feature only fragments the vendor's own analytics/management surface with no security or functional benefit (M20-S05 precedent, 2026-08-25 — `cloudflare_turnstile_widget.site`, renamed from an initial `.lead_form` after exactly this question was raised) |
...
| A new full-page hotsite component sets `color: 'var(--ba-text)'` but no `backgroundColor` | `applyBranding()` only defines `--ba-*` custom properties, it never paints an actual background — wrap in a `min-h-screen` element with `backgroundColor: 'var(--ba-background)'`, matching every existing full-page view (login, booking, `Unavailable`, `InformationCompletionPrompt`). Invisible in CI (jsdom-based axe-core doesn't compute real contrast); only surfaces against a real dark-themed tenant (M20-S09 PR #433 precedent, 2026-08-26 — full incident: `docs/ENGINEERING_RULES.md` § Hotsite full-page components must explicitly paint `--ba-background`) |
...
**Before asking, confirm mergeability directly — a green Codex/CI/SonarCloud-issues read is not sufficient on its own.** Run `gh pr view <PR-number> --repo lmmoreira/ikaro --json mergeStateStatus,mergeable` and only ask once `mergeStateStatus` is `CLEAN`. SonarCloud's aggregate Quality Gate conditions (e.g. `new_coverage`) can fail even with 0 open issues from the issues-search API, and GitHub's own branch-protection evaluation can diverge from what the individual check list shows (TD37-S23/PR #450 precedent, 2026-08-31: asked to merge with 0 Sonar issues and 0/0/0 Codex findings, but `mergeStateStatus` was `BLOCKED` on a `new_coverage` condition invisible to the issues-only check — caught only when the merge command itself failed).
