...
- **Never put cross-service network I/O inside `txManager.run()` — not before, and not as a post-commit side effect either.** An HTTP call to another app/service (cache invalidation, a webhook, a cross-context adapter that leaves the process) holds the DB connection/transaction open for that call's full latency if placed inside the block, risking pool exhaustion and lock contention under load, and couples write durability to an unrelated system's availability. Cross-context side effects that involve network I/O (e.g. `BookingPlatformAdapter.revalidatePublicPages()`, called after service create/update/activate/deactivate) must always run *after* `txManager.run()` returns.
- **A port method documented as "best-effort/never throws" must have its *entire* implementation verified against that contract, not just the one call that looks obviously risky.** `BookingPlatformAdapter.revalidatePublicPages()`'s docstring said "never throws (see `FrontendRevalidationAdapter`)," but only the network leg was actually wrapped in try/catch — the DB read before it (`GetTenantByIdUseCase.execute()`) wasn't, so a missing/errored tenant lookup still propagated past an already-committed write, and a pre-existing unit test (`"throws when the tenant does not exist"`) locked in the wrong behavior instead of catching it (Codex review finding, PR #267, 2026-07-27 — fixed by wrapping the whole method body).
- **A `declare global` augmentation of an interface member a dependency already declares (e.g. BFF `Express.Request.user`, owned by `@types/passport`) silently does nothing under this repo's `skipLibCheck: true`** — TypeScript neither errors on the conflict nor merges the two declarations; the pre-existing one just wins everywhere, while `tsc --noEmit` still passes cleanly. Confirmed empirically (TD31 PR4, 2026-07-29): an `express.d.ts` augmentation type-checked but every consumer's `req.user` still resolved to Passport's own type, not the intended union. Don't trust "`tsc` passed" as proof a `declare global` block took effect — verify the augmented property's inferred type at a real usage site. If the interface is already owned elsewhere, use a shared accessor function instead (`docs/ENGINEERING_RULES.md` § Express `Request.user` typing).
- **A Server Component's `generateMetadata()` that needs the same data as the page/layout body must share one fetch, not re-fetch independently.** Next.js does not deduplicate a plain `async function` data-fetcher across `generateMetadata()` and the component body on its own — only `fetch()` calls that keep the Next.js Data Cache enabled get that for free, and a per-request helper that explicitly disables it (e.g. `revalidate: isDev ? 0 : ...`, common in dev) gets no deduplication at all. Wrap the shared fetch function in React's `cache()` (from `'react'`) so every call site within one request shares a single result. (M18-S03 precedent, 2026-07-29: `app/[slug]/layout.tsx`'s new `generateMetadata()` added a second, undeduplicated `fetchManifest()` call to a layout every customer `/my-account/**` route also shares — under CI's `next dev`, the two concurrent calls occasionally raced and one returned 404, intermittently rendering the global tenant-not-found page mid-test. Root-caused via the actual CI Playwright trace's page snapshot, not local reproduction — see `docs/CI_TRAPS.md`.)
...
- **A Dependabot Docker base-image digest bump must be verified to stay within the same image variant/family before merging** — digest-only `FROM image@sha256:...` pins give Dependabot no tag to anchor updates to, so it can silently resolve to a same-repository-different-variant digest (e.g. `node:22-alpine` → the full Debian `node:22` image), a different OS/package set entirely that can introduce a wave of new Trivy CVEs with no corresponding code change. Before merging any Dependabot Docker PR, confirm the proposed digest still matches the intended variant (`docker manifest inspect <image>@sha256:<digest>` or the Docker Hub tags API — compare image size/OS family against the current digest). Keep the `FROM` line pinned by digest only (no tag) — this repo's SonarCloud rule `docker:S8431` fails the build on the tag+digest form, even though both forms are cryptographically equivalent for pinning; digest-only is the decided convention here (PR #309 precedent, 2026-08-03: Dependabot's proposed bump for `apps/{backend,bff,web}/Dockerfile` silently jumped from `node:22.23.1-alpine` (~55MB) to the full Debian `node:22` image (~390MB), introducing 40 new HIGH-severity `linux-libc-dev` CVEs).
