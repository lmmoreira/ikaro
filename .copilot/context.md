...
- Several invariants are already fully covered in `docs/CODE_STANDARDS.md`/`docs/ENGINEERING_RULES.md` — don't restate them here, go read them: `mapXxxError(err): never` at the HTTP layer (**CI-enforced**: ESLint `HTTP_EXCEPTION_IN_USE_CASE_SELECTOR`); aggregate-driven events auto-flushing via the outbox on `save()`; the domain error base class's `Object.setPrototypeOf` requirement (**CI-enforced**: `prototype-chain` detector); use cases never injecting `RequestContext`; controllers/route files as composition-layer-only; feature-owned transport helpers staying with the feature.
- **Protected-area layouts** read `resolveSupportedLocale(payload.locale ?? 'pt-BR')` from the decoded JWT — never hardcode `'pt-BR'`. **CI-enforced**: ESLint `LOCALE_LITERAL_SELECTOR`.
- **Anything that must exist even for a Guard-rejected request (a trace ID, a request-scoped flag) must be Express middleware, not a NestJS Interceptor** — Nest's pipeline runs `Middleware → Guards → Interceptors`, so an Interceptor never runs for a Guard-rejected request. → `docs/ENGINEERING_RULES.md` § RequestContext (per-request shared state)
- **Never put cross-service network I/O inside `txManager.run()`** — not before, not as a post-commit side effect — and verify a "best-effort/never throws" port's *entire* method body honors that, not just the risky-looking call. **CI-enforced**: `transactional-io` detector + ESLint `TX_MANAGER_PUBLISH_SELECTOR`/`RUN_IN_TRANSACTION_SELECTOR`. → `docs/ENGINEERING_RULES.md` § Transactions
- **A `declare global` augmenting an interface member a dependency already declares silently no-ops** under `skipLibCheck: true`, even though `tsc --noEmit` passes — verify the real type at a usage site, or use a shared accessor function instead. → `docs/ENGINEERING_RULES.md` § Express `Request.user` typing
- **A Server Component's `generateMetadata()` needing the same data as the page/layout body must share one fetch via React's `cache()`**, not re-fetch independently. → `docs/CODE_STANDARDS.md` § shared data fetch between `generateMetadata()` and the page/layout body
- **A field documented as "unset renders identically to today" must check whether today's behavior is an *explicit* value or an *implicit* engine default** before wiring the field's own default through generically. → `docs/15-HOTSITE_DYNAMIC_ARCHITECTURE.md` § implicit vs. explicit CSS defaults
- **Cloud Run timer/async starvation, OTel sampler defaults, and exporter limits have each independently caused silent trace loss** — a timer-driven flush/queue on a CPU-throttled instance can starve (every container, sidecars included, though confirmed as of 2026-08-06 to be low-frequency/intermittent, not the dominant cause); `ParentBasedSampler` needs `remoteParentNotSampled`/`localParentNotSampled` set explicitly or it silently drops most spans (this was the actual dominant cause); `OTLPTraceExporter` needs an explicit `concurrencyLimit`; every dispatcher branch needs its own tracing span, not assumed symmetry; a traces-only bootstrap's metric-reader config must stay in sync with the collector's real pipeline (metrics were re-enabled 2026-08-12 — `metricReaders: []` is no longer the steady state). Before documenting a root cause as confirmed, check it against the target system's own direct evidence, not arithmetic alone. → `docs/ENGINEERING_RULES.md` § Cloud Run CPU throttling — timer/async work can be silently starved (sidecars included) *(actively maintained)*
- **There is no `InsertQueryBuilder.onConflict()`** — use `.orUpdate(overwrite, conflictTarget, { overwriteCondition })` with real DB column names, not entity property names. And because `useDefineForClassFields` makes an unassigned class field a real own-property, assert partial-upsert exclusion on the value (`.toBeUndefined()`), never on `in` presence. → `docs/ENGINEERING_RULES.md` § TypeORM upsert internals
- **Adding a `CHECK` constraint to an existing table with live rows via a plain `ADD CONSTRAINT` takes `ACCESS EXCLUSIVE` for a full-table validation scan** — split into `ADD CONSTRAINT ... NOT VALID` + a separate `VALIDATE CONSTRAINT` (the weaker `SHARE UPDATE EXCLUSIVE`) instead, even when the checked columns are new in the same migration. → `docs/ENGINEERING_RULES.md` § Adding a CHECK constraint to an existing table with live rows
- **A domain event drained into the outbox needs at least one real `eventBus.subscribe()`/`triggerBus.registerTrigger()` consumer before it ships** — the Pub/Sub topic auto-generator can't see an event that's only constructed and published, so every outbox sweep fails permanently once deployed wherever the topic doesn't exist. A logger-only handler is fine if there's no real business consumer yet. → `docs/ENGINEERING_RULES.md` § Aggregate domain events → outbox (repo auto-flush)
...
- **A shared test builder's date-typed default (`expiresAt`, `startedAt`) must be computed relative to `Date.now()`, never a hardcoded calendar date** — a fixed date silently drifts into "already expired" and gets swept by a retention-purge job. → `docs/ENGINEERING_RULES.md` § Shared test-builder date defaults
- **A new cross-tenant, unscoped system job (a daily purge, a global sweep with no `tenant_id` predicate) needs its own standalone index matching its filter column** — an existing `(tenant_id, X)` composite index can't be seeked once the query drops the `tenant_id` filter. → `docs/ENGINEERING_RULES.md` § Standalone index for a cross-tenant system job
- **A merge that changes a workspace package's exports (`packages/types`, etc.) needs that package rebuilt before trusting a local type-check, and a worktree's own `.env` can go stale relative to sibling stories merged after it was created** — both local-sandbox-only risks, never a real-CI one. → `docs/CI_TRAPS.md` § Stale local state after a merge/pull
- **Cloudflare Turnstile's test sitekey never renders an interactive challenge iframe** — it writes a dummy token straight into the hidden `cf-turnstile-response` input; wait on that input's value in E2E, not an iframe selector. → `docs/ENGINEERING_RULES.md` § Cloudflare Turnstile's test sitekey never renders an interactive iframe
- **A CSP allowance for a new external UI resource must be scoped to every page a user could soft-navigate *from*, not just the page that needs it** — CSP is only re-read on a fresh top-level navigation, never a Next.js client-side transition. → `docs/ENGINEERING_RULES.md` § CSP allowances for a new external UI resource must be scoped to what a fresh document load can carry
- **A new full-page hotsite component must explicitly paint `backgroundColor: 'var(--ba-background)'`** on its `min-h-screen` wrapper — `applyBranding()` only defines the `--ba-*` custom properties, it never paints an actual background. → `docs/ENGINEERING_RULES.md` § Hotsite full-page components must explicitly paint `--ba-background`
- **A documented "field X must never appear inside field Y" invariant isn't enforced unless validated at the request boundary** — especially when Y is an unconstrained `z.record(...)` schema; add schema-level enforcement, not just a doc comment or client-side strip. → `docs/ENGINEERING_RULES.md` § Schema-level enforcement of "never persisted here" invariants
- **Before skipping a migration's backfill because "no source data can exist yet," check whether the *endpoint/controller* has already merged to `main`** — not whether a dedicated UI page has shipped; an endpoint is a live traffic path the moment it deploys. → `docs/ENGINEERING_RULES.md` § Migration backfills
- **Any user-supplied search term wrapped in a `%...%` LIKE/ILIKE pattern must be escaped first** (`escapeLikePattern()`) — a caller-controlled `%`/`_` is itself a wildcard and can defeat a length-based guard. → `docs/ENGINEERING_RULES.md` § LIKE/ILIKE pattern escaping for user-supplied search terms
- **A Cloud Run service's `vpc_egress` mode determines whether outbound calls to public destinations even reach the VPC's firewall/NAT layer** — check whether a more permissively-egressing service already in the codebase can host a new third-party call before adding network infrastructure. → `docs/ENGINEERING_RULES.md` § Cloud Run `vpc_egress` mode determines third-party outbound reachability
- **A new `no-restricted-syntax` ESLint selector must be checked against the 3 already-documented bypass classes** (computed-literal member access, a bare non-member call, a value nested in `JSXExpressionContainer`/conditional) before considering it complete. → `docs/ENGINEERING_RULES.md` § `no-restricted-syntax` selectors must be checked against every already-documented bypass shape
- **Before a blind `Write` on a file believed to be new, grep for its expected exported symbols first** — the tool's "must Read first" safeguard only tracks files *this session* has read, not actual on-disk state. → `docs/ENGINEERING_RULES.md` § Before a blind `Write` on a file believed to be new, grep for its expected exported symbols first
- **Adding one new simple test can retroactively make SonarCloud's `S5976` flag a new 3+ duplicate-test group among pre-existing tests** — grep the file for similarly-shaped tests before adding one; parameterize proactively. → `docs/ENGINEERING_RULES.md` § SonarCloud's duplicate-test rule (`S5976`) can retroactively flag pre-existing tests
- **A lock only orders callers who both acquire it** — it doesn't guarantee a post-lock read is fresh if that read normally goes through an independent cache (pair the lock with a cache-bypassing read, e.g. `findByIdForUpdate()`) or reflects in-memory state loaded *before* the lock was acquired (re-read fresh, e.g. `findById()`, after acquiring it). → `docs/ENGINEERING_RULES.md` § A lock only orders callers who both acquire it
- **An advisory lock's key format (or any computed key needing to match exactly across concurrently-running instances) cannot change once it has protected real production traffic** without accounting for a rolling deploy running old and new code side by side; a brand-new key can be namespaced freely. → `docs/ENGINEERING_RULES.md` § Choosing a race-condition primitive, and where its lock port should live
...
- **A Terraform module with its own passing `terraform test` suite is not proof it's actually applied by any real root** — grep for a real `module` block before treating it as precedent, and remember a new secret's IAM accessor role needs three places updated together, not one. → `infra/terraform/README.md` § Gotchas (dead-module / 3-places-IAM precedent)
- **A manual diagnostic Cloud Run deploy can silently pin a service's traffic policy to one revision, and normal CI deploys don't reset that pin on their own** — check `spec.traffic` for `latestRevision: true` after any manual operation. → `infra/terraform/README.md` § Gotchas (traffic-pin precedent)
- **A CI gate built on a path-diff filter checks a static fact about one commit's diff, never live state — `gh run rerun` cannot clear it.** The only recovery is a genuinely new push (an empty commit is fine). → `docs/CI_TRAPS.md` (path-diff CI gate row)
- **A diff-based CI gate is structurally insufficient for "is prerequisite Y currently satisfied"** — verify against a system reflecting current reality (e.g. the GitHub Actions API for whether a workflow actually succeeded), preferring a zero-new-trust signal over a credential from across a deliberately-separated security boundary. → `docs/CI_TRAPS.md` (same row)
- **Migrations that grant privileges to infrastructure-created database roles must enforce provisioning order or provide convergent reconciliation** — a migration that silently skips a missing role is safe only when deployment guarantees Foundation creates the role first. → `docs/ENGINEERING_RULES.md` § Migration-driven privilege grants to infrastructure-created roles
- **Security dependency overrides are temporary compatibility boundaries, not permanent pins** — update the override and lockfile together once a fixed upstream release exists, then verify with the repository scanners. → `docs/CI_TRAPS.md` § Snyk SCA failures
- **A Dependabot Docker base-image digest bump must be verified to stay within the same image variant/family before merging** — a digest-only pin gives Dependabot no tag to anchor to, so it can silently resolve to a same-repo-different-variant digest, introducing a wave of new CVEs with no code change. → `docs/17-GITHUB_WORKFLOWS_GUIDELINES.md` § Docker base-image digest pinning
- **The autonomous implementation chain must not treat "tests pass, lint clean, bots clean" as sufficient for a story touching Terraform/IAM/Pub/Sub/CI-CD** — run the concrete live check the change implies (a module wired into a real root, an IAM grant that actually resolves, a real `terraform plan -refresh-only`) before presenting the PR as merge-ready; a failed or un-runnable check is its own stuck condition (3 separate M19 stories broke on live deploy despite clean bot review — none were spec-ambiguity or code-reading failures).
- **A Terraform `type = string` variable with no `default` does not reject an empty string** — `TF_VAR_x=""` satisfies "a value was provided" and `plan` proceeds normally; only an explicit `validation` block with a real format check fails closed. → `infra/terraform/README.md` § Gotchas (empty-string var precedent)
- **When a bot review flags "this PR deletes/removes X" and X should obviously still exist, check whether the branch is simply stale relative to `main`** (`git merge origin/main`, never rebase) before assuming a real regression — and if `git merge-base`/`git merge` fails oddly on a worktree clone, check `git rev-parse --is-shallow-repository` first (`git fetch --unshallow`). → `docs/CI_TRAPS.md` (stale-branch / shallow-clone entry)
- **Before adding any Terraform `output` block, check whether its value derives from a secret/sensitive-marked resource attribute** — outputs are operational metadata, never secrets; `sensitive = true` only masks terminal display, not removal from state. → `infra/terraform/README.md` § Public-repository security
