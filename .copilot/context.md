...
- **A migration backfilling a newly-derived table doesn't automatically need batching/resumability machinery — check whether the source table could hold meaningful data yet (has a public-facing endpoint/UI writing to it actually shipped) before building defensively for a "production scale" that may not exist.** If the destination is a derived lookup/cache rather than the record of truth, a missing backfilled row is often a self-correcting gap going forward, not a data-loss risk (M20-S08 precedent, 2026-08-26: an unbounded `INSERT ... SELECT ... jsonb_array_elements(...)` backfill was removed entirely, not made resumable, since the source table had no live traffic path yet — full incident: `docs/ENGINEERING_RULES.md` § Migration backfills).
