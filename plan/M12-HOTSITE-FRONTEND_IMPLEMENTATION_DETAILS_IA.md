...
4. **`buttonBackgroundColor`/`buttonTextColor` must be added to BOTH** the backend `HotsiteBrandingSchema` (`update-hotsite-content.dto.ts`) **and** the BFF's separate `UpdateHotsiteContentBodySchema` (`hotsite-admin.schemas.ts`) — the BFF schema is `.partial()` with default Zod "strip" mode; omitting a field there silently drops it from `PATCH /v1/tenants/hotsite` even though backend and frontend tests pass independently.
...
13. **`HotsiteModuleType` must be added in four places when introducing a new module type** — `packages/types/src/enums.ts`, `apps/backend/…/hotsite-config.aggregate.ts` (domain), `apps/backend/…/update-hotsite-content.dto.ts` (Zod enum), `apps/bff/…/hotsite-admin.schemas.ts` (BFF Zod enum). Missing any one silently rejects the new type at the respective validation layer.
