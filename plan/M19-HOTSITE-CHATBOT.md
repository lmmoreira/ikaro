...
**Secret rollout is staged across 3 PRs, not landed in one go like S02's original (pre-TD39) shape — a deliberate deviation, decided during implementation, 2026-08-14.** TD39's resolution (`infra/terraform/README.md`'s gotcha entry) confirms the same live-IAM-read-at-plan-time problem that blocked pubsub topics also applies to `google_secret_manager_secret_iam_member` ("the equivalent for secrets" is named explicitly) — so the `foundation` accessor grant can't land in the same PR as the secret container, same `no-foundation-plus-other-infra-mix` CI guardrail as the pubsub/scheduler split above. Secrets add a second hazard pubsub topics don't have: wiring `secret_env_vars` on `cloudrun_backend` makes Cloud Run resolve the secret at container startup, so wiring it before the backend SA's accessor grant exists would fail that Cloud Run deploy (old revision keeps serving — not a live outage, but a broken CI deploy step). Sequence:
1. **This story's PR** (`envs/*` only): `openrouter-management-api-key` Secret Manager container (`modules/secrets`) + all backend code (job/client/trigger handler/controller route) reading `OPENROUTER_MANAGEMENT_API_KEY` via `ConfigService` + the pubsub topic/scheduler job. **No `secret_env_vars` wiring yet** — the job's own designed fault-tolerance ("API failure → log a warning, leave the row unchanged, never throw") already covers the unset-key window gracefully, so nothing breaks by shipping the code first.
2. **Follow-up PR** (`foundation` only): backend SA's `secretmanager.secretAccessor` grant on `openrouter-management-api-key` (`foundation/modules/runtime-identities` + both env roots), alongside the already-planned `scheduler_publisher_cron-chatbot-balance-poll` grant.
3. **Final PR** (`envs/*` only): wire `OPENROUTER_MANAGEMENT_API_KEY = module.secrets.secret_ids["openrouter-management-api-key"]` into `cloudrun_backend`'s `secret_env_vars` on both env roots — safe now, since the SA can already read it. Also add `.env.example` entry for local dev in whichever of these PRs is most convenient (no ordering constraint locally).
...
**Acceptance Criteria:**
- [ ] Job calls OpenRouter's real credits API using `OPENROUTER_MANAGEMENT_API_KEY` (not `OPENROUTER_API_KEY`) in production; fully stubbed in all automated tests
...
- [ ] Terraform: `google_pubsub_topic` + `google_cloud_scheduler_job` (`*/15 * * * *`) per `docs/14-API_CONTRACTS.md`'s spec — implemented in this story, not deferred to S14 (S07 precedent: `modules/scheduler/variables.tf`'s validation requires every `cron-*` catalog topic to have a matching `locals.jobs` entry, so deferring the job while the mandatory `pubsub-catalog.json` regeneration already registers the topic was never actually viable)
- [ ] Coverage ≥80%; `tsc --noEmit`, lint, tests green

**Dependencies:** S01, S02.
**New secret:** `OPENROUTER_MANAGEMENT_API_KEY` (see Description) — Secret Manager container provisioned in this story; accessor grant and `secret_env_vars` wiring land in 2 follow-up PRs per the staged sequence above.
