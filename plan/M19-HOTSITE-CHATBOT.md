...
**Devops half (resolved during M19-S07 story-discovery, 2026-08-13; corrected during PR #365 review, 2026-08-13):** the `google_pubsub_topic` (`ikaro-cron-chatbot-retention-purge`) is auto-provisioned by `modules/pubsub` from `infra/terraform/pubsub-catalog.json`, regenerated by `pnpm --filter @ikaro/infra-scripts run pubsub-catalog` and diff-checked in CI (`pr-quality.yml`) — a mandatory, automatic consequence of the code change (the new `registerTrigger()` call site). The `google_cloud_scheduler_job` itself (daily, `0 3 * * *`, `modules/scheduler/main.tf`'s `locals.jobs` + `variables.tf`'s validation) is **implemented in this story**, not deferred — story-discovery had proposed deferring it to S14, but `variables.tf`'s own validation requires every `cron-*` catalog topic to have a matching `locals.jobs` entry, so deferring the job while the (mandatory) catalog regeneration already registers the topic was never actually viable; caught by the Terraform plan CI check turning red on PR #365. **What genuinely is deferred:** the `scheduler_publisher_cron-chatbot-retention-purge` IAM grant in `infra/terraform/foundation/envs/{staging,prod}/main.tf` — `foundation`'s `google_pubsub_topic_iam_member` does a live read of the target topic's IAM policy even at plan time, which 404s until the topic exists in the real project, and that topic is created by the separate `envs/*` root/state. Lands in a follow-up PR/story once `envs/*` has actually deployed. Local dev is unaffected either way, since it uses the manual `POST /cron/chatbot-retention-purge` trigger.

**Acceptance Criteria:**
- [ ] Job deletes `chatbot_messages` rows past 180 days, across all tenants in one pass
- [ ] Job deletes now-orphaned `chatbot_sessions` rows (`started_at` AND `last_message_at` past the same 180-day window AND zero remaining messages) — never deletes a session that still has messages
...
- [ ] `infra/terraform/pubsub-catalog.json` regenerated (`pnpm --filter @ikaro/infra-scripts run pubsub-catalog`) and committed in this story, so the new trigger's topic auto-provisions via `modules/pubsub` — CI diff-checks this file, so a missed regeneration fails `pr-quality.yml`, not just leaves an unprovisioned topic
- [ ] `google_cloud_scheduler_job` implemented in this story (see Devops half above) — the `foundation` IAM grant is the only piece deferred, to a follow-up once `envs/*` has deployed
