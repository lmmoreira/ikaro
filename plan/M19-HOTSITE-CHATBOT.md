...
**Devops half (resolved during M19-S07 story-discovery, 2026-08-13):** the `google_pubsub_topic` (`ikaro-cron-chatbot-retention-purge`) is auto-provisioned by `modules/pubsub` from `infra/terraform/pubsub-catalog.json`, regenerated by `pnpm --filter @ikaro/infra-scripts run pubsub-catalog` and diff-checked in CI (`pr-quality.yml`) — this is a mandatory, automatic consequence of the code change (the new `registerTrigger()` call site), not an optional devops task; it must land in this story regardless of the scheduler-job decision below. The `google_cloud_scheduler_job` itself (daily, `0 3 * * *`) is a separate, genuinely deferrable piece — it needs a manual `locals.jobs` entry in `modules/scheduler/main.tf`, a `variables.tf` validation update, and a `scheduler_publisher_cron-chatbot-retention-purge` IAM grant added to both `infra/terraform/foundation/envs/staging/main.tf` and `.../prod/main.tf`. **Decision: deferred to S14** — local dev is unaffected, since it uses the manual `POST /cron/chatbot-retention-purge` trigger.

**Acceptance Criteria:**
- [ ] Job deletes `chatbot_messages` rows past 180 days, across all tenants in one pass
- [ ] Job deletes now-orphaned `chatbot_sessions` rows (`started_at` past the same 180-day window AND zero remaining messages) — never deletes a session that still has messages
...
- [ ] `google_cloud_scheduler_job` itself deferred to S14 (see Devops half above) — not part of this story's scope
