...
**Note (found during M19-S05 story-discovery, 2026-08-12):** like S05, this story's own text names only a use-case class with no backend HTTP controller/route — the same gap S05 had. `GET /public/platform/chatbot/status` (`docs/14-API_CONTRACTS.md`) needs a real backend endpoint for S09's BFF to call, same as S05's `POST .../messages`. Scope this story's own controller the same way S05 did — see S05's Description for the `ServiceController`/`TenantSettingsController` precedent — when this story reaches `/story-discovery`, don't re-derive it from scratch.
