...
**Agent:** `backend-ts`
**Complexity:** M
**Docs to load:** `docs/04-USE_CASES.md` § UC-018/019/020, M11 IA file, S02 implementation (`PubSubPushGuard`, `PubSubPushController`, `GcpPubSubEventBusAdapter`)
...
A cron tick is not a domain event, though — it carries no `tenantId` (the job itself fans out across all tenants internally) and no business fact, only “run now.” Modeling it as a `DomainEvent` subclass (even a `tenantId`-optional one) would let a sentinel/fake tenant leak into logging, OTel span attributes, and DLQ payloads that assume every `DomainEvent` carries a real tenant — a real tension with §2 invariant #3 even if scoped to a subclass. Instead, `IEventBus`/`GcpPubSubEventBusAdapter` gain a second, structurally separate **trigger channel** — `registerTrigger(name, handler)` / `publishTrigger(name)` — dispatched via the same `dispatchPushMessage` routing-by-subscription-name, checked before falling into the `DomainEvent`-typed path. Local dev (pull mode) and prod (push mode) both exercise this identical path: the cron controllers become thin publishers (`publishTrigger(...)`) instead of calling jobs directly, so a local `curl` and a real Cloud Scheduler tick run through the exact same wiring — no parallel implementation to drift apart.

`BookingReminderJob` and `AdminScheduleReminderJob` (bundled today under one `/cron/reminders` call) are independent — verified in code: disjoint tenant loops, own `correlationId`, own published event, no shared state or ordering dependency. Same for `ExpirePointsUseCase` and `NotifyExpiringPointsUseCase` (bundled under `cron-loyalty.controller.ts`): disjoint time windows (past-due vs. not-yet-due), no shared state. Each becomes its own trigger handler — satisfies the “handler calls exactly one use case” rule (§7) and gives each independent Pub/Sub retry/DLQ isolation. This also fixes a latent bug in today’s bundled design: if `BookingReminderJob.run()` throws mid-loop, `AdminScheduleReminderJob.run()` never runs at all for that invocation (sequential `await` in one controller method, no isolation).

**What to implement:**
1. **Port + adapter:** add a trigger channel to `IEventBus`/`IPushableEventBus`/`GcpPubSubEventBusAdapter` — `registerTrigger(name: string, handler: () => Promise<void>)` at bootstrap (mirrors `subscribe()`), `publishTrigger(name: string)` (mirrors `publish()`, called by the cron controllers), and `dispatchPushMessage` checks the trigger map by subscription name before the `DomainEvent`-typed path. Local pull mode: `onApplicationBootstrap` opens a pull subscription per registered trigger the same way it does per domain-event subscription.
2. **Four trigger handlers** (one use case each; `handle()` calls it and rethrows, zero domain logic):
   - `BookingReminderTriggerHandler` (consumer `booking-reminder`) → `bookingReminderJob.run()`
   - `AdminScheduleReminderTriggerHandler` (consumer `booking-admin-schedule-reminder`) → `adminScheduleReminderJob.run()`
   - `ExpirePointsTriggerHandler` (consumer `loyalty-expire-points`) → `expirePointsUseCase.execute()`
   - `NotifyExpiringPointsTriggerHandler` (consumer `loyalty-notify-expiring-points`) → `notifyExpiringPointsUseCase.execute()`
3. **Cron controllers become publishers:** `CronBookingController.reminders()` → `eventBus.publishTrigger(‘cron-reminders’)`, returns `{ ok: true }` once published (not once the jobs complete). `CronLoyaltyController`’s two methods → `eventBus.publishTrigger(‘cron-loyalty-expiry’)`. No `PubSubPushGuard` on these controllers — they stay behind the existing global `InternalApiGuard`, used only as the local/manual trigger path.
4. Keep the underlying jobs idempotent per delivery: the reminder job’s 06:00–06:29 window logic already tolerates repeated invocations — add a spec asserting a second run in the same window doesn’t double-publish reminder events (check existing dedup; if absent, dedup on `(tenantId, date, reminderType)` — verify against M11 implementation before adding anything).
5. Remove/deprecate `CRON_SECRET` on the BFF: it protects nothing anymore (cron never transits the BFF). Delete from `apps/bff/src/config/env.validation.ts` + `.env.example` + docs reference, or mark clearly as local-only if any local tooling uses it — grep first.
6. Local dev: document (README section) how to trigger cron locally — `curl -X POST localhost:3001/cron/reminders` publishes the trigger; the pull-mode consumer (already open per S02 local default) picks it up asynchronously. Note this now depends on the local Pub/Sub emulator being up, and the curl response no longer waits for job completion.
...
- [ ] All four trigger handlers exist, each calling exactly one use case/job
...
3. **Cron topics:** `ikaro-cron-reminders`, `ikaro-cron-loyalty-expiry`, each with **two** independent push subscriptions (S03 splits the previously-bundled controller calls into independent trigger handlers) — all four use the *same* shared push config as point 2 above (`/pubsub/push`, not a dedicated cron URL): `ikaro-cron-reminders-booking-reminder`, `ikaro-cron-reminders-booking-admin-schedule-reminder`, `ikaro-cron-loyalty-expiry-loyalty-expire-points`, `ikaro-cron-loyalty-expiry-loyalty-notify-expiring-points`.
...
**Acceptance criteria:**
- [ ] Both jobs created; `gcloud scheduler jobs run ikaro-cron-reminders` → message lands on the topic → both subscriptions push to `/pubsub/push`, each dispatched to its own trigger handler (verify post-S27; checklist there)
