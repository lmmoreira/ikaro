...
| # | Decision | Rationale |
|---|---|---|
| D1 | **Cloud Run for everything** — backend, BFF, web as services; migrations as a Cloud Run Job. **No Cloud Functions/lambdas, no VMs** for application workloads. **Narrow exception (TD32, 2026-07-24; TD34 control-plane handoff):** a single minimal, on-demand `e2-micro` relay VM (`modules/relay-vm`) runs inside each target environment solely to give an authorized human IAP-only network access to VPC-internal resources (Cloud SQL's private IP, the ingress-internal backend). Its state, identity attachment, and `create_relay_vm` toggle are owned exclusively by the separately protected Foundation root; it runs no application code, defaults off, and is destroyed after each session — infrastructure-access tooling, not a compute workload. D1's actual intent (no rewriting app services into VMs/functions) is untouched. | Existing Dockerfiles run as-is; scale-to-zero gives lambda economics without rewriting NestJS/Next.js into function handlers. |
...
| D4 | **Backend is internal-ingress only.** No public URL, no Cloud Armor + IAP stack (old M15-S12 is cut). | Four layers protect operator provisioning: internal ingress (network) + relay service-account IAM identity + `InternalApiGuard` shared secret (M115-S03) + `PLATFORM_ADMIN_KEY` timing-safe check (M02-S05). **Operator access uses the on-demand IAP relay VM (TD32, 2026-07-24)** — `gcloud run services proxy` does **not** work for this because it remains an external request path. |
...
| D8 | **Single Artifact Registry in `ikaro-prod`;** staging deployer has read+write, so the *same image SHA* validated in staging is promoted to prod (backend/BFF/**web**). | Matches docs/23. **Original exception removed 2026-07-22 (`docs/16-DASHBOARD_FRONTEND_ARCHITECTURE.md`):** web previously needed a per-environment rebuild because `NEXT_PUBLIC_*` vars were inlined into the client bundle at build time. TD29 replaced that with a runtime accessor fed by a root-layout Server Component reading live `process.env` — web now builds once like every other service; environment-specific values are Cloud Run runtime env vars (S25/S26), not build args. |
...
| D11 | **Domain: `ikaro.online`** (owned). Browser traffic uses `ikaro.online` (web, hotsites, and `/v1/*` BFF gateway); `bff.ikaro.online` remains an upstream BFF hostname. | Sessions are host-only cookies on `ikaro.online`; `/v1/*` removes browser cookie sharing between hosts. |
...
`plan/M17-CLOUD-DEPLOY.md`. Prod is unchanged (Story B,
...
| Surface | Control |
|---|---|
| Human access (dev/operator) | Google account + **2FA enforced**; `gcloud auth login`; per-resource IAM. **No permanent bastion** — VPC-internal access (Cloud SQL, ingress-internal Cloud Run) goes through an on-demand IAP relay VM (`modules/relay-vm`, TD32) created by a reviewed Foundation PR plus protected Foundation apply, then destroyed the same way after each session. The VM stays in the target environment's VPC; only its Terraform control plane is Foundation-owned. IAP tunnel access is logged as the human; Secret Manager reads and Cloud SQL IAM logins are logged as the relay VM service account. Metadata-token issuance and in-VM shell activity are not Cloud Audit Log events. |
...
| Tenant provisioning | IAP relay VM inside the VPC + relay service-account IAM identity + `INTERNAL_API_KEY` + `PLATFORM_ADMIN_KEY` (timing-safe, sent via `X-Platform-Admin-Key`). No backend rate limit is currently enforced; the bundled `/usr/local/bin/provision-tenant.sh` script supplies the headers and request body. |
...
**Agent:** `backend-ts`
**Complexity:** L
**Docs to load:** `docs/03-DOMAIN_EVENTS.md`, `docs/ENGINEERING_RULES_BACKEND.md` (event handler rules), `docs/CODE_STANDARDS.md`
...
**Agent:** `backend-ts`
**Complexity:** M
**Docs to load:** `docs/04-USE_CASES.md` § UC-018/019/020, M11 IA file, S02 implementation (`PubSubPushGuard`, `PubSubPushController`, `GcpPubSubEventBusAdapter`), `docs/ENGINEERING_RULES_BACKEND.md` § Event Handlers (test-wiring table) + `docs/ENGINEERING_RULES_SHARED.md` § `RequestContext` is HTTP-request-scoped only (`*.job.ts` category)
...
- [ ] `InMemoryEventBus`/`RoutingInMemoryEventBus` support triggers; `docs/ENGINEERING_RULES_BACKEND.md` test-wiring table has a row for the pattern
...
5. **Implementation mechanism (clarified in discovery, 2026-07-18):** attach the ID token via a single axios request interceptor registered once against the shared `HttpService`, not by patching each call site individually — this covers all three distinct patterns that currently reach the backend: `BackendHttpService`'s `headers()`-based methods (get/post/patch/delete), its `*ForPublic` methods (get/post/patchForPublic — currently build headers inline, bypassing `headers()`), and `HealthController`'s readiness ping (a separate `HttpService` injection in `app.module.ts`, not through `BackendHttpService` at all). Missing any of the three would silently leave that path unauthenticated once the backend goes `--no-allow-unauthenticated`.
...
6. **Org policies** (once org exists): `iam.disableServiceAccountKeyCreation` (enforce — pairs with D6), `iam.automaticIamGrantsForDefaultServiceAccounts` (disable), `compute.requireOsLogin` (defense in depth; no VMs planned at the time — TD32, 2026-07-24, later added the on-demand relay VM described at D1/D4, which enables OS Login at the instance level regardless, so this org policy and that design agree rather than conflict). Document any policy that must be exempted later, with rationale.
...
2. Enable APIs on both: `run.googleapis.com`, `sqladmin.googleapis.com`, `pubsub.googleapis.com`, `secretmanager.googleapis.com`, `artifactregistry.googleapis.com`, `cloudscheduler.googleapis.com`, `compute.googleapis.com` (LB, prod), `servicenetworking.googleapis.com` (private SQL), `iamcredentials.googleapis.com` (WIF), `monitoring.googleapis.com`, `cloudtrace.googleapis.com`, `logging.googleapis.com`, **`iam.googleapis.com` and `cloudresourcemanager.googleapis.com`** (missing from this list originally — found via M17-S24's first real `terraform plan` run, 2026-07-21: reading a service account's or a project's own IAM policy needs both, distinct from `iamcredentials.googleapis.com`'s narrower WIF-token-exchange purpose; neither was auto-enabled by project creation either). Terraform enables `iap.googleapis.com` unconditionally in each environment root, before the TD32 relay module can create its instance-scoped IAP binding; it is intentionally not an operator-run bootstrap command.
...
4. **CI service accounts — three per project, split by blast radius** (review finding, 2026-07-07). **No keys on any** (org policy blocks them):
   - `ikaro-tf-deployer@<project>` — Terraform applies only. Broad infra roles: `roles/run.admin`, `roles/cloudsql.admin`, `roles/pubsub.admin`, `roles/secretmanager.admin`, `roles/compute.networkAdmin`, `roles/iam.serviceAccountAdmin`, `roles/iam.serviceAccountUser`, `roles/storage.admin`, `roles/cloudscheduler.admin`, `roles/monitoring.editor`, `roles/serviceusage.serviceUsageAdmin` (enables Terraform-managed project APIs, including IAP) (prod one additionally `roles/artifactregistry.admin` on the shared registry). `roles/storage.objectAdmin` on the state bucket, **scoped by IAM condition to its own env prefix** (`resource.name.startsWith("projects/_/buckets/ikaro-tfstate/objects/envs/<env>")`) — the staging deployer is mintable from any merge to `main` with no approval, so unscoped access would let it tamper with prod state ahead of the next prod apply. **Corrected/completed by M17-S24 (2026-07-21, found via the first real push-to-main apply — staging's `apply-staging` had never actually run under this identity before):** two grants were still missing. (1) The env-prefix-conditioned `objectAdmin` binding above can gate object-level `get`/`create`/`delete` but never `storage.objects.list` (GCS evaluates `list` against the bucket resource, which has no per-env object suffix to match); fixed with an unconditioned `roles/storage.legacyBucketReader` grant (bucket-wide `list` + metadata `get` only — deliberately excludes `storage.objects.get`, so staging's deployer still cannot read prod's actual state content). (2) Applying any of `modules/iam`'s `google_project_iam_member` resources needs `resourcemanager.projects.getIamPolicy`/`setIamPolicy`, which none of the broad roles above include (they cover IAM policy for their own resource type only); fixed with `roles/resourcemanager.projectIamAdmin` (project-level IAM policy read/write, nothing else). Both fixes applied to both envs' tf-deployer proactively; only staging's apply has actually verified them live (clean "0 added, 0 changed, 0 destroyed") — prod's `apply-prod` has never run (see S37).
   ...
   - `ikaro-tf-planner@<project>` — PR `terraform plan` only. Read-only: `roles/viewer` + `roles/storage.objectViewer` on the state bucket (+ `roles/secretmanager.viewer` for metadata). Cannot mutate anything — safe to expose to PR-triggered workflows. **Corrected/completed by M17-S24 (2026-07-21, found via the first real pipeline `terraform plan` run):** two grants were still missing and both fixes preserve the read-only guarantee — (1) `roles/viewer` deliberately excludes `*.getIamPolicy` permissions (a GCP design choice), so tf-planner couldn't read the IAM policy of any `google_*_iam_member`-bound resource (storage buckets, Pub/Sub topics, service accounts — `modules/iam` creates many); fixed initially with `roles/iam.securityReviewer` (purpose-built, `*.getIamPolicy`-only, no writes) at the project level, then **narrowed further the same day** to a custom project-level role, `tfPlannerIamPolicyReader` (created separately per project — no org-level role-creation permission available), containing exactly the 8 `getIamPolicy` permissions this codebase's `google_*_iam_member` resources actually need (grepped live: artifact registry, Cloud Run v2, project, Pub/Sub topic/subscription, Secret Manager, service account, storage bucket). Verified by removing `securityReviewer` first and re-testing a real plan with only the narrow role — identical result to before, confirming no gap. (2) Even a read-only `terraform plan` acquires a GCS state lock, which needs `storage.objects.create`/`delete` on the `.tflock` object — `storage.objectViewer` alone doesn't cover it; fixed with a `roles/storage.objectAdmin` binding condition-scoped to exactly `envs/<env>/default.tflock` (same pattern as tf-deployer's existing prefix-scoped binding, just narrowed to one object instead of one prefix).
...
2. **OAuth clients** (Web application), one per env:
   - staging: authorized redirect `https://<web-staging-run-url>/v1/auth/google/callback` (the web-origin `/v1` gateway forwards it to BFF). Register it after the web Cloud Run URL exists; redirect URIs are freely editable and changes take effect in minutes.
   - prod: `https://ikaro.online/v1/auth/google/callback`
...
### M17-S15 — Artifact Registry module (single registry, prod project) ✅ Done
...
**Description:**
`modules/registry`, instantiated **only in the prod env** (D8): Docker repo `ikaro-registry` in `southamerica-east1`. Cleanup policies (revised 2026-07-18, PR review — a KEEP policy alone never deletes anything, it only exempts matched versions from a DELETE policy, so retention requires **three** policies, not two): delete tagged versions >7 days, delete untagged versions >7 days, keep the 30 most recent versions per image regardless of tag state (`most_recent_versions` has no tag-state filter — a very recent untagged version can outlive the untagged rule until it ages out of this top-30 window; accepted, bounded edge case). **Revised 2026-07-23, M17-S26 cross-tool review:** the original 5-most-recent floor was too tight against staging's per-merge push frequency (`deploy-staging.yml` pushes a new tagged version on every merge touching `apps/**`/`packages/**`/the lockfile, independent of promote cadence) — as few as 5 staging merges after a prod promote could evict the exact image S26's "thorough" rollback path needs. Raised to 30 for real headroom; `docs/RUNBOOKS.md`'s rollback section documents falling back to `rollback-production.yml`'s traffic-shift path (unaffected — works off already-deployed Cloud Run revisions, not the registry) if this floor is ever exhausted anyway. IAM (clarified in discovery, 2026-07-18 — S08's table named this role but it was never actually executed; closed out here as a manual grant): `ikaro-tf-deployer@ikaro-prod` gets `roles/artifactregistry.admin` on the project, granted **manually** via `gcloud`, same pattern as S08's other 8 deployer roles — Terraform can't grant this to its own applier starting from zero permissions. Fully Terraform-managed within this module (no further bootstrap needed): `ikaro-app-deployer@ikaro-staging` gets `roles/artifactregistry.writer` on the repo (cross-project — staging pushes the image later validated for prod, D8); each project's **Cloud Run service agent** (`service-<PROJECT_NUMBER>@serverless-robot-prod.iam.gserviceaccount.com`) — not the app runtime SAs (backend/bff/web, created later in S17) — gets `roles/artifactregistry.reader`, since that's the actual identity Cloud Run uses to pull images; only staging's needs an explicit grant (prod's same-project pull is automatic). `staging_project_number` enters as a plain non-secret var in `envs/prod/terraform.tfvars` (same treatment as `project_id`/`db_tier` — not gitignored). Images: `ikaro-backend:<sha>`, `ikaro-bff:<sha>`, `ikaro-web:<sha>` (single image, no per-env suffix — corrected 2026-07-22, `docs/16-DASHBOARD_FRONTEND_ARCHITECTURE.md`: this row originally documented a per-env `-staging`/`-prod` web build that TD29 removed the need for; see D8/S25/S26), `ikaro-otel-collector:<version>` (S34).

**Acceptance criteria:**
- [ ] `docker push` from a WIF-authenticated GitHub Actions run succeeds (staging deployer)
  - ⚠️ Not verified as of 2026-07-18 — requires the M17-S24 infra/app pipeline (WIF-authenticated CI), which doesn't exist yet; the module's IAM grant itself is confirmed via a live terraform plan against ikaro-prod.
- [ ] Cloud Run in the **staging** project can pull from the prod-project registry (cross-project reader verified)
  - ⚠️ Not verified as of 2026-07-18 — requires M17-S18 (Cloud Run services module, not yet built) to have a real staging Cloud Run service to test the pull against; the reader grant itself is plan-verified.
- [ ] Cleanup policies visible in console; Checkov clean
  - ⚠️ Not verified as of 2026-07-18 — Checkov clean confirmed (module + repo-wide); console visibility awaits the first real apply, deferred to S24/S37 per M17 §0.
...
| `internal-api-key` | backend, BFF, relay VM | M115-S03 shared secret — `InternalApiGuard` is registered only in the backend and gates BFF→backend calls; the relay VM reads it through its own service account for UC-024; web is NOT a consumer |
| `platform-admin-key` | backend, relay VM | UC-024 provisioning; the relay VM reads it through its own service account |
...
**Description:**
`modules/iam`: three runtime SAs per env — `ikaro-backend@`, `ikaro-bff@`, `ikaro-web@` — plus `ikaro-pubsub-invoker@` (the identity Pub/Sub push mints OIDC tokens as; the Pub/Sub **service agent** needs `iam.serviceAccountTokenCreator` on it — granted in S19). **No custom Scheduler SA** (corrected 2026-07-08): `pubsub_target` Scheduler jobs cannot run as a user SA — the **Cloud Scheduler service agent** performs the publish and gets `pubsub.publisher` on the three cron topics (S21). Takes `secret_ids` (the map output by S16's `modules/secrets`) as an input variable to reference the actual secret resources when granting per-SA accessor roles.

**Scope split (clarified in discovery, 2026-07-18):** this module creates the 4 SAs and every binding resolvable against resources that already exist (S14 buckets, S16 secrets, project-level roles, self-grants). Bindings targeting resources created by *later* stories are documented in the table below for the full picture, but are actually created by the module that owns the target resource — `run.invoker` on a specific Cloud Run service is **S18's** (which already depends on S17 for the principal to exist); `pubsub.publisher` on a specific topic is **S19's** (same reasoning, transitively after S17 via S18).

| SA | Roles |
|---|---|
| backend | `cloudsql.client`, `pubsub.publisher` (its topics — granted in S19, once topics exist), per-secret accessor, `storage.objectAdmin` on the two buckets, `iam.serviceAccountTokenCreator` on itself (signed URLs), `cloudtrace.agent`, `monitoring.metricWriter` |
| bff | per-secret accessor, `cloudtrace.agent`, `monitoring.metricWriter`, `run.invoker` on backend service (granted in S18, once the service exists) |
| web | per-secret accessor (jwt/revalidate — not internal-api-key, web isn't a consumer per the S16 catalog), `run.invoker` on BFF (granted in S18, once the service exists; not strictly needed while BFF is public, but harmless) |
| pubsub-invoker | `run.invoker` on backend (granted in S18, once the service exists; push delivery) |
| Cloud Scheduler **service agent** (not a custom SA) | `pubsub.publisher` on the three cron topics (granted in S21) |
...
- **backend:** ingress `INGRESS_TRAFFIC_INTERNAL_ONLY`; direct VPC egress (network+subnet from S12, egress `PRIVATE_RANGES_ONLY`); `min=0,max=10` staging / `min=0,max=20` prod (push model makes min=0 safe — D2); CPU 1 / 512Mi; probes (**Cloud Run has startup + liveness probes only — no readiness probe**): startup probe `/health/ready` (gates new instances on real dependencies), liveness `/health/live`; a running instance that loses the DB is **not** pulled from rotation (Cloud Run cannot do that) — the S35 5xx/uptime alerting is the mitigation; env per backend schema incl. `PUBSUB_CONSUMER_MODE=push`, `PUBSUB_AUTO_CREATE=false`, `PUBSUB_PUSH_AUDIENCE=<own run URL>/pubsub/push`, `PUBSUB_PUSH_SERVICE_ACCOUNT=ikaro-pubsub-invoker@…`; secrets via `value_source.secret_key_ref`. `--no-allow-unauthenticated` + `run.invoker` for bff/pubsub-invoker SAs (IAM layer on top of internal ingress).
  - **Missing operator IAM grant (discovery finding, 2026-07-18; superseded 2026-07-24, TD32):** §2's security model at the time described `gcloud run services proxy ikaro-backend` as the access path for tenant provisioning (UC-024), but no story anywhere in this plan actually granted `roles/run.invoker` on the backend service to a human operator identity — S17's `run.invoker` grants are all service-account-to-service-account (bff/web/pubsub-invoker), and S13's `iam_admin_user` is scoped only to Cloud SQL IAM DB auth, an unrelated mechanism. This module added that grant — `admin@ikaro.online` (the operator identity from S07's bootstrap) — directly on the backend Cloud Run service, the same way S15 handles `ikaro-tf-deployer@`'s manual admin grant. **TD32 later proved the underlying premise wrong**: `gcloud run services proxy` never actually worked as an access path regardless of this grant (`ingress: internal` blocks it at the network layer, before IAM is even checked). This grant on `admin@ikaro.online` itself is still live (untouched by TD32 — removing an existing human grant was a separate, not-requested change) but no longer does anything useful, since the human never directly calls the backend anymore; the relay VM's own service account has its own separate `run.invoker` grant (`envs/*/main.tf`'s `cloudrun_backend` invoker list) and does the actual invoking today. See D4/§2's current rows for the real mechanism. Note this is IAM-only reachability: `PlatformAdminGuard`'s own `PLATFORM_ADMIN_KEY` check uses `X-Platform-Admin-Key`; it is a separate, second gate on top.
- **bff:** public (`INGRESS_TRAFFIC_ALL` staging; `INTERNAL_AND_CLOUD_LOAD_BALANCING` prod once S22 lands); allow-unauthenticated (app does its own auth — requires the S07 step-7 org-policy exception: allow-unauthenticated is an `allUsers` grant on `run.invoker`, blocked by default domain-restricted sharing on new orgs; same applies to web); VPC direct egress with **`ALL_TRAFFIC`** (fixes a would-break-staging gap found 2026-07-07: `*.run.app` resolves to public Google IPs, so `PRIVATE_RANGES_ONLY` would route the backend call outside the VPC — it would arrive as external traffic and internal ingress would reject it; under all-traffic egress the BFF's Google-API calls ride the subnet's Private Google Access — S12); `BACKEND_INTERNAL_URL=https://<backend run URL>` (internal ingress accepts VPC-originated calls to the run.app URL); CPU 1 / 512Mi; `min=0`. Also grant `run.invoker` on this service to the **web** SA (S17 — not strictly needed while BFF is public, but harmless; completes the run.invoker set alongside backend's bff/pubsub-invoker grants above, discovery finding 2026-07-18).
...
- **Probe paths per service (discovery finding, 2026-07-19):** backend has no global route prefix — probes target `/health/live`/`/health/ready`. **BFF sets `app.setGlobalPrefix('v1')`** (`apps/bff/src/main.ts`) — its probes must target `/v1/health/live` and `/v1/health/ready` (confirmed by the BFF Dockerfile's own `HEALTHCHECK` line, which already uses `/v1/health/live`). **web differs further:** web health lives under Next.js route handlers — probes must target `/api/health/live` and `/api/health/ready` (S04). A wrong probe path = a service that never turns READY.
...
**Env-var contract check (added 2026-07-08):** the Terraform env/secret maps must mirror the app schemas (`env.validation.ts` is the source of truth — S16), and nothing enforces that today; the failure mode is a startup error discovered at deploy time. Follows the same script+JSON-catalog+CI-diff-check pattern S19 will also use for Pub/Sub (not literal reuse — S19 depends on this story, not the other way around). **New workspace package `packages/infra-scripts`** (discovery finding, 2026-07-19 — `scripts/` today is shell-only and outside any pnpm workspace/test runner; this package establishes the convention S19 reuses): `packages/infra-scripts/src/env-contract.ts`, with its own `.spec.ts` per repo rule, exports the cloud-required keys from both `apps/backend/src/config/env.validation.ts` and `apps/bff/src/config/env.validation.ts` and cross-checks them against this module's env + secret map keys; wired as a new step in `pr-quality.yml`; CI fails on mismatch (allowlist for local-only vars like `GCS_KEY_FILE`). This converts "added a var in code, forgot Terraform" from a failed staging deploy into a failed PR.

**Connection-math invariant (owned here):** backend env sets `DB_POOL_SIZE=3` (S06 wires it into TypeORM); the module asserts — via a `variable { validation {} }` on `max_instance_count` (a `check` block was considered and rejected: confirmed empirically, 2026-07-19, that a failing `check` assertion only warns and still exits 0) — that `backend max_instances × DB_POOL_SIZE × 2 ≤ 0.8 × max_connections` of the chosen `db_tier` (tier→max_connections map encoded as a local). **The ×2 is a rollout-overlap factor, corrected 2026-07-19 against a real staging apply:** the service-level (combined) `scaling` block is required — Cloud Run's `template.scaling` is per-revision and auto-defaulted to an uncontrolled `10` when unset — but Google's own docs confirm the service-level cap is *still* enforced per-revision underneath, so two revisions (old + new) can each independently reach it during a deploy; the invariant must assume the documented worst case, not steady state. Launch numbers: `3 instances × 3 × 2 = 18 ≤ 20` (80% of 25 on f1-micro) — so launch caps backend at `max_instances=3` until the tier upgrade (D12 note; this halves the originally-planned `max_instances=6`, which was steady-state-only and does not survive the rollout-overlap correction). Raising max_instances without raising the tier (or enabling S46 MCP) must fail `terraform plan`. **Encoded permanently (2026-07-08, corrected 2026-07-19) as `tests/connection_math.tftest.hcl`** (Wave 2 preamble rule): `command = plan` + `mock_provider "google"` cases — valid combination (`max_instances=3`) plans clean; `max_instances=6` (the old value) now fails, proving the rollout-overlap factor is load-bearing, not just documentation; `max_instances=20` on f1-micro fails with the descriptive error; the tier→max_connections map covers every tier named in the S13 ladder. No credentials or resources involved; runs in the S24 PR job and protects the invariant when S46 later rewrites the math.
...
- [ ] Checkov clean
- [x] Relay VM service account granted `run.invoker` on the backend service; the bundled provisioning script reaches the internal backend from inside the VPC (verified through the TD32 relay flow; direct `gcloud run services proxy` is intentionally unsupported)
...
**Description:**
`modules/edge`, instantiated **prod only**: global external Application LB → two serverless NEGs (web, bff). Host routing: `ikaro.online` + `www.ikaro.online` → web NEG; `bff.ikaro.online` → bff NEG. Google-managed certificate (Certificate Manager) for the three hostnames — **DNS authorization** validation (works while Cloudflare proxies traffic; load-balancer authorization would not). The DNS-authorization CNAME records created at Cloudflare must be **DNS-only (unproxied / gray-cloud)** — a proxied validation record breaks issuance (2026-07-08). HTTP→HTTPS redirect. Cloudflare side (terraform `cloudflare` provider with the S09 token, in the prod env root): proxied A/AAAA (or CNAME) records for the three hostnames → LB IP; `www` → apex redirect via Cloudflare rule. Update prod service ingress to `INGRESS_TRAFFIC_INTERNAL_LOAD_BALANCER` (S18 variable — corrected 2026-07-19 against the real `google_cloud_run_v2_service` v2 API enum; `INTERNAL_AND_CLOUD_LOAD_BALANCING` was the old v1/Knative annotation string, not a valid v2 value). Set prod `ALLOWED_ORIGINS=https://ikaro.online`, `FRONTEND_URL=https://ikaro.online`, `GOOGLE_CALLBACK_URL=https://ikaro.online/v1/auth/google/callback`.

**Gateway/cookie note (TD35):** browser traffic must use the same-origin
`https://ikaro.online/v1/*` gateway, with `/v1/*` routed to the BFF and all
other paths routed to web. The BFF must issue a host-only `__Host-access_token`
cookie through that route; it must not rely on `bff.ikaro.online` sharing a
cookie with `ikaro.online`. Staging uses the same public `/v1` contract via the
web-side gateway until it has edge path routing.
...
Repository **variables** (non-sensitive): `GCP_PROJECT_STAGING=ikaro-staging`, `GCP_PROJECT_PROD=ikaro-prod`, `GCP_REGION=southamerica-east1`, `GAR_HOST=southamerica-east1-docker.pkg.dev`. Repository **secrets** (corrected 2026-07-22 — `docs/17-GITHUB_WORKFLOWS_GUIDELINES.md` §7's Secrets-vs-Variables convention, and this repo's live config, both confirm these are Secrets, not Variables as this line originally said; none is a bearer credential by itself since WIF trust is scoped to `repository == lmmoreira/ikaro` plus branch/environment claims, but Secrets auto-redact in logs): `WIF_PROVIDER_STAGING` / `WIF_PROVIDER_PROD` (full resource names from S08), the six SA emails from the S08 split (`TF_DEPLOYER_SA_*`, `APP_DEPLOYER_SA_*`, `TF_PLANNER_SA_*`). Also `CLOUDFLARE_API_TOKEN` (added when the edge module lands — S22 needs it for DNS/cert automation; scope extended later by S40/S41).
...
### M17-S24 — Infrastructure pipeline (`infra-deploy.yml`) ✅ Done
...
- **On `pull_request` touching `infra/terraform/**`:** `terraform fmt -check`, `validate`, **`terraform test` per module** (mocked providers — needs no credentials, so it runs before any auth step; Wave 2 preamble rule), Checkov, then `terraform plan` for **both** envs — authenticated as the read-only **`tf-planner`** SAs (S08: PR refs cannot mint deployer credentials, by WIF binding). Post plan output to the **workflow job summary only**, with a short status comment on the PR linking to the run (revised 2026-07-08: the repo is public — full plans in PR comments would publish infra topology: resource names, CIDRs, SA emails). No apply ever on PRs. Public-repo note (2026-07-07): fork PRs are not granted `id-token: write`, so they cannot mint even planner credentials — plans run only on same-repo branches (fine for a solo repo). *(Note from M17-S19: `modules/pubsub`'s tests read the real, committed `infra/terraform/pubsub-catalog.json` via `jsondecode(file(...))` rather than a self-contained fixture — this works automatically under a normal checkout with no special CI wiring, just don't be surprised it's not fixture-only like every other module's tests.)*
- **On `push` to `main` touching `infra/terraform/**`:** job `apply-staging` (environment `staging-infrastructure`, authenticated as **`tf-deployer`** — mintable from `main` only): `terraform apply -auto-approve` for `envs/staging`. Then job `apply-prod` (environment `production-infrastructure`, `needs: apply-staging`): **wait for approval**, then apply. Neither job reuses a saved plan artifact — both replan fresh at apply time (revised, 2026-07-21 review: a saved plan file embeds every input variable's real value regardless of `sensitive`, which would have persisted `TF_VAR_cloudflare_api_token` in cleartext on this public repo; kept symmetric between staging/prod so a future sensitive variable in either env is already safe by default). The reviewer's material for the approval gate is `plan-prod`'s job summary, already posted earlier in the same push — GitHub does not start a protected-environment job's steps until after approval, so there is no way for `apply-prod`'s own log to show a plan *before* that approval happens.
...
- **`TF_VAR_iam_admin_user` (S13 discovery):** plan/apply/drift jobs must supply this from a **repository-level GitHub Variable** (`vars.IAM_ADMIN_USER`) — **not a GitHub Secret, and not environment-scoped** (revised 2026-07-21 review: `plan-*`/`drift-*` jobs deliberately declare no `environment:`, since PR plans and drift checks must run without an approval gate, and GitHub only exposes environment-scoped values to jobs that declare that exact environment — an environment-scoped attempt resolves empty everywhere except `apply-staging`/`apply-prod`). The email is an identity, not a credential (it grants nothing by itself; Google auth + the IAM role bindings in `modules/database` are what actually gate access — settled in S13 discovery). Using a Secret here would be the wrong tool: Actions masks secret values in every log line, so a legitimate `terraform plan` diff or error message that includes `user:admin@ikaro.online` would render as `***`, hurting debuggability for zero security gain — and this exact email is already public throughout `docs/BOOTSTRAP_LOG.md`, so a repo-level Variable adds no new exposure. The value is still kept out of git (public repo); the database module's own email-format validation makes a missing/empty value fail `terraform plan` immediately (not a silent deletion of the IAM DB user).
- **`TF_VAR_brevo_smtp_login` (S16 discovery):** plan/apply/drift jobs must supply this from a **repository-level GitHub Secret** (`secrets.BREVO_SMTP_LOGIN`) — same repo-level-not-environment-scoped reasoning as `iam_admin_user` above, but a Secret rather than a Variable (revised again, 2026-07-21 review): unlike `iam_admin_user`, this value had never appeared anywhere else in the repo, and a live run confirmed GitHub prints Variable values in plaintext in the job's `env:` log block (Secrets are masked, Variables are not) — non-secret per the S16 catalog is not the same question as "acceptable to have permanently public in Actions log history" on this public repo.
- **`TF_VAR_cloudflare_api_token` (prod only, found reading `envs/prod/variables.tf` in full during implementation):** plan/apply jobs for `envs/prod` must supply this from the existing `secrets.CLOUDFLARE_API_TOKEN` GitHub Secret (created in S23) — a genuine credential, correctly a Secret not a Variable. Staging has no Cloudflare resources and needs no equivalent. The variable is already declared `sensitive = true` in Terraform, so plan/apply output redacts it automatically.
- Failure of staging apply blocks the prod job.

**Acceptance criteria:**
- [x] PR touching only `infra/**` runs plans + comment, and does NOT trigger app deploy workflows (path filters verified both directions)
- [x] `actionlint` + `zizmor` run in `pr-quality.yml` over all workflows and pass (Wave 3 preamble)
- [x] Merge auto-applies staging; prod apply visibly waits for approval
- [ ] Two rapid merges do not run concurrent applies (concurrency proven)
  - ⚠️ Not verified as of 2026-07-21 — design relies on GitHub's own concurrency-group primitive (`staging-mutations`/`production-mutations`, `cancel-in-progress: false`), never tested with two actual rapid merges.
- [x] Checkov, `terraform fmt`/`validate`/`terraform test`, `actionlint`, and `zizmor` all block merge on failure. Checkov is already a required check pre-dating this story. The other three (`Infra PR Checks Passed`, `actionlint`, `zizmor`) **must not** be added to branch protection's required status checks until *after* this PR merges and their workflows have run successfully at least once on `main` — adding a required-check context before its producing workflow exists on `main` leaves it permanently "expected, never reported" for every other open PR, forcing an admin-override merge (live incident, story-discovery/implementation session, 2026-07-21: broke an unrelated PR this way; see `feedback_required_check_merge_ordering` memory). Correct order: (1) merge this PR, (2) confirm the 3 new jobs report on a real `main`-triggered run, (3) only then `gh api PATCH .../required_status_checks` to add them — a distinct post-merge step, not part of the PR itself. **Done 2026-07-21:** all 3 confirmed running successfully on a real `main` push, then added to `required_status_checks`.
- [ ] Weekly drift plan: green on clean state; red on injected drift (test once with an out-of-band label change, then revert)
  - ⚠️ Not verified as of 2026-07-21 — mechanism proven via real scenarios instead (`workflow_dispatch`: staging clean → green; prod genuinely unapplied, 251 pending → red, exit code 2), but the specific out-of-band-label-then-revert test on an already-applied resource was not performed.

**Dependencies:** M17-S11, M17-S23

---

### M17-S25 — Staging deploy pipeline (`deploy-staging.yml`) ✅ Done
...
1. **Build+scan+push (matrix, parallel):** backend, bff, web. Tags: `ikaro-backend:<sha>`, `ikaro-bff:<sha>`, `ikaro-web:<sha>` (no `-staging` suffix — **revised 2026-07-22, docs/16-DASHBOARD_FRONTEND_ARCHITECTURE.md**: web no longer needs any `NEXT_PUBLIC_*` Docker build `ARG`/`ENV` at all. TD29 replaced every client-bundle read of these vars with a runtime accessor fed by a root-layout Server Component that reads real `process.env` per request — build-time values are gone entirely, so web builds **once**, generically, exactly like backend/bff, and D8's original per-environment-rebuild exception no longer applies. **Still required, separately, in Terraform** — not a build-time concern: `modules/cloudrun-service`'s web composition (`envs/staging/main.tf`, `envs/prod/main.tf`) currently only sets `NODE_ENV`/`APP_ENV` in `env_vars`; add `NEXT_PUBLIC_BFF_URL`, `NEXT_PUBLIC_SITE_URL`, `NEXT_PUBLIC_HOTSITE_IMAGE_BASE_URL` there per environment — the root layout's `PublicEnvScript` and `next.config.ts` (evaluated at Next.js process *startup*, not build) both need these as real Cloud Run runtime env vars). Trivy scan each image, fail on HIGH/CRITICAL, `ignore-unfixed: true` — deliberately matching the PR gate's `pr-tests.yml` config, not stricter (confirmed in story-discovery, 2026-07-22: a stricter `ignore-unfixed: false` was considered and rejected — an unfixed CVE has no available remediation, so blocking on it would leave this pipeline red indefinitely and, via S26's staging-must-be-current precondition, transitively block prod promotion too, including during an incident where the fix being shipped has nothing to do with the CVE. Unfixed-CVE visibility stays the weekly SARIF report's job, not a deploy gate's). `docs/09`'s `pr-security.yml` name is stale, see its own supersession banner. Push to the shared GAR (app-deployer WIF). Use registry layer caching (`--cache-from`) to keep the pipeline under ~15 min.
...
- [ ] Merge to `main` (app change) auto-deploys staging end-to-end in <15 min
  - ⚠️ Not fully verified as of 2026-07-23 — build→scan→push, the cross-project registry push, and the `check-current` staleness guard are all proven live; `migrate→deploy→smoke` is blocked by staging's expected `bootstrap_mode` (no real Cloud SQL instance yet — M17-S27's job to activate), not a pipeline defect. Full end-to-end timing pending S27.
- [ ] Failed Trivy scan → nothing pushed; failed migration → nothing deployed (prove both with deliberate temp failures on a branch, screenshots/log links in PR)
  - ⚠️ "Failed migration → nothing deployed" proven live for real on 2026-07-23 (unplanned, not a deliberate branch test — see `docs/BOOTSTRAP_LOG.md`'s same-date entry): `Deploy backend`/`bff`/`web` and `Smoke test` all correctly skipped after the real migrate step failed. "Failed Trivy scan → nothing pushed" and a deliberate scratch-branch proof with screenshots are still outstanding.
...
- [ ] `ikaro-web:<sha>` is a single generic image with no per-environment variant (TD29 follow-through) — confirm no build step conditionally injects `NEXT_PUBLIC_*` anywhere in this workflow
- [ ] `envs/staging/main.tf`'s `cloudrun_web` module sets `NEXT_PUBLIC_BFF_URL`, `NEXT_PUBLIC_SITE_URL`, `NEXT_PUBLIC_HOTSITE_IMAGE_BASE_URL` as real Cloud Run runtime env vars (not build args) — the Stage-1 TD29 follow-through above, made an explicit AC after story-discovery (2026-07-22) found it stated only in prose; also corrects that module's stale comment attributing this solely to S26
- [ ] `packages/infra-scripts/src/env-contract.ts` extended to also cover `apps/web`'s `PUBLIC_ENV_KEYS` (new array-literal extractor, `public-env-keys.ts`, mirroring `schema-keys.ts`'s AST approach) against `envs/staging/main.tf`'s `cloudrun_web` module — scoped to **staging only** for now (S26 widens to prod once it adds prod's runtime values there). The `Terraform env-var contract` CI job (added in S18, never previously required) is added to `required_status_checks` **only as a post-merge step** — merge this PR first, confirm the job reports a real clean run on `main` covering all three apps, *then* register it as required. Same ordering discipline as S24's `actionlint`/`zizmor`/`Infra PR Checks Passed` (never register a required check before its producing workflow has run clean on `main`, or every other open PR gets stuck on a check that's "expected, never reported").
- [ ] **Deferred verification (TD29's own Tests section explicitly pushes this here, not into TD29's PR):** once this pipeline produces the first real staging deploy, confirm client-side BFF calls resolve to the correct origin in an actual browser against the real production build (not `next dev`) — this is the first point a genuine `next build`/`next start` artifact exists to test against. Exercised for real in S27's validation checklist and/or S28's Playwright suite (add a check there too — cross-referenced, not duplicated).
...
**Description:**
`workflow_dispatch` with input `image_sha`. Promotes the staging-validated artifacts: backend, bff, **and web** all by **exact same image** — **revised 2026-07-22, docs/16-DASHBOARD_FRONTEND_ARCHITECTURE.md**: web's per-environment rebuild (D8's original exception) is gone. TD29 removed every build-time `NEXT_PUBLIC_*` dependency from the client bundle, so the same `ikaro-web:<sha>` validated in staging promotes unmodified, same as backend/bff. Prod-specific values (`NEXT_PUBLIC_BFF_URL=/v1`, `BFF_UPSTREAM_URL=https://bff.ikaro.online/v1`, etc.) are supplied as **Cloud Run runtime env vars** on the prod web service (`modules/cloudrun-service`'s prod composition — see S25's note on the same Terraform gap), not baked into a separate image.

**`image_sha` is optional (added 2026-07-23, discussion during implementation):** leave it blank to promote whatever SHA staging is currently running — resolved from `ikaro-backend`'s live image tag before any other check runs. Deliberately still `workflow_dispatch`-only rather than auto-chained off `deploy-staging.yml`'s success the way `infra-deploy.yml` chains `apply-staging` into `apply-prod`: staging deploys fire on every merge to `main` (high frequency), so auto-creating a pending `production` approval on each one would flood the environment with requests nobody intends to act on, and GitHub environment protection has no auto-deny-on-timeout — those would just accumulate until manually dismissed. The blank-default removes the "go look up the SHA" step without changing when or how often the workflow runs.

**Stages:**
1. **Validate input:** resolve `<sha>` (from the input, or from staging's current image if blank); images for `<sha>` exist in GAR AND `<sha>` is an ancestor of `main` AND staging currently runs it on **all three services** (query `gcloud run services describe` in staging) — else fail with a clear message.
...
3. **Migrate:** prod migrate job with `…backend:<sha>` — `--wait`, hard stop on failure.
4. **Deploy sequential** backend → bff → web; each waits READY. No separate web build step — all three deploy the exact images already validated in staging.
5. **Smoke:** `https://bff.ikaro.online/v1/health/ready`, `https://ikaro.online/api/health/ready`, plus one real public read (`GET /v1/hotsite/<seeded-slug>` — adjust to the live BFF route), **plus** a real-browser check that the served HTML/JS references `bff.ikaro.online` and contains no `run.app`/staging origin — this remains a genuine risk even with a single image, since it's now verifying the *Cloud Run runtime env var* was set correctly for prod, not a build arg. Same failure mode, different root cause to catch.
6. **Rollback procedure (workflow header + `docs/RUNBOOKS.md` section):** two paths, added during implementation (2026-07-23). Fastest: companion workflow `rollback-production.yml` (`workflow_dispatch`, per-service or `all-three`) — instant traffic-shift to the previous Cloud Run revision, same WIF auth (no local `gcloud` needed), same `production` approval gate; scoped to traffic only, no migration involvement (`migration:revert` stays a manual last resort — see below). Thorough: re-run `deploy-production.yml` with the previous SHA, which brings all three services back to the same known-good SHA together (the traffic-shift path can't guarantee that across services when used with `all-three`, since each service resolves its own previous revision independently). Migrations follow expand/contract (repo rule) so rolling back code without reverting schema is safe by construction; `migration:revert` is the documented last resort.
...
- [ ] `envs/prod/main.tf`'s `cloudrun_web` module sets `NEXT_PUBLIC_BFF_URL=/v1`, `BFF_UPSTREAM_URL=https://bff.ikaro.online/v1`, `NEXT_PUBLIC_SITE_URL`, and `NEXT_PUBLIC_HOTSITE_IMAGE_BASE_URL` as real Cloud Run runtime env vars.
...
- [ ] `image_sha` is optional on `deploy-production.yml` — a blank input resolves to whatever SHA staging is currently running (added during implementation, 2026-07-23, to remove the manual "go find the SHA" step; deliberately still `workflow_dispatch`-only, not auto-chained off every staging deploy the way `infra-deploy.yml` chains `apply-staging` into `apply-prod` — see the workflow's own header comment for why)
- [ ] `rollback-production.yml` exists: per-service (or `all-three`) instant traffic-shift to the previous Cloud Run revision, gated by the same `production` environment, no migration involvement
...
**Note from M17-S24 (2026-07-21):** the infra pipeline's mechanics are now live-verified for staging — `apply-staging` ran for real (as `tf-deployer`, not a human operator, for the first time ever) and completed cleanly. But that run was a no-op (`enable_database` was still `false`, 0 resources changed) — it never actually exercised *creating* anything. Step 1 below (the real `apply-staging` run that creates the database) will be the first time this pipeline creates a brand-new Cloud SQL instance and its project-level IAM bindings for real. `tf-deployer`'s permissions were extended for exactly this (`resourcemanager.projectIamAdmin`, verified against prod's plan showing the same resources as "to add" — see S08's corrected bullet and `docs/BOOTSTRAP_LOG.md`), but a plan showing the diff isn't the same as an apply actually creating it. Treat a possible new permission gap here as a real possibility, not a surprise, same as this story's own experience.

**Discovery correction (2026-07-23):** staging's `enable_database` deferral flag has been removed entirely, not flipped — once turned on for real it could never meaningfully go back to `false` (that would mean tearing down the live database the app depends on), so the count-gate no longer served a purpose. `modules/database` is now instantiated unconditionally in `envs/staging/main.tf`; Step 1 below is a plain merge-to-`main` of that Terraform change, which triggers `apply-staging` and creates the instance. Prod's identical `enable_database`/`enable_edge` flags are untouched — still gated until S37 does the same cleanup there.

**Further discovery corrections (2026-07-23, same session):** `bootstrap_mode` got the same treatment once flipped for good — staging's own wiring to it was removed entirely (hardcoded `false` at each of the 4 module call sites in `envs/staging/main.tf`), not left as a variable; prod's copy is untouched, same reasoning as above. Step 3's "empty commit or re-run" no longer applies as written: an empty commit never matches `deploy-staging.yml`'s `paths:` filter (GitHub matches on changed files, and a truly empty commit changes none), and `gh run rerun` on a past run permanently fails the workflow's own `check-current` staleness guard once `main` moves past that run's original commit — `workflow_dispatch` was added to the workflow specifically to solve this (see `docs/CI_TRAPS.md`'s "CI workflow configuration traps" for both gotchas in full). Two more real defects surfaced only by this story's first genuine attempt at Step 3: Cloud SQL's `ssl_mode=ENCRYPTED_ONLY` was initially handled with an unverified TLS configuration, then resolved by the shipped `@google-cloud/cloud-sql-connector` adapter (`apps/backend/src/shared/infrastructure/database/cloud-sql-connector.adapter.ts`); and `NEXT_PUBLIC_BFF_URL` was missing the `/v1` prefix BFF's global route prefix requires, in both env roots — breaking every web→BFF call including web's own `/api/health/ready` (see `docs/24-BFF_ARCHITECTURE.md` § Web → BFF Transport Layer).

**Description:**
Turn staging from placeholder to a working environment. This is a runbook + checklist story; its PR carries only doc updates.

**Steps:**
1. **Merge staging's `enable_database` removal + apply** (instance creation deferred from S13 — the ~$9/mo charge starts here; see the discovery correction above for why this is a merge, not a tfvars flip), then verify the deferred S13 AC: instance private-IP only + SSL enforced ✅, `cloud-sql-proxy --auto-iam-authn` login from the dev machine — direct dev-machine access remains blocked (⚠️ see `infra/terraform/modules/relay-vm/README.md`, no VPN/bastion exists to reach the private IP), **superseded and resolved via the IAP-reachable relay VM** (TD32, ✅ Done, validated via PRs #259–#261, 2026-07-25 — same mechanism used for tenant provisioning below), backup configuration confirmed live via `gcloud sql instances describe` (2026-07-27: `backupConfiguration.enabled=true`, `retainedBackups=7`, `startTime=02:00`, matching Terraform's declared spec; `pointInTimeRecoveryEnabled=false` is correct — PITR is prod-only by design). Populate staging secret values (`gcloud secrets versions add`): jwt-secret (`openssl rand -hex 64`), internal-api-key, platform-admin-key, hotsite-revalidate-secret, OAuth pair (S10), Brevo SMTP key. **Create both DB users + passwords out-of-band per the S13/S20 snippets** (`gcloud sql users create` for `ikaro` (app, DML-only) and `ikaro_migrator` (migrate job, DDL) + their `db-password`/`db-migrator-password` secret versions — never via Terraform, §2; grant the migrator role's `CREATE ON DATABASE`/`CREATE ON SCHEMA public` per `docker/init-db.sh`'s pattern). Record rotation dates in `SECRETS.md`.
2. Remove staging's `bootstrap_mode` wiring entirely (S18; see the discovery note above — hardcoded `false` at each call site, not a flipped variable) via the infra pipeline; set staging env vars: `APP_ENV=staging` (which is what makes `ENABLE_DEV_AUTH=true` legal — S06), `ENABLE_DEV_AUTH=true`, `EMAIL_ADAPTER=brevo`, `LOG_LEVEL=DEBUG`.
...
3. Trigger `deploy-staging.yml` via `workflow_dispatch` (added this story — see the discovery note above; neither an empty commit nor `gh run rerun` reliably works) → first real images deploy.
4. **Validation checklist (all deferred items from Wave 2 land here):**
   - [x] 3 services READY with real images; probes green (2026-07-27, confirmed via browser navigation)
   - [x] Backend unreachable from internet; reachable from BFF (S18 check) (2026-07-27, confirmed via relay)
   - [x] Migrations applied (S20 check); tables present via `cloud-sql-proxy` + psql (2026-07-27, confirmed)
   ...
   - [x] Google login works on the staging web URL (test user from S10) (2026-07-27, confirmed)
   - [x] Guest booking flow end-to-end on the staging hotsite (creates booking, email lands via Brevo) (2026-07-27, confirmed end-to-end incl. Brevo delivery)
   - [x] Pub/Sub push observed: `BookingRequested` handled, notification sent, no DLQ messages (S19 check) (2026-07-27, confirmed via outbox/inbox)
   - [x] `gcloud scheduler jobs run` → cron push hits backend (S21 check) (2026-07-27, confirmed via outbox/inbox)
   - [x] Signed-URL photo upload works from the browser (S14 CORS check) (2026-07-27, confirmed — upload/delete of photos in buckets)
   - [ ] Tenant provisioning via the IAP-reachable relay VM and bundled `/usr/local/bin/provision-tenant.sh`; a staging tenant was provisioned through the relay on 2026-07-27, but the new automatic self-fetch of both Secret Manager values still needs live verification.
   - [x] **Client-IP position in staging's X-Forwarded-For header verified (M17-S30 PR #167 review, 2026-07-19):** the BFF's `getClientIp()` (`apps/bff/src/shared/http/client-ip.ts`) assumes the rightmost XFF hop is the real client IP for staging's no-LB, direct-`*.run.app` path — unverified at the time S30 shipped (nothing was deployed to test against; GCP's docs describe GFE-fronted paths appending more than one hop in some configurations, so "rightmost" was not a confirmed fact). Make a real request from a known IP, log/inspect the actual `X-Forwarded-For` header Cloud Run delivers, and fix the code + its comment with a citation to the verified behavior if the assumption is wrong.
     **Instrumented (2026-07-27):** `AppThrottlerGuard.getTracker()` now debug-logs the raw `X-Forwarded-For` header + resolved IP, permanently (cheap to keep for future re-verification; the guard's own env check was later dropped too — `AppLogger`'s `LOG_LEVEL` already filters it in production).
     **Verified and found broken (2026-07-27):** a real staging request from a known IP (`179.118.166.67`) showed `x-forwarded-for="34.34.231.138" resolved="34.34.231.138"` — a Google Cloud–owned address, not the requester's IP. Root cause isn't hop-selection: the same-origin `/v1` gateway (`apps/web/app/v1/[...path]/route.ts`) makes BFF see only `ikaro-web`'s own egress IP for all real traffic, in both staging and prod. This is a bigger architectural gap than this checklist item anticipated — filed as `plan/M17-CLOUD-DEPLOY.md` with the full proposed fix (BFF ingress lockdown + IAM auth from web, mirroring the existing backend↔BFF pattern).
     **Closed (2026-08-01, TD38 Story A):** the original rightmost-XFF assumption was never fixed as such — it was replaced entirely. BFF now trusts a `X-Real-Client-Ip` header that `ikaro-web` resolves on the genuinely trustworthy browser→web hop and forwards, once BFF's ingress only accepts calls from `ikaro-web`. Re-verified live in staging post-fix: a real browser request logged `x-real-client-ip="177.76.129.80" resolved="177.76.129.80"`, a real ISP address. The checklist's underlying goal (staging resolves the real client IP correctly) is now met, via TD38's mechanism instead of the originally-assumed one.
...
**Dependencies:** Waves 2–3 complete (except S22/S26 which are prod-only); TD30 Story 1 (prod's registry/IAM/secrets containers applied — Step 3's first real deploy needs the shared Artifact Registry, D8, which otherwise only exists via S37's atomic apply — see plan/M17-CLOUD-DEPLOY.md)

---

### M17-S28 — Playwright E2E against staging ❌ Cut

**Cut 2026-08-08, via story-discovery.** Investigated whether to port the golden-path Playwright suite to run against staging (absorbing TD38 Story C's gateway-routing redesign). Found, with evidence, that the two failure classes this would address are already covered elsewhere:
- **Product/UI logic** — already covered by `pr-tests.yml`'s `e2e` job (docker-compose, every PR, fresh seeded data).
- **Deploy/infra-wiring bugs** (IAM auth chain, `/v1` gateway routing, secret propagation, real DB connectivity) — already covered by `deploy-staging.yml`'s `smoke` job, which routes through the real public gateway and chains through BFF → backend → a real TypeORM DB ping, exercising the IAM token + `X-Web-Internal-Key` handshake end to end. Confirmed by reading the job and both `/health/ready` controllers, not assumed.
- Per TD38 Story C's own retrospective, every real staging bug found to date (`X-Web-Internal-Key` newline mismatch, IAM apply-ordering, a blocked smoke-test URL) was in this second category — none needed a browser.

**Known, accepted gaps (not closed by this cut, tracked here so they aren't silently lost):**
1. Real Google OAuth login handshake has no automated coverage (pre-existing since M115-S02's Dev Login decision — a full Playwright port wouldn't have closed this either, since its auth journeys would still use Dev Login).
2. Environment-only behavioral differences a health-check-shaped smoke test wouldn't catch (real Cloud SQL config/version quirks, Cloud Run cold-start latency, Cloudflare edge-caching behavior, prod-build-only differences) are unconfirmed — no real bug in this category has been observed. Revisit only if one actually surfaces; building the full suite now to guard against a speculative gap repeats the "unconfirmed hypothesis treated as justification" mistake called out elsewhere in this doc (see the M17-S34 Cloud Trace quota bullet in CLAUDE.md).

**TD38 Story C** is closed as absorbed-then-cut by this decision — no further action needed there.

**Superseded scope (for traceability):** the original story description, dependencies, and acceptance criteria are preserved in git history (see this commit) rather than kept inline.
...
**Description:**
The collector is where GCP appears — and the only place. Build a pinned image `infra/docker/otel-collector/` (FROM `otel/opentelemetry-collector-contrib@sha256:<pinned digest>` — digest-only, no tag, matching the app Dockerfiles' own convention; config baked in). **Validate the config at build time (added 2026-07-08):** run the collector's built-in `validate` subcommand against `config.yaml` in the image-build step — a YAML/pipeline typo becomes a build failure instead of a sidecar crash-loop discovered in staging. Config (`config.yaml`, versioned):
- receivers: `otlp` (http 4318, grpc 4317, localhost only)
- processors: `memory_limiter`, `batch`, `resource_detection` (gcp) — not `resourcedetection`, which the real collector binary logs as a deprecated alias (confirmed empirically, 2026-08-04)
- exporters: `googlecloud` (traces). Metrics pipeline stub commented for future `googlemanagedprometheus`.
- A commented **alternate exporter block** (`otlp` → self-hosted Tempo/other vendor) with the note: *swapping vendors = edit this file + rebuild via the dedicated workflow below — the next backend/BFF deploy picks it up automatically. No app change, no Terraform change.*

**Build/deploy design (settled 2026-08-04, after discovery review):** the collector rebuilds far less often than backend/BFF (only on real config/Dockerfile changes) but must ride along on *every* backend/BFF deploy as a sidecar — so build and deploy are deliberately two separate pipelines, joined through GAR itself rather than any hand-maintained "current version" pointer:
- **Build** — a dedicated workflow (`.github/workflows/build-otel-collector.yml`), triggered on `infra/docker/otel-collector/**` changes. Split into a `build-and-scan` job (runs on **both** `pull_request` and `push` — build, `validate`, Trivy scan, no push) and a `push` job (runs only on `push` to `main`, cache-hit rebuild of the already-scanned image, pushes `:latest` + the commit SHA to the same single GAR repo, D8). The `pull_request` trigger was added after the first review pass (cross-tool finding, PR #318, 2026-08-04): a push-only workflow meant a broken Dockerfile/config could pass every PR check and only fail post-merge — the exact gap `pr-tests.yml`'s own Trivy job already closes for backend/bff/web.
- **Deploy** — stays where backend/BFF already deploy (`deploy-staging.yml`/`deploy-production.yml`'s existing `gcloud run deploy ikaro-backend`/`ikaro-bff` steps, previously single-container). These become multi-container deploys (`--container=app --image=... --container=otel-collector --image=...`), resolving `ikaro-otel-collector:latest` to a concrete digest via `gcloud artifacts docker images describe ... --format='value(image_summary.digest)'` **immediately before** each deploy. The live revision is always pinned to that resolved digest — `:latest` is only a GAR-side lookup convenience, never what's actually deployed. This mirrors how backend/BFF/web already work today: `var.image` has no Terraform default and is permanently excluded from `lifecycle.ignore_changes`-tracked drift (`modules/cloudrun-service/main.tf:164-169`) because the pipeline, not Terraform, owns "what's currently live" — the collector's `:latest`-resolved-to-digest lookup is the same pattern adapted for an independently-cadenced image. **Rejected alternatives:** (a) a GitHub Actions repo variable written by the build workflow — needs a new token/PAT scope beyond what `GITHUB_TOKEN` grants for repo variables, and the value would live outside git, invisible in diffs; (b) a manually-bumped tag line in the deploy workflow — requires a human to remember an unrelated second edit every time the collector changes, with no CI signal if they forget, silently redeploying a stale collector indefinitely. Both rejected in favor of resolving from GAR fresh on every deploy — nothing to keep in sync because there's nothing to hand-maintain.
- Terraform: activate the sidecar in the S18 module (`modules/cloudrun-service`) for backend + BFF only — **web is unaffected, no sidecar, no OTel wiring** (D9/S33 already scope OTel to backend+BFF). Extend the `sidecar_containers` variable (`variables.tf:173-`) and its `dynamic "containers"` block (`main.tf`) to accept the sidecar's `name`/`image`/`cpu`/`memory`. `envs/staging/main.tf` and `envs/prod/main.tf` pass `sidecar_containers = [{ name = "otel-collector", image = <bootstrap/placeholder>, cpu = "0.1", memory = "128Mi" }]` on the `cloudrun_backend`/`cloudrun_bff` module calls. Add the sidecar's `image` field to `main.tf`'s existing `lifecycle.ignore_changes` block alongside `template[0].containers[0].image` — like the app container, the collector's live image is pipeline-owned post-deploy, not Terraform-tracked, so Terraform must not revert it on a later `apply`.
- **No container-dependency/startup_probe wiring between the app container and the sidecar** (corrected 2026-08-04, cross-tool PR review finding on PR #318, confirmed via a `docker-compose depends_on: condition: service_healthy` reproduction — the collector container exiting caused Compose to report `dependency otel-collector failed to start` and the dependent container never even attempted to start). An earlier version of this design gave the app container `depends_on = [sidecar names]` gated by a `startup_probe` on the collector's `health_check` extension, intended to avoid dropping early spans during cold start. That directly contradicts this story's own "app boots even if collector crashes" acceptance criterion: Cloud Run will not start a container until every container it `depends_on` passes its own startup probe, so a crash-looping or misconfigured collector would have permanently blocked the app from booting — a strictly worse failure mode than a few unbuffered spans during a cold start, which S33's OTLP exporter already tolerates gracefully (no crash, warn once) without any Cloud Run-level ordering guarantee. Sidecar and app containers start independently.
- Add the base image to Dependabot (`docker` ecosystem, `directory: /infra/docker/otel-collector`) so the pin receives update PRs (2026-07-08).
...
- [x] Backend + BFF `gcloud run deploy` steps resolve `ikaro-otel-collector:latest` to a concrete digest immediately before deploying, so the live sidecar is always digest-pinned, never on a floating tag
- [x] `sidecar_containers`' `image` field is added to the S18 module's `lifecycle.ignore_changes` — a `terraform apply` never reverts the live collector digest
- [x] Collector config/exporter change → rebuild via the dedicated workflow → next backend/BFF deploy picks it up automatically, no manual Terraform edit and no manual `terraform apply` needed; documented in `infra/docker/otel-collector/README.md`
- [x] Config `validate` runs in the image build; a deliberately broken config fails the build (prove once) — proven via a real `docker build` with a config referencing an undefined processor, exit code 1
- [x] `.github/dependabot.yml` gains a `docker` ecosystem entry for `/infra/docker/otel-collector`
- [x] web has no sidecar container and no OTel wiring — confirmed its deploy step stays single-container in both `deploy-staging.yml` and `deploy-production.yml`

**Post-launch incident history (2026-08-05 to 2026-08-06):** the `processors:` list above (`memory_limiter`, `batch`, `resource_detection`) was the original design, but `batch` (plus the exporter's `sending_queue`) was removed the day after this story shipped over a real CPU-throttling timer-starvation finding, then reintroduced the next day once that finding turned out to be measured under unrepresentative (sampler-bug-suppressed) traffic, and a different, still-not-fully-understood failure mode appeared under real traffic instead. Today's config matches this story's original description again, but arriving there took a real investigation with a genuine methodology mistake along the way (a root-cause theory shipped as fact before being checked against actual evidence, then corrected via cross-tool review). None of this affects this story's own acceptance criteria — full incident: `docs/ENGINEERING_RULES_INFRA.md` § Cloud Run CPU throttling, `infra/docker/otel-collector/README.md`.
...
### M17-S55 — OTel metrics + Managed Prometheus export pipeline ✅ Done

**Agent:** `devops` + `backend-ts`/`bff-ts` (instrumentation)
**Complexity:** M
**Docs to load:** `docs/10-OBSERVABILITY_STRATEGY.md`, `docs/ENGINEERING_RULES_INFRA.md` § Cloud Run CPU throttling (the OTel-pipeline incident history), `infra/docker/otel-collector/README.md`
...
**Description:**
1. Re-enable OTel SDK metrics: change `packages/observability/src/otel-tracing.ts`'s `bootstrapTracing()` from `metricReaders: []` to a real `PeriodicExportingMetricReader`. **New dependencies required** (confirmed via story-discovery, 2026-08-12: neither exists in `packages/observability/package.json` today) — add `@opentelemetry/sdk-metrics` (for `PeriodicExportingMetricReader`) and `@opentelemetry/exporter-metrics-otlp-http` (matching the existing `OTLPTraceExporter`'s HTTP protocol, for consistency with the traces side) as direct dependencies of `@ikaro/observability` — never added directly to either app's own `package.json`, same rule the existing trace packages already follow (`docs/10-OBSERVABILITY_STRATEGY.md`'s no-restricted-imports note).
...
3. Add real instrumentation (counters/histograms) at the call sites that matter. **Decided during implementation, 2026-08-12:** no custom app-code instrumentation was written. `@opentelemetry/instrumentation-http` (already active for tracing, via `getNodeAutoInstrumentations()`) emits an `http.server.duration` histogram per route automatically, once a real `MeterProvider` exists — re-enabling `metricReaders` alone is sufficient for request count + duration on `POST /v1/bookings`, filterable by its `http.route` label. Structurally simpler than a hand-written `Counter`/`Histogram` (no new instrumentation code, generic per-route coverage instead of one hand-picked endpoint) and consistent with this story's own "per-route throughput" framing above. Confirmed against the real pinned `instrumentation-http@0.220.0` source: the emitted metric name is the *old* semconv `http.server.duration` (unit `ms`), not the newer `http.server.request.duration` — this repo doesn't set `OTEL_SEMCONV_STABILITY_OPT_IN`, and changing that is out of scope here (it would also rename trace-span attributes the existing `requestHook` code in `otel-tracing.ts` depends on).
...
5. **Apply the same careful, isolated empirical verification this repo's OTel tracing pipeline has already needed three separate times** (a `ParentBasedSampler` default silently dropping most spans, `OTLPTraceExporter`'s concurrency limit silently rejecting exports, Cloud Run's CPU throttling starving the batch-flush timer — all real incidents, all caught only via live, empirical verification, not code review alone; full history: `docs/ENGINEERING_RULES_INFRA.md` § Cloud Run CPU throttling). Enabling the metrics half deserves the same rigor, not a quick toggle bundled into a bigger story. **The three specific checks this means for the metrics path (added via story-discovery, 2026-08-12 — see Acceptance Criteria below for the concrete form each takes.)**
6. **Stale-reference sweep, same commit (added via story-discovery, 2026-08-12):** this story flips a state that several files currently document as the deliberate, correct steady-state — update all of the following, not just the code:
   - `docs/ENGINEERING_RULES_INFRA.md` § Cloud Run CPU throttling — the `metricReaders: []` fix narrative needs a note that metrics are now intentionally re-enabled, with the new timer-starvation risk documented per AC below.
   - `docs/ANTI_PATTERNS.md` — the row documenting "`new NodeSDK({...})` constructed without disabling metrics" as an anti-pattern needs updating: metrics are no longer universally wrong to enable, only wrong to enable *without* the safeguards this story adds.
   - `docs/10-OBSERVABILITY_STRATEGY.md` — the file-level superseded-banner and `## Overview`/`## Prometheus Metrics` sections currently say "no native metrics layer exists yet (deferred, M17-S55)" — flip once this story ships.
   - `infra/docker/otel-collector/config.yaml` — remove/replace the "Metrics pipeline stub — deferred to a future story" comment now that the pipeline is real.
   - `infra/docker/otel-collector/README.md` — its "Follow-up, same day: a third, unrelated bug" section explicitly says "the metrics pipeline stays deferred until M17-S55 needs it" — update once it's built.
   - This same file (`plan/M17-CLOUD-DEPLOY.md`) — S34's own description text (around line 1307) makes the same "M17-S55 owns re-enabling it" forward-reference; update once done.
...
**Acceptance criteria:**
- [x] `metricReaders` re-enabled in code (`packages/observability/src/otel-tracing.ts`, unit-tested in `otel-tracing.spec.ts`, `tsc --noEmit` clean on both `apps/backend` and `apps/bff`). **Live-verified in staging, 2026-08-12** (post-merge deploy, PR #362 → `main` → `deploy-staging.yml`): no traces regression — the trace pipeline continued operating normally alongside the newly re-enabled metrics reader.
- [x] Collector's metrics pipeline coded (`infra/docker/otel-collector/config.yaml`'s `metrics:` pipeline + `googlemanagedprometheus` exporter) and validated against the real pinned collector binary (`docker run ... validate`, exit 0; full `Dockerfile` build also succeeds). **Correction (cross-tool PR review, 2026-08-12):** the pinned digest's own `--version` output reports `otelcol-contrib version 0.158.0`, not the `0.157.0` this file and the Dockerfile's own comment assumed — checked directly (`docker run <pinned-digest> --version`), not inferred from the tag name. The Dockerfile/README's pre-existing `0.157.0` references predate this story (M17-S34) and are left as-is here rather than retroactively edited without knowing the original circumstances of that pin; this story's own new claim is corrected to match the verified reality. **Live-verified in staging, 2026-08-12:** deployed revision's collector image digest (`sha256:d390c065...`) confirmed byte-for-byte identical to `build-otel-collector.yml`'s push for this exact commit; a real GMP-sourced panel query against `prometheus.googleapis.com/http_server_duration_milliseconds/histogram` returned real data (see below) — the widget renders correctly.
- [x] Native histogram (`http.server.duration`, `POST /v1/bookings` via `http.route` filter) confirmed available through existing auto-instrumentation, no new app code (see Description step 3) — Terraform widget added (`booking_creation_latency_widget` in `infra/terraform/modules/monitoring/main.tf`, `terraform validate`/`fmt`/`test` all clean). **Fixed via cross-tool PR review, 2026-08-12:** the first version used `timeSeriesFilter` + `perSeriesAligner = "ALIGN_PERCENTILE_99"` directly on the raw histogram — confirmed invalid against Cloud Monitoring's own `Aligner` API reference (that aligner is valid only for `GAUGE`/`DELTA` distributions, never `CUMULATIVE`, and GMP-exported OTLP histograms are cumulative). Now uses `timeSeriesQuery.prometheusQuery` (native PromQL `histogram_quantile(rate(..._bucket[5m]))`) instead — sidesteps the aligner-type mismatch entirely. **Live-verified in staging, 2026-08-12 — exact match, no correction needed:** a real `POST /v1/bookings` request (201) produced a time series at exactly `metric.type="prometheus.googleapis.com/http_server_duration_milliseconds/histogram"`, `resource.type="prometheus_target"`, `metric.label.http_route="/v1/bookings"`, `metric.label.http_method="POST"` — the widget's own PromQL string, verified byte-for-byte via the Cloud Monitoring API (`timeSeries.list`) and cross-checked visually in Metrics Explorer.
- [x] **Timer-starvation check (direct analog of the CPU-throttling bug — same mechanism, a different processor):** `PeriodicExportingMetricReader` is timer-driven, exactly like the traces-side `BatchSpanProcessor` already proven vulnerable to `run.googleapis.com/cpu-throttling: "true"` starvation. **Live-verified in staging, 2026-08-12:** observed a genuinely idle 5-minute window (23:19:11–23:24:05 UTC, zero deliberate requests) via the Cloud Monitoring API — 6 consecutive successful export points, timestamped exactly ~60s apart (23:19:23, 23:20:23, 23:21:23, 23:22:23, 23:23:23, 23:24:23), zero gaps, zero export-error log lines on `ikaro-bff` for the same window. The periodic reader's background timer fired reliably every interval with no request-driven CPU allocation — no starvation observed.
- [x] **Export concurrency/rejection check:** determine whether `OTLPMetricExporter` (http) has a concurrency cap analogous to `OTLPTraceExporter`'s default `concurrencyLimit: 30` (which silently rejected exports past that limit — 598 rejections in ~80 minutes on staging, before being fixed). **Confirmed via cross-tool PR review, 2026-08-12: yes** — `OTLPMetricExporterOptions` extends the same `OTLPExporterConfigBase` as the trace exporter and shares the identical library default of 30, verified against the real `@opentelemetry/otlp-exporter-base@0.220.0` source. Code now sets `concurrencyLimit: 200` explicitly (`buildOtlpMetricExporterOptions()`), carried over from the trace exporter's value as a starting point. **Live-verified in staging, 2026-08-12:** zero "Concurrent export limit reached" (or equivalent) log lines on either `ikaro-backend` or `ikaro-bff` across a 20-concurrent-request burst plus real traffic — consistent with, not contradicting, the limit being sufficient at current staging volume. **Residual note:** unlike the trace exporter's 200 (empirically sized from measured staging rejections at real production-scale traffic), this number has not been stress-tested at comparable volume — the same posture the trace exporter's 200 originally shipped with before its own real-traffic validation. Revisit if rejections appear once real tenant traffic exists.
- [x] **Temporality/aggregation correctness check:** confirm the exporter's default temporality is cumulative (Google Managed Prometheus's requirement) rather than assuming OTLP's default "just works" — verify against a real GMP-sourced query, not just that the collector accepts the data without error. Code sets `temporalityPreference: CUMULATIVE` explicitly (see `buildOtlpMetricExporterOptions()`), matching GMP's requirement. **Live-verified in staging, 2026-08-12:** the real `timeSeries.list` response for `prometheus.googleapis.com/http_server_duration_milliseconds/histogram` shows `"metricKind": "CUMULATIVE"`, `"valueType": "DISTRIBUTION"` — `temporalityPreference: CUMULATIVE` is genuinely honored end-to-end, not just set in code.

**Bonus finding, live-verified 2026-08-12, not a blocker:** one collector-side (not app-side) metrics export failure was observed on the new revision's very first export cycle post-deploy — `googlemanagedprometheus` exporter, `context deadline exceeded` against its `timeout: 2s`, 70 items dropped (`ikaro-backend-00078-26p`, `service.instance.id: 14af6d19...`, 23:10:58 UTC, ~53s after the revision became Ready). This matches the exact same cold-start pattern already well-documented for the `googlecloud` (traces) exporter in this pipeline's history (`docs/ENGINEERING_RULES_INFRA.md` § Cloud Run CPU throttling, `infra/docker/otel-collector/README.md`) — a fresh collector instance's first gRPC/auth-channel setup to `monitoring.googleapis.com` exceeding the 2s deadline. Zero repeats observed since (checked the full window on this revision) — consistent with a one-time cold-start cost, not a persistent problem, same conclusion this codebase already reached for the analogous traces-side behavior after its own extended observation.
- [x] Stale-reference sweep (Description step 6) completed in the same commit — all 6 named files updated: `docs/ENGINEERING_RULES.md`, `docs/ANTI_PATTERNS.md`, `docs/10-OBSERVABILITY_STRATEGY.md`, `infra/docker/otel-collector/config.yaml`, `infra/docker/otel-collector/README.md`, this file (S34's forward-reference). **Deepened via cross-tool PR review (CodeRabbit, 2026-08-12):** the first sweep only updated each file's top-level banner/summary; CodeRabbit found `docs/10-OBSERVABILITY_STRATEGY.md` still had 4 deeper stale spots — the `## NestJS OTel Implementation` scope line, an embedded `bootstrapTracing()` code sample still showing `metricReaders: []`, the `## SLOs` section's "no Prometheus pipeline deployed" banner, and a trace-propagation diagram's "traces only" labels. All fixed; the embedded code sample now shows the real metrics reader + a new `buildOtlpMetricExporterOptions()` illustration, matching the actual `otel-tracing.ts`.
- [x] **Tenant_id invariant check (found via cross-tool PR review, CodeRabbit, 2026-08-12):** CLAUDE.md §2 rule 8 requires `tenant_id` on every metric, but the `http.server.duration` histogram this story relies on carries none — confirmed structurally impossible via the auto-instrumentation path: `instrumentation-http@0.220.0`'s metric attributes come from a fixed internal function (`getIncomingRequestMetricAttributes()`) with no hook to inject request-scoped context, verified against the real pinned source. **Decision (user, 2026-08-12): documented as a scoped exception**, not a blocker — added to `docs/10-OBSERVABILITY_STRATEGY.md`'s `## Overview`, with the same cardinality rationale `## Loki Label Strategy` already applies to logs (`tenant_id` as a label multiplies series count; tenant drill-down stays available via logs/traces, which already carry it). A future tenant-scoped request metric would need custom instrumentation (an interceptor reading `RequestContext`) as its own story, not a retrofit here.
- [x] **CI/CD deployment race (found via cross-tool PR review, CodeRabbit, 2026-08-12):** `deploy-staging.yml` and `build-otel-collector.yml` trigger independently on push to main, filtered to disjoint paths, with no cross-workflow dependency — confirmed via the actual workflow files, not assumed. A commit touching both path sets (this one) could have `deploy-staging.yml` resolve `otel-collector:latest` before `build-otel-collector.yml`'s `push` job finished publishing, deploying app code paired with the old, metrics-less collector image and reproducing the exact 404 export-loop this pipeline has already hit three times. **Fixed in this PR:** `deploy-staging.yml`'s `check-current` job gained a new step that finds the most recent commit touching `infra/docker/otel-collector/` via `git log`, then polls GitHub's Actions API for a successful `build-otel-collector.yml` push run at or after that commit (same idiom already established in `infra-deploy.yml`'s `foundation-applied-check`, adapted for a bounded poll since this workflow is fully automatic with no approval gate). Validated with `zizmor`, `actionlint`, and a YAML parse, all clean. `deploy-production.yml` doesn't need the equivalent fix — it's `workflow_dispatch`-only (a human manually promotes an already-verified staging SHA), no automatic push trigger, no race.

**Implementation note (2026-08-12):** everything checkable from a coding session without staging/GCP access is done: app code, collector config, Terraform widget + module tests, the CI ordering fix, the tenant_id documentation, and the doc sweep (both passes). The three explicit empirical-verification checks and the dashboard panel's live rendering are unchecked by design, not overlooked — this story's own §5 explicitly calls for live, empirical verification "not code review alone," matching the discipline this exact pipeline has needed three times before. Whoever deploys this to staging should run those checks before considering the story done, then check the remaining boxes. Two rounds of cross-tool review ran against this PR (Codex via `/pr-review`, then CodeRabbit) — findings from both are reflected above; `.claude/commands/pr-review.md` was also updated with the specific gaps this review surfaced (primary-source verification mandate, checklist-as-floor reframing, metric-tenant_id check, cross-workflow-ordering check), so future reviews are more likely to catch this class of issue on the first pass.

**Dependencies:** M17-S35, M17-S54 (conceptually — same dashboard/monitoring foundation; not a hard blocker)

---

### M17-S56 — Split Engineering/Business dashboards + per-tenant drill-down (Cloud Monitoring `dashboardFilters`) ✅ Done
...
- [x] `dashboard_id_engineering`/`dashboard_id_business` outputs resolve correctly at both the module and each env root (`terraform output` in `envs/staging` and `envs/prod`) — verified 2026-08-13 via PR #364's merge-to-main auto-apply (`.github/workflows/infra-deploy.yml`, run 31699136717): staging `Apply complete! Resources: 1 added, 1 changed, 0 destroyed` — `dashboard_id_business = projects/729809528251/dashboards/c0d591a8-a04f-4ebb-95dc-b6899b529518`, `dashboard_id_engineering = projects/729809528251/dashboards/046a254a-f82a-4bad-8460-250dc3798e49` (same ID as the pre-existing combined dashboard — the `moved` block preserved its identity, exactly as intended); prod applied identically clean with its own real IDs. Both env roots' re-exported outputs resolved along with the module's own.
- [x] Live-verified in staging: both dashboards render post-apply with their correct widget sets; picking a tenant in `.business`'s console filter control actually re-filters the 6 business charts to that tenant's data, not just a clean `terraform apply` — verified 2026-08-13 two ways: (1) `gcloud monitoring dashboards describe` against the real `dashboard_id_business` confirms the stored config matches exactly what was intended (`dashboardFilters: [{filterType: METRIC_LABEL, labelKey: tenant_id, valueType: STRING}]`, no `stringValue`/`templateVariable`, all 6 widgets present with `REDUCE_SUM`/no `groupByFields`); (2) user confirmed directly in the Cloud Monitoring console — both dashboards render, the filter control offers `*` (all tenants) alongside real `tenant_id` values, and selecting a specific tenant re-filters the 6 charts
...
**Implementation note (2026-08-13, updated post-merge same day):** everything checkable from this coding session is done — both dashboard resources, the filter, the split outputs, `terraform fmt`/`validate`/`test` all clean across the module and both env roots, and the full stale-reference sweep. Cross-tool PR review (Codex + CodeRabbit) caught a real gap before merge: the initial resource rename had no `moved` block, which would have destroyed the live combined dashboard and recreated it — fixed and confirmed in the real apply below. PR #364 merged, triggering `infra-deploy.yml`'s auto-apply on both staging and prod — both applied clean (`1 added, 1 changed, 0 destroyed`), resolving the outputs AC with real evidence. The one remaining item is the actual browser/console-interaction check (pick a tenant in `.business`'s toolbar, confirm the 6 charts re-filter) — not yet performed.
...
**Agent:** `devops`
**Complexity:** M
**Docs to load:** this file §2, S22 (edge), S37, and `docs/24-BFF_ARCHITECTURE.md` § throttling/client-IP handling
...
- [ ] This story's production BFF-lockdown acceptance criteria are recorded as complete, with the current ingress/IAM choice documented in §2
...
**Note from M17-S24 (2026-07-21), corrected 2026-07-22 by TD30:** `enable_database` and `enable_edge` were originally both committed as `true`/ungated in prod's tfvars, meaning step 3 below was going to be the first-ever prod apply, creating all **251 resources** (database + edge + everything else) in one shot. `plan/M17-CLOUD-DEPLOY.md` changed this: both now default to `false`, decoupled from the registry/IAM/secrets containers specifically so M17-S27 doesn't secretly depend on this story running first. By the time this story runs, the registry/IAM/secrets containers will already exist from an earlier apply — **this story's actual remaining scope is narrower**: flip `enable_database=true` and `enable_edge=true` in `terraform.tfvars` (Terraform blocks a plan where only one is flipped — `variables.tf`'s cross-variable `validation`, not a mere `check`), then apply. `tf-deployer`'s core permission gaps (state-bucket `list`, project-level IAM management — see S08's corrected bullet) are fixed and proactively applied to prod's `tf-deployer` too, but only staging's apply has actually verified them live; prod's has never run (correctly — it's gated behind `production-infrastructure` approval, deliberately not approved during S24). **Prod's plan creates the `edge` module (ALB, Certificate Manager, Cloudflare DNS records) — resources no CI identity has exercised in *any* environment**, since staging has no Cloudflare resources at all. Budget time in this story for at least one live IAM-gap debugging pass on step 3, the same way M17-S24 needed several rounds for staging (missing APIs, missing `getIamPolicy`-class permissions, missing bucket-list permission, missing project-IAM-admin permission — each only surfaced by actually running the pipeline, never by reasoning about the role list alone). See `docs/BOOTSTRAP_LOG.md` for the full history.

**Further correction (2026-07-23, operational incident):** the "prod's apply-prod has never run" premise above is now partly out of date. A stale, pre-TD30 deployment request got approved by mistake (wrong commit, not this story), which — after recovery (the accidental database was deleted by hand; the accidental partial `edge` HTTP-redirect stub was destroyed by the follow-up correct-commit apply) — ended up exercising prod's real `apply-prod` for the first time anyway, against every module TD30 does *not* gate. Concretely: `network`, `registry`, `iam`, `secrets`, `storage`, `pubsub`, `scheduler`, `migrate_job`, `cloudrun_backend`, and `cloudrun_bff` are now genuinely live in prod (`backend`/`bff` both `Ready: True` at their `*.run.app` URLs — no database yet, so any DB-touching request will fail; no edge/LB/DNS in front of them, so they're not reachable via `ikaro.online`). This also surfaced and fixed the IAM gap this section already predicted: `tf-deployer` was missing `roles/compute.securityAdmin` (needed for `google_compute_firewall.deny_all_ingress` — `compute.networkAdmin` deliberately excludes firewall/SSL-cert permissions), now granted to both envs' `tf-deployer` SAs. Full incident detail: `docs/BOOTSTRAP_LOG.md`'s 2026-07-23 entry. **Net effect on this story:** step 3's IAM-gap risk is now smaller — the broad-strokes first-real-apply gap-hunting is done for every non-`edge` module. The `edge` module itself (ALB, Certificate Manager, Cloudflare DNS/cert issuance) remains genuinely unexercised — no cert or HTTPS backend was ever created during the incident, only the plain HTTP-redirect stub, which was destroyed again. Still budget time for step 3's `edge`-specific portion; just not for the rest.
...
6. **Smoke through the edge:** `https://bff.ikaro.online/v1/health/ready` → 200 via Cloudflare (orange-cloud); login page loads on `https://ikaro.online`; browser login and authenticated calls use `https://ikaro.online/v1/*` with the host-only cookie (verify in Chrome and Safari). Verify origin lockdown active (S36): direct LB-IP access → 403.

   **Hotsite publish + Cloudflare cache (verify here too):** `apps/web/app/api/revalidate/route.ts`'s `revalidatePath`/`revalidateTag` calls only purge Next.js's own origin-level cache — they have no awareness of Cloudflare's edge cache sitting in front once `enable_edge=true`. If Cloudflare caches the hotsite HTML at its edge, a manager's publish action will correctly refresh the origin but the edge may keep serving the pre-publish version until Cloudflare's own cache separately expires. Before relying on "publish is instant" in prod, confirm whether Cloudflare's cache rules for `ikaro.online/*` bypass/short-TTL the hotsite paths, or add an explicit Cloudflare cache-purge API call to `/api/revalidate` alongside the existing `revalidatePath`/`revalidateTag` calls.
7. **First tenant (UC-024):** enable the production relay VM through the protected Foundation workflow, connect with IAP, and run the bundled script inside the VM:
   ```bash
   /usr/local/bin/provision-tenant.sh "<tenant-name>" "<slug>" "<admin-email>" "BR" "America/Sao_Paulo"
   ```
   The script obtains the metadata-server identity token and reads both `platform-admin-key` and `internal-api-key`; no secret is copied to the operator machine or sent through `Authorization`. Verify the `201` response, asynchronous manager/invitation processing, and the tenant hotsite.
...
10. **Remove the now-permanent `enable_database`/`enable_edge` toggle (TD30 follow-through)** — as a separate apply, only after step 9 confirms stability, never bundled into the same apply as step 3's activation:
    1. In `infra/terraform/envs/prod/main.tf`, change `module "database"`'s `count = var.enable_database ? 1 : 0` to `count = 1` (hardcoded) — **not** removing `count` entirely. Same for `module "edge"`'s `count = var.enable_edge ? 1 : 0` → `count = 1`. This keeps the resource address exactly as `module.database[0]`/`module.edge[0]` — Terraform sees no change to the actual resources, only to how the count is computed. Removing `count` entirely instead would change the resource address (`module.database[0]` → `module.database`) and Terraform would plan to destroy and recreate a live, data-bearing Cloud SQL instance — the hardcoded-`count=1` approach avoids that entirely.
    2. Remove the `enable_database`/`enable_edge` variable declarations from `variables.tf` (including `enable_edge`'s cross-variable `validation` — no longer needed once neither can vary) and their entries from `terraform.tfvars`.
    3. **Tests, in order:** `terraform fmt`/`validate` clean → `terraform test` still green for `modules/database`/`modules/edge` (unaffected — they test the modules in isolation, not this call-site change, but confirm rather than assume) → live `terraform plan` against real `ikaro-prod` **must show 0 to add, 0 to change, 0 to destroy**. If it shows anything else, stop and investigate before applying — that means the `count` hardcoding didn't preserve the resource address as expected.
    4. Only apply once the plan confirms zero changes.
    5. Optional cleanup: `outputs.tf`'s `try(module.edge[0].lb_ip_address, "")` no longer needs the `try()` fallback once `edge` can never be absent — safe to simplify to a direct reference, but harmless to leave as-is.
11. Add the runbooks to `docs/RUNBOOKS.md`: relay access and tenant provisioning, rollback, secret rotation, and **DLQ inspection + replay** (per the S19 contract: pull from the `-inspect` subscription, fix root cause, re-publish envelope to the source topic — safe via `eventId` idempotency).
...
**Agent:** `frontend-ts`
**Complexity:** M
**Docs to load:** `docs/15-HOTSITE_DYNAMIC_ARCHITECTURE.md`, `apps/web/proxy.ts` + its spec, S38

**Description:**
`apps/web/proxy.ts` gains host routing: if the request `Host` is not a platform hostname (`ikaro.online`, `www.`, staging/run.app hosts — env-configured allowlist), resolve host → slug via the S38 lookup (through the canonical BFF transport; **the proxy runs on the server — no `bffClient`**; use a fetch helper under `shared/lib/` with unit tests, honoring the CSP/connect-src rules) with an in-memory TTL cache (5 min) to avoid a lookup per request. On hit: rewrite `/<path>` → `/{slug}/<path>` (public hotsite routes only). On miss: 404 page. **Scope decision (document in code + UC-032):** custom domains serve the public hotsite only; any auth/dashboard link on a custom-domain page points absolutely to `https://ikaro.online/...` (cookies never span customer domains). Audit hotsite components for relative links that would break under a custom host (login CTA, booking flow, sitemap/robots — exclude custom hosts from sitemap generation).
...
**Agent:** `devops`
**Complexity:** M
**Docs to load:** entire `docs/` folder via `/docs-audit` (widened from the original infra-only scope). The 6 already-flagged docs (`docs/12-DEPLOYMENT_STRATEGY.md`, `docs/23-INFRASTRUCTURE_SETUP.md`, `docs/09-CI_CD_PIPELINE.md`, `docs/19-INFRASTRUCTURE_TOOLING_MAP.md`, `docs/20-COST_OPTIMIZATION_STRATEGY.md`, `docs/10-OBSERVABILITY_STRATEGY.md`) carry known specific fixes below. Likely additional candidates worth close attention (not yet verified, let `/docs-audit` confirm): `docs/17-GITHUB_WORKFLOWS_GUIDELINES.md` (CI/CD structure fully redone in Wave 3), `docs/22-TECH_STACK_DECISIONS.md`, `docs/24-BFF_ARCHITECTURE.md`. **`docs/18-RELEASE_LIFECYCLE_OPERATIONS.md` (flagged M17-S26, 2026-07-23):** unlike `docs/12`, has no "superseded by M17" banner at all, and its § 4 Rollback Procedures / release flow describe an architecture that was never built here — Fargate (this repo is Cloud Run only, D1), Blue/Green deploys, Loki/Grafana (D9: OTel + Cloud Trace/Monitoring/Logging, no self-hosted stack at launch), AWS Secrets Manager (GCP Secret Manager only), a GitHub Action named "Release to Production"/"Rollback Production" (the real workflows are `deploy-production.yml` and `rollback-production.yml`, and the real rollback mechanism is a Cloud Run revision traffic-shift, not "reverts the container image to the previous tag").
...
4. CSP check: if the web `connect-src`/`img-src` in `apps/web/proxy.ts` enumerates image origins, add the new hostname + spec (repo anti-pattern: silent CSP blocks).
...
1. **Terraform (`modules/storage`), uploads bucket only:**
   ...
   - Age `var.booking_photo_retention_days` → `Delete`. Default **365** (confirmed at `/story-discovery`, 2026-08-14 — must be ≥ the tenant dispute/warranty window; it is a business + LGPD decision).
   - Scope rules with `matches_prefix = ["tenants/"]` so any future non-photo object class in this bucket isn't caught accidentally; document the constraint in the module README.
...
2. **App-side graceful degradation (frontend-only — confirmed at `/story-discovery`, 2026-08-14):** after deletion, DB photo references dangle by design (no cleanup job — the storage layer is the source of truth for existence). No backend change needed: `get-booking-by-id.use-case.ts`'s `signPhotoUrls()` already signs blind with no existence check, and the photo `<img>` GET goes direct browser→GCS, never through our backend/BFF — so there is no server-side "not found" to type. Add a `PhotoTile` component (`onError` swaps the `<img>` for a same-footprint placeholder — `ImageOff` icon + label, `aspect-square`/rounded/border unchanged) in both `apps/web/features/booking/components/dashboard/bookings/BookingDetailMain.tsx` and `apps/web/features/customer/components/my-account/BookingDetailMain.tsx` (duplicated views — both need the fix). New i18n keys `photoUnavailableLabel`/`photoUnavailableAlt` under both `dashboard.bookingDetail` and `customer.bookingDetail` namespaces, pt-BR + en. Specs for the missing-photo path in the same commit (`.spec.tsx` per repo rule).
3. **Orphan check (hotsite bucket):** verify whether removing/replacing an image in `GalleryImageManager` deletes the old GCS object. **Confirmed at `/story-discovery`, 2026-08-14: `GalleryImageManager.handleRemove()` already calls delete-from-GCS for promoted (`tenants/`) and tmp paths** — re-verify during implementation rather than assuming a gap; only file a `td/` entry if that verification finds a real hole.
...
- [ ] `PhotoTile` renders the localized placeholder on image load failure in both `dashboard.bookingDetail` and `customer.bookingDetail` views (specs both locales)
