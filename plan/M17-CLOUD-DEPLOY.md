...
3. Journeys (golden paths only): guest booking on hotsite; customer login + booking + “Próximos agendamentos”; staff approval via CommandCenter; complete + loyalty points visible; hotsite renders per tenant (modules + `--ba-*` branding).
   - **Cross-reference (added 2026-07-22, `td/TD29-WEB-RUNTIME-PUBLIC-CONFIG.md`):** since every journey here already runs a real browser against the real deployed staging web service (a genuine `next build`/`next start` artifact, not `next dev`), this suite is the first point TD29's deferred verification actually gets exercised — no client-side request should ever resolve to a same-origin/relative URL instead of `bff.ikaro.online`'s staging equivalent. A guest-booking or login journey failing with a same-origin 404 on a BFF call is exactly the TD29 regression this was deferred here to catch.
