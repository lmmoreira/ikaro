...
- **`TF_VAR_brevo_smtp_login` (S16 discovery, same treatment as `iam_admin_user`):** plan/apply jobs must also supply this from a GitHub **environment variable** (`vars.BREVO_SMTP_LOGIN`) on `staging-infrastructure` / `production-infrastructure` — not a Secret (a non-sensitive login string; `infra/terraform/envs/{staging,prod}/variables.tf`'s `brevo_smtp_login` already documents this exact wiring, tagged S24). Neither `IAM_ADMIN_USER` nor `BREVO_SMTP_LOGIN` exists yet on either environment — verified live in story-discovery (2026-07-21); creating both is part of this story.
