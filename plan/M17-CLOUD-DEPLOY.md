...
- [x] Collector's metrics pipeline coded (`infra/docker/otel-collector/config.yaml`'s `metrics:` pipeline + `googlemanagedprometheus` exporter) and validated against the real pinned `otelcol-contrib:0.157.0` binary (`docker run ... validate`, exit 0; full `Dockerfile` build also succeeds) — **a real GMP-sourced panel actually rendering on the S35 dashboard is still open** (needs a live deploy + real traffic)
...
- [x] Stale-reference sweep (Description step 6) completed in the same commit — all 6 named files updated: `docs/ENGINEERING_RULES.md`, `docs/ANTI_PATTERNS.md`, `docs/10-OBSERVABILITY_STRATEGY.md`, `infra/docker/otel-collector/config.yaml`, `infra/docker/otel-collector/README.md`, this file (S34's forward-reference)

**Implementation note (2026-08-12):** everything checkable from a coding session without staging/GCP access is done: app code, collector config, Terraform widget + module tests, and the doc sweep. The three explicit empirical-verification checks and the dashboard panel's live rendering are unchecked by design, not overlooked — this story's own §5 explicitly calls for live, empirical verification "not code review alone," matching the discipline this exact pipeline has needed three times before. Whoever deploys this to staging should run those checks before considering the story done, then check the remaining boxes.
