...
- [ ] **Export concurrency/rejection check:** determine whether `OTLPMetricExporter` (http) has a concurrency cap analogous to `OTLPTraceExporter`'s default `concurrencyLimit: 30` (which silently rejected exports past that limit — 598 rejections in ~80 minutes on staging, before being fixed). If so, size it explicitly from real staging load, the same way the trace exporter's `concurrencyLimit: 200` was sized empirically, not assumed safe from the library default. **Not run — needs a live staging deploy.**
