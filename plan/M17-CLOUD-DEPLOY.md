...
- [ ] Dedicated `infra/docker/otel-collector/**`-triggered workflow builds, validates, and pushes `:latest` (+ commit SHA) to GAR — decoupled from `deploy-staging.yml`'s own `apps/**`-scoped trigger
- [ ] Backend + BFF `gcloud run deploy` steps resolve `ikaro-otel-collector:latest` to a concrete digest immediately before deploying, so the live sidecar is always digest-pinned, never on a floating tag
- [ ] `sidecar_containers`' `image` field is added to the S18 module's `lifecycle.ignore_changes` — a `terraform apply` never reverts the live collector digest
- [ ] Collector config/exporter change → rebuild via the dedicated workflow → next backend/BFF deploy picks it up automatically, no manual Terraform edit and no manual `terraform apply` needed; documented in `infra/docker/otel-collector/README.md`
- [ ] Config `validate` runs in the image build; a deliberately broken config fails the build (prove once)
- [ ] `.github/dependabot.yml` gains a `docker` ecosystem entry for `/infra/docker/otel-collector`
- [ ] web has no sidecar container and no OTel wiring — confirm its deploy step stays single-container
