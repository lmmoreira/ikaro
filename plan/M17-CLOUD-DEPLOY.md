...
| Tenant provisioning | IAP relay VM inside the VPC + relay service-account IAM identity + `INTERNAL_API_KEY` + `PLATFORM_ADMIN_KEY` (timing-safe, sent via `X-Platform-Admin-Key`) + strict rate limit. The bundled `/usr/local/bin/provision-tenant.sh` script supplies the headers and request body. |
...
4. **Validation checklist (all deferred items from Wave 2 land here):**
   ...
   - [x] Tenant provisioning via the IAP-reachable relay VM and bundled `/usr/local/bin/provision-tenant.sh`, with both Secret Manager values read by the relay service account (TD32, ✅ Done; a real staging tenant was provisioned through the relay on 2026-07-27).
