...
**Note from M17-S24 (2026-07-21):** `enable_database=true` is already committed in prod's tfvars (unlike staging, which stays `false` until S27 flips it) — a live `terraform plan` as of this date shows **251 resources to add**, i.e. step 3 below (`Prod Terraform apply`) is genuinely the *first-ever* prod apply, not an incremental change. `tf-deployer`'s core permission gaps (state-bucket `list`, project-level IAM management — see S08's corrected bullet) are fixed and proactively applied to prod's `tf-deployer` too, but only staging's apply has actually verified them live; prod's has never run (correctly — it's gated behind `production-infrastructure` approval, deliberately not approved during S24). **Prod's plan additionally creates the `edge` module (ALB, Certificate Manager, Cloudflare DNS records) — resources no CI identity has exercised in *any* environment**, since staging has no Cloudflare resources at all. Budget time in this story for at least one live IAM-gap debugging pass on step 3, the same way M17-S24 needed several rounds for staging (missing APIs, missing `getIamPolicy`-class permissions, missing bucket-list permission, missing project-IAM-admin permission — each only surfaced by actually running the pipeline, never by reasoning about the role list alone). See `docs/BOOTSTRAP_LOG.md` for the full history. **Corrected 2026-07-22, TD30:** the registry, IAM, and secrets containers referenced above will already exist by the time this story runs — `td/TD30-PROD-BOOTSTRAP-GATING.md` decouples them from `database`/`edge` specifically so M17-S27 doesn't secretly depend on this story happening first. This story's actual remaining scope is narrower than "251 resources from scratch": flipping `enable_database`/`enable_edge` and applying just those two, not re-creating everything.
