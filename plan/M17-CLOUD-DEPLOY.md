...
**Agent:** `backend-ts`
**Complexity:** M
**Docs to load:** `docs/04-USE_CASES.md` § UC-018/019/020, M11 IA file, S02 implementation (`PubSubPushGuard`, `PubSubPushController`, `GcpPubSubEventBusAdapter`), `docs/ENGINEERING_RULES_BACKEND.md` § Event Handlers (test-wiring table) + § `RequestContext` is HTTP-request-scoped only (`*.job.ts` category)
