...
**Description:**
`workflow_dispatch` with input `image_sha`. Promotes the staging-validated artifacts: backend, bff, **and web** all by **exact same image** — **revised 2026-07-22, td/TD29-WEB-RUNTIME-PUBLIC-CONFIG.md**: web's per-environment rebuild (D8's original exception) is gone. TD29 removed every build-time `NEXT_PUBLIC_*` dependency from the client bundle, so the same `ikaro-web:<sha>` validated in staging promotes unmodified, same as backend/bff. Prod-specific values (`NEXT_PUBLIC_BFF_URL=/v1`, `BFF_UPSTREAM_URL=https://bff.ikaro.online/v1`, etc.) are supplied as **Cloud Run runtime env vars** on the prod web service (`modules/cloudrun-service`'s prod composition — see S25's note on the same Terraform gap), not baked into a separate image.
...
**Dependencies:** Waves 2–3 complete (except S22/S26 which are prod-only); TD30 Story 1 (prod's registry/IAM/secrets containers applied — Step 3's first real deploy needs the shared Artifact Registry, D8, which otherwise only exists via S37's atomic apply — see td/TD30-PROD-BOOTSTRAP-GATING.md)
