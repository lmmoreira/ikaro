...
`docs/archive/td/TD38-BFF-CLIENT-IP-RESOLUTION-BROKEN-BY-SAME-ORIGIN-GATEWAY.md`. Prod is unchanged (Story B,
...
3. **Cron topics:** `ikaro-cron-reminders` (booking, `*/30 * * * *`), `ikaro-cron-loyalty-expiry` (loyalty daily expiry, `0 2 * * *`), `ikaro-cron-loyalty-expiry-warning` (loyalty weekly warning, `0 6 * * 1`) — three topics as of S03 (S03 splits the previously-bundled controller calls into independent trigger handlers, and keeps loyalty's two concerns on their pre-existing separate cadences rather than merging them). `ikaro-cron-reminders` gets **two** independent push subscriptions; the two loyalty topics get **one** subscription each. All four subscriptions use the *same* shared push config as point 2 above (`/pubsub/push`, not a dedicated cron URL): `ikaro-cron-reminders-booking-reminder`, `ikaro-cron-reminders-booking-admin-schedule-reminder`, `ikaro-cron-loyalty-expiry-loyalty-expire-points`, `ikaro-cron-loyalty-expiry-warning-loyalty-notify-expiring-points`. **Not a fixed count (added 2026-07-11, TD24 cross-reference):** this list is not hardcoded in Terraform — the scanner in point 1 derives it from actual `registerTrigger()` call sites, so it grows automatically as new triggers are registered in code. `docs/archive/td/TD24-OUTBOX-INBOX-PATTERN.md` (S01) adds a 4th trigger, `cron-outbox-relay`, registered the same way as the three above — once that story lands, the scanner picks it up with zero Terraform logic change here. Don't read "three topics" as a ceiling this module enforces.
...
**Description:**
`workflow_dispatch` with input `image_sha`. Promotes the staging-validated artifacts: backend, bff, **and web** all by **exact same image** — **revised 2026-07-22, docs/archive/td/TD29-WEB-RUNTIME-PUBLIC-CONFIG.md**: web's per-environment rebuild (D8's original exception) is gone. TD29 removed every build-time `NEXT_PUBLIC_*` dependency from the client bundle, so the same `ikaro-web:<sha>` validated in staging promotes unmodified, same as backend/bff. Prod-specific values (`NEXT_PUBLIC_BFF_URL=/v1`, `BFF_UPSTREAM_URL=https://bff.ikaro.online/v1`, etc.) are supplied as **Cloud Run runtime env vars** on the prod web service (`modules/cloudrun-service`'s prod composition — see S25's note on the same Terraform gap), not baked into a separate image.
...
**Dependencies:** Waves 2–3 complete (except S22/S26 which are prod-only); TD30 Story 1 (prod's registry/IAM/secrets containers applied — Step 3's first real deploy needs the shared Artifact Registry, D8, which otherwise only exists via S37's atomic apply — see docs/archive/td/TD30-PROD-BOOTSTRAP-GATING.md)
