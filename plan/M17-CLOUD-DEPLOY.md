...
| # | Decision | Rationale |
|---|---|---|
| D1 | **Cloud Run for everything** — backend, BFF, web as services; migrations as a Cloud Run Job. **No Cloud Functions/lambdas, no VMs** for application workloads. **Narrow exception (TD32, 2026-07-24):** a single minimal, on-demand `e2-micro` relay VM (`modules/relay-vm`) exists solely to give an authorized human IAP-only network access to VPC-internal resources (Cloud SQL's private IP, the ingress-internal backend) — nothing reachable any other way once §2's "no VPN, no bastion" posture turned out to also mean "no way in at all" for a human. It runs no application code, is count-gated (`create_relay_vm`, default `false`), and is destroyed after each session — infrastructure-access tooling, not a compute workload. D1's actual intent (no rewriting app services into VMs/functions) is untouched. | Existing Dockerfiles run as-is; scale-to-zero gives lambda economics without rewriting NestJS/Next.js into function handlers. |
