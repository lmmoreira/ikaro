...
6. **Rollback procedure (workflow header + `docs/RUNBOOKS.md` section):** re-run with the previous SHA (Cloud Run also keeps previous revisions for instant console rollback of a single service). Migrations follow expand/contract (repo rule) so rolling back code without reverting schema is safe by construction; `migration:revert` is the documented last resort.
