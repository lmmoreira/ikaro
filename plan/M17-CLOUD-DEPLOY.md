...
4. **Validation checklist (all deferred items from Wave 2 land here):**
   ...
   - [ ] Tenant provisioning via `gcloud run services proxy` + `PLATFORM_ADMIN_KEY` documented and exercised (creates the staging demo tenant used by E2E) ⚠️ **blocked — see `td/TD32-CLOUD-SQL-DEVELOPER-ACCESS-NO-NETWORK-PATH.md`**: `gcloud run services proxy` does not bypass the backend's `ingress: internal` setting (ingress is enforced at Google's front-end before IAM auth is even checked, and the proxy tool still sends the request over the same public path a raw `curl` would) — a real attempt 404'd identically regardless of auth validity. Same root cause and same proposed fix (an IAP-reachable relay VM inside the VPC) as the Cloud SQL gap; filed as a follow-up, not hotfixed here.
