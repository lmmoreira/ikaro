...
| D4 | **Backend is internal-ingress only.** No public URL, no Cloud Armor + IAP stack (old M15-S12 is cut). | Three layers already exist: internal ingress (network) + `InternalApiGuard` shared secret (M115-S03) + `PLATFORM_ADMIN_KEY` timing-safe check (M02-S05). **Operator access via the on-demand IAP relay VM (TD32, 2026-07-24)** — `gcloud run services proxy` does **not** work for this: it still sends the request over the same public path a raw `curl` would, so `ingress: internal` blocks it regardless of IAM validity (confirmed by a real attempt against staging). |
...
- [ ] Checkov clean
- [ ] `admin@ikaro.online` granted `run.invoker` on the backend service; `gcloud run services proxy ikaro-backend --region=southamerica-east1` succeeds from that identity (verify post-S27 alongside the other live-state items above)
...
4. **Validation checklist (all deferred items from Wave 2 land here):**
   ...
   - [x] Tenant provisioning via `gcloud run services proxy` + `PLATFORM_ADMIN_KEY` documented and exercised (creates the staging demo tenant used by E2E) — direct `gcloud run services proxy` access remains blocked (see `td/TD32-CLOUD-SQL-DEVELOPER-ACCESS-NO-NETWORK-PATH.md`: ingress is enforced at Google's front-end before IAM auth is even checked, so the proxy tool 404'd identically regardless of auth validity), **superseded and resolved via the IAP-reachable relay VM** (TD32, ✅ Done, validated via PRs #259–#261, 2026-07-25; re-confirmed live 2026-07-27 — a real tenant was provisioned on staging through the relay).
...
11. Add the runbooks to `docs/RUNBOOKS.md`: proxy access, rollback, secret rotation, and **DLQ inspection + replay** (per the S19 contract: pull from the `-inspect` subscription, fix root cause, re-publish envelope to the source topic — safe via `eventId` idempotency).
