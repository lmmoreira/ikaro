...
- `apps/backend/src/test/integration-global-setup.ts` (modify — register the new migration class; `ScheduleClosureEntity`/`ScheduleOpeningEntity` are already registered. **Added during story discovery 2026-09-03**, missing from the original file list — mandatory per `docs/ENGINEERING_RULES_TESTING.md` § Migration/entity registration)
...
The shipped design: **`ResourceFilterMenu`** (a floating, multi-select checkbox filter mirroring `ScheduleStatusFilterMenu`'s existing trigger+popover shape — zero checked = tenant-wide default, one-or-more checked = those resources' closures/openings merged into the timeline, fetched via `useQueries` as an explicit tenant-wide request plus one request per checked resource — the backend's `resourceId` filter is exact, never both at once, so the tenant-wide items can't be assumed to ride along inside a resource-scoped response — and de-duplicated client-side) for the *view*, and **`ResourceSelectField`** (a single-select `<select>` embedded directly inside `ClosureFormSheet`/`OpeningFormSheet`, always resetting to "Todo o negócio" fresh on open, never inheriting the filter menu's selection) for *which resource a new block/opening applies to*. Everything below this note that still describes a single `ResourcePicker` reflects the original, superseded design — see the corrected Files/Acceptance Criteria further down and `plan/journey/staff/prototypes/horarios/dev-notes.md`'s own "What shipped" section for the authoritative final shape.
...
- `apps/web/features/booking/schedule/schedule-preferences.ts` (modify — add `selectedResourceIds`/`setSelectedResourceIds`, persisted the same `localStorage`-backed way as `selectedStatuses` but keyed by `tenantId`, since resource ids — unlike the fixed `BookingStatus` enum — are tenant-specific)
- `apps/web/features/booking/schedule/schedule-page-query-data.ts` (modify — thread `resourceIds: readonly string[]` into the `useScheduleClosures`/`useScheduleOpenings` calls; gate the SSR `initialData` fallback on the selection being empty — the server-fetched initial week data is always tenant-wide, so it must not be used as a stale fallback once any resource is checked; surfaces a `scheduleFetchError` when either fetch fails)
- `apps/web/features/booking/schedule/schedule-page-core-data.ts` (modify — pass `selectedResourceIds` through to `useScheduleQueryData`, derive `selectedResourceIdSet`, reconcile the persisted selection against the tenant's live active-resource list every render for MANAGER)
- `apps/web/features/booking/schedule/useSchedule.ts` (modify — `useScheduleClosures`/`useScheduleOpenings` now accept `resourceIds: readonly string[]`; when non-empty, fan out an explicit tenant-wide request plus one request per id via `useQueries` — the backend's `resourceId` filter is exact, so the tenant-wide scope can't ride along inside a resource-scoped response — merge+de-duplicate by item id, and surface `isError`/`error` if any request fails; mutation hooks unchanged)
...
- Unit:
  ...
  - [x] `useScheduleClosures`/`useScheduleOpenings` accept `resourceIds: readonly string[]`, fan out an explicit tenant-wide query plus one query per id whenever the array is non-empty, merge+de-duplicate by item id, surface `isError`/`error` on any failed query, and omit `resourceId` from the single outgoing request when the array is empty (regression guard for the "byte-identical default behavior" AC)
...
**Agent:** `backend-ts`
**Complexity:** S
**Docs to load:** `docs/ENGINEERING_RULES_BACKEND.md` § Choosing a race-condition primitive, and where its lock port should live; `docs/ENGINEERING_RULES_BACKEND.md` § Transactions (Scope rule); `docs/02-DOMAIN_MODEL.md` § Resource (Cross-context note); `docs/04-USE_CASES.md` UC-045/UC-046/UC-048/UC-049
