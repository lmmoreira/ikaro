...
- **Lint:** every migration file needs an explicit entry in `apps/backend/eslint.config.js`'s `PERSISTENCE_BYPASS_IGNORES` array (TD37-S02's `QueryRunner`-import ban has no directory-wide `migrations/**` exemption, only a per-file allowlist) — added `1748500000008-BackfillLocationResources.ts` alongside the S01 entry.
...
- `apps/backend/src/contexts/booking/infrastructure/backfill-location-resources.integration.spec.ts` (new — `createBookingIntegrationApp()`-based integration test invoking the migration's `up(queryRunner)` directly — see Test strategy above; lives outside `migrations/` per the jest-config correction above)
- `apps/backend/eslint.config.js` (modify — add the new migration file to `PERSISTENCE_BYPASS_IGNORES`)
- `apps/backend/src/test/builders/platform/tenant-entity.builder.ts` (modify — add `withName()`, matching the existing `withSlug()`/`withIsActive()` pattern; needed to assert the generated `<tenant.name> (unidade única)` resource name against distinct, non-default tenant names in the test)
