...
> The CI infrastructure work originally scoped here — spinning up the full stack (docker-compose infra + backend + BFF + web) in GitHub Actions and running Playwright per-PR — has been implemented in `.github/workflows/pr-tests.yml` as part of AUD-015. The existing spec suite (`guest-booking.spec.ts`, `localization.spec.ts`, `not-found.spec.ts`, `hotsite-auth-bar.spec.ts`) already runs in CI on every PR against the local dev stack.
>
