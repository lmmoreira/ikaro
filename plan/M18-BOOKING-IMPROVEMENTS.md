...
- `HeroModule.tsx`'s centered variant: replace the mobile-default height class (`min-h-screen`) with `aspect-[21/9]`, keeping `sm:min-h-[60vh]` unchanged for `sm:` and up — only the default (mobile) breakpoint's height-determination mechanism changes; `sm:` and larger are untouched since desktop was never reported broken.
- Left-aligned variant's right-panel image container (line 166): same treatment — replace the mobile-default `h-64` with `aspect-[21/9]`, keeping `sm:h-full sm:min-h-[40vh]` unchanged. The section's own overall `min-h-screen sm:min-h-[60vh]` (line 152, governs the whole hero section's height, including the text column) is a separate whitespace/layout concern, not an image-crop concern — out of scope here.
...
- [ ] Both `HeroModule` variants use `aspect-[21/9]` on the mobile-default breakpoint instead of viewport-relative height (`min-h-screen`/`h-64`); `sm:` and up are unchanged from today
