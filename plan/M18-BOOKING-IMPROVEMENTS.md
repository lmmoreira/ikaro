...
- [ ] Both `HeroModule` variants replace their mobile-default viewport-relative height (`min-h-screen`/`h-64`) with a `21:9`-equivalent mechanism — literal `aspect-[21/9]` for the left-aligned variant's standalone image panel, `min-h-[42.86vw]` (the same ratio as a floor, to avoid clipping the overlaid text) for the centered variant; `sm:` and up are unchanged from today
