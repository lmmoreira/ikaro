...
**Module-type registration (moved here from S07 during story-discovery, 2026-08-24 — S07 has no dependency ordering before S01, but S01's own `PATCH` needs `'LEAD_FORM'` to already be a valid module type or it cannot write a `layout[]` entry for it):** add `'LEAD_FORM'` to `packages/types/src/enums.ts`'s `HotsiteModuleType` union. Add a matching `LeadFormModuleData` interface to the **backend-local** `apps/backend/src/contexts/platform/domain/hotsite-config.types.ts` (mirrors that file's own `ChatbotModuleData`'s "mirrors packages/types" convention — `{ title: string; subtitle?: string; eyebrow?: string; ctaLabel: string; variant?: 'centered' | 'left-aligned'; backgroundImageUrl?: string | null; backgroundImagePosition?: 'left' | 'center' | 'right'; bgStyle?: 'primary' | 'background' }`) and add it to that file's own `HotsiteModuleData` union. Add `'LEAD_FORM'` to `hotsite-config.aggregate.ts`'s `MODULE_TYPES` Set — no entry needed in `MODULE_DATA_VALIDATORS`, since the teaser data carries no special business rule of its own (unlike `BOOKING_CTA`'s `carouselDays` check). S07 still owns `packages/types/src/hotsite.ts`'s own `LeadFormModuleData` (the frontend/BFF-facing copy) — see S07's own corrected checklist item 1.
...
- [ ] `'LEAD_FORM'` registered in `packages/types/src/enums.ts`'s `HotsiteModuleType`, the backend-local `HotsiteModuleData` union (`hotsite-config.types.ts`), and `MODULE_TYPES` (`hotsite-config.aggregate.ts`) — a `HotsiteModule` literal with `type: 'LEAD_FORM'` type-checks and passes `validateLayout()` without throwing `HotsiteModuleTypeInvalidError`
...
1. `packages/types/src/enums.ts`'s `HotsiteModuleType` and the backend-local `hotsite-config.types.ts`/`MODULE_TYPES` registration are **already done by S01** (moved there during story-discovery, 2026-08-24 — S01's own consolidated `PATCH` needed `'LEAD_FORM'` to be a valid module type before S07 would otherwise have landed it). Confirm they're present rather than re-adding them. This story's own scope is only `packages/types/src/hotsite.ts` — add `LeadFormModuleData` (`title`, `subtitle?`, `eyebrow?`, `ctaLabel`, `variant?`, `backgroundImageUrl?`, `backgroundImagePosition?`, `bgStyle?` — same shape family as `BookingCtaModuleData`, deliberately excluding `audienceMode`/`questions`, which live behind S01's own endpoints, not the cached manifest; must match S01's backend-local interface shape exactly).
