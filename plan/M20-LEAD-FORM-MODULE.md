...
- [ ] A required question left unanswered → `400 GENERIC_FIELD_REQUIRED`; an answer for a `questionId` not in the current catalog → `400 GENERIC_VALUE_INVALID`, whole submission rejected, backend use case never calls `CreateLeadFormSubmissionUseCase` — both are unit-tested against `SubmitLeadFormUseCase` directly, using the catalog read from a stubbed `ILeadFormConfigRepository`
