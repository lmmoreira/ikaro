...
**Description:**
Add a **basic** search input above the list on `apps/web/app/dashboard/leads/page.tsx`, wired to S12's `?search=`. **Button-driven, not live/debounced (locked in at story-discovery, 2026-08-27 — overrides an earlier debounced draft):** the search box and the date range control (below) share one **"Aplicar"/"Limpar"** button pair — typing into the search box or picking dates does nothing by itself; clicking **"Aplicar"** submits the search term (when ≥3 chars) together with any active date range in one navigation; **"Aplicar"** is disabled while the search box holds 1-2 characters (mirrors S12's own backend rejection, so the UI never round-trips a request it knows will `400`); **"Limpar"** clears the search box and date range together, back to the unfiltered list, without leaving basic mode. Add an **"+ Adicionar filtro"** affordance that reveals structured filter rows — each a [pergunta ▾] (populated from `GET .../submissions/filter-options`) + [valor] pair, repeatable, wired to `?filters=` as a JSON-encoded array; this **advanced** mode gets its own **"Aplicar filtros"/"Limpar filtros"** pair governing the filter rows *and* the date range together (same button-driven shape as basic mode — neither the rows nor the range are live-typed), staying in advanced mode. Basic search and advanced filters are alternative *modes*, switched via a **"Busca avançada"/"Voltar para busca simples"** toggle — switching modes clears the other mode's active query (matching S12's mutually-exclusive backend contract) but never clears the shared date range control. Enforce the 3-character minimum client-side too: disable "Aplicar" while the search box holds 1-2 characters; disable "Aplicar filtros" while any active row's value holds 1-2 characters — matching S12's backend rejection so the UI never round-trips a request it knows will `400`.
...
**Acceptance Criteria:**
- [ ] Basic search box: button-driven via "Aplicar" (not live/debounced) — "Aplicar" disabled while the box holds 1-2 characters; empty box + "Aplicar" is equivalent to no search active
- [ ] Advanced filters: add/remove rows (max 5), each a question dropdown + value input; "Aplicar filtros" disabled while any active row's value holds 1-2 characters; submitting sends the correctly-shaped `filters` JSON array
