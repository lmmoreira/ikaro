...
**Devops PR sequence (3 PRs — `infra/terraform/README.md`'s "New-resource PR-sequencing playbook", row "A new secret whose consumer degrades safely if unset," confirmed with the user during story-discovery, 2026-08-25 — the safe row, not the accept-risk row):** PR1 (`envs/*`): container only (`modules/secrets`'s `base_secret_ids`), no `env.validation.ts` schema entry yet (`ConfigService.get(key, fallback)` falls back to raw `process.env` for an undeclared key). PR2 (`foundation` only): the accessor grant (`secret_accessors_base` in `runtime-identities` + both env roots' `secret_ids` map) — **immediately after this PR merges and applies, populate the real value** (`gcloud secrets versions add TURNSTILE_SECRET_KEY --project=<project> --data-file=-`), an ordered step, not a footnote. PR3 (`envs/*`): the `env.validation.ts` schema entry **and** `secret_env_vars` wiring together (`env-contract`'s CI check requires both in the same PR). `NEXT_PUBLIC_TURNSTILE_SITE_KEY` is a plain public env var (not a secret) — 1 PR, `env.validation.ts` + `env_vars` in `cloudrun_web`, both env roots, together.
