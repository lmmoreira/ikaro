...
**Correction found during PR #423 review (rounds 2–4), 2026-08-25 — prod's `turnstile_site_key` no longer has a default, and the real value is supplied via a GitHub Actions repository variable, not `terraform.tfvars`:** staging keeps its Cloudflare always-pass test-sitekey default (safe — staging never serves real customer traffic), but Codex correctly flagged that a same-shaped default in prod would let the always-pass test sitekey silently reach production undetected, defeating Turnstile's actual protection (Cloudflare's own guidance: test credentials must never reach production). Fix: prod's `variable "turnstile_site_key"` in `envs/prod/variables.tf` has no default, so `terraform plan`/`apply` fails closed with "No value for required variable" until one is supplied. The user explicitly chose the GitHub Actions repository-variable route over committing the value to `envs/prod/terraform.tfvars` (both are equally safe — the sitekey is not a secret, Cloudflare's own docs confirm it's meant for client-side embedding — but the `vars.*` route was preferred): `vars.TURNSTILE_SITE_KEY` is injected as `TF_VAR_turnstile_site_key` in every prod-scoped Terraform step in `infra-deploy.yml` (plan, apply, verify-normal-deployer, drift), the same `TF_VAR_*` pattern already used for `iam_admin_user`/`notification_email`. A non-blocking `check` block (`turnstile_site_key_not_test_default`) in `envs/prod/main.tf` additionally guards against someone supplying one of Cloudflare's documented test sitekeys as the "real" value. **Outstanding:** the `vars.TURNSTILE_SITE_KEY` GitHub Actions repository variable itself still needs to be created with Ikaro's real Cloudflare Turnstile sitekey (from registering a widget for `ikaro.online`) before prod's Terraform plan/apply can succeed — this can't be done by an agent, since it requires a real Cloudflare account action.
