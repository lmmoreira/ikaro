...
- **E2E, login-required gate + returnTo:** same or a third spec — `audienceMode: CUSTOMER_ONLY` + unauthenticated → gate shown (not the form); completing login lands back on `/[slug]/lead-form`, now resolved as the prefilled path (this is the one assertion that actually exercises the new `returnTo` mechanism end-to-end).
...
- [ ] After logging in from the gate, the customer returns to `/[slug]/lead-form` (via the new OAuth `returnTo` field), which now resolves as the prefilled path — not the hotsite home
