...
**Blocking prerequisite, found via `/docs-audit` 2026-09-17 — resolve before/at the start of implementation:** there is no `GET` endpoint anywhere (backend or BFF) to read a service's active intake schema or its version history — only `POST /services/:id/intake-schema` (publish) exists. `IServiceIntakeSchemaRepository.findActiveByServiceId()`/`findAllByServiceId()` exist at the repository layer with zero controller callers, and `GET /services/:id` doesn't surface the schema at all. The "Formulário de reserva" tab's question builder (pre-filled from the active version) and version-history list cannot be implemented against a real API until this is added. Decide at `/story-discovery M22-S04`: fold a small backend/BFF read-endpoint addition into this story, or split it into a preceding story this one depends on. See `docs/02-DOMAIN_MODEL.md` § Aggregate: ServiceBookingIntakeSchema and `docs/14-API_CONTRACTS.md`.

**Files to create/modify:**
- `apps/web/features/booking/api/services.ts` (modify — add `updateServiceResourceRequirements`, `updateServiceLegs`, `updateServiceBookingPolicy`, `publishServiceIntakeSchema` functions, matching the existing plain-async-function shape; a `getServiceIntakeSchema`-style read function is blocked on the prerequisite above)
...
- [ ] Admin can add/reorder/remove intake-schema questions (free text, named attendees, pickup address) on the Formulário de reserva tab, each marked required/optional, and publish a new version; the previous version stays visible as history (blocked on the prerequisite above).
...
- Unit:
  ...
  - [ ] `ServiceIntakeSchemaPanel` shows a read-only preview (a simple dialog is sufficient) of any non-active intake-schema version on click — found via the same round of testing: *"there is no way to see older versions?"*. Requires the still-open read-path prerequisite above (a real historical-version fetch, not just the active one)
  ...
  - [ ] Each tab's save mutation resolves without navigating away from `ServiceEditPage`; a shared dirty-state store (not per-component local state) drives both the tab-label dot and the router-level unsaved-changes guard — found via UX review, 2026-09-17: every tab PATCHes a different endpoint with no shared "have I saved this" signal in the old design, which also had every save redirect to the list (would have forced re-opening the service after each tab)
