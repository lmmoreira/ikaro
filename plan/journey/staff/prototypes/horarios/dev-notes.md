...
- **Story 2 (`/story-discovery` 2026-09-25) — replaced the per-resource `ResourceNameBadge` row with a compact summary line:** Story 1's original render gave each matched booking one `ResourceNameBadge` per matched resource, wrapped inline next to the status badge — unbounded height growth as match count grew, a risk Story 1's own implementation notes flagged as unverified (short bookings + `flex-wrap` could clip under `overflow-hidden`). Story 2 replaces it with a fixed-shape single line instead: the title/subtitle/status row is unchanged; a new line is added directly below it, separated by a divider (the existing time-range text moves below this new line). Zero matched resources → no divider, no line (non-regression). Exactly one match → the name, no count. Two or more → the type-prioritized name (**STAFF > ROOM > EQUIPMENT**, alphabetical tiebreak within the same type — priority is baked into `resourceNames`' own sort order, not re-derived by the renderer) + a `+N` suffix for the rest (e.g. 3 matched → "Camila +2"). The line's `aria-label` lists every matched name, not just the visible one. `data-testid="timeline-block-resource-summary"` — distinct from `timeline-block-resource-name`, which stays reserved for openings/closures (untouched by this story). Files: `schedule-resource-priority.ts` (new — `resourceTypeRank`/`compareResourceIdsByTypePriority`, extracted into its own module rather than inlined, since `schedule-page-timeline-derived.ts` was already near the 250-line cap), `schedule-page-core-data.ts` (new `resourceTypeById` lookup, sibling to `resourceNameById`), `schedule-page-timeline-derived.ts` (`buildBookingResourceNamesById` now sorts by type priority before mapping ids to names), `BookingResourceSummaryLine.tsx` (new — the rendered line itself, extracted from `ScheduleTimelineEventRenderer.tsx` for the same file-cap reason), `ScheduleTimelineEventRenderer.tsx` (removes the old per-badge trailing row, composes the new footer).
