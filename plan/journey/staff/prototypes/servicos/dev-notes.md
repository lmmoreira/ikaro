...
**Real functional gap, not just a doc/prototype gap (found via `/docs-audit` 2026-09-17):** there is no `GET` endpoint anywhere — backend or BFF — to read a service's active intake schema or its version history. `IServiceIntakeSchemaRepository.findActiveByServiceId()`/`findAllByServiceId()` exist at the repository layer but have zero controller callers; `GET /services/:id` doesn't include any intake-schema field. This means the "Formulário de reserva" tab's version-history list and pre-filled question builder can't actually be populated from a real API today — a preceding backend/BFF addition (or an expanded M22-S04 scope) is needed before this tab can be implemented as designed. See `docs/02-DOMAIN_MODEL.md` § Aggregate: ServiceBookingIntakeSchema.
...
**Open design question, not resolved by the above (found by the user, 2026-09-18):** on `03-service-edit.html`, the Profissional row offers both `CUSTOMER_CHOICE` and `AUTO_ANY`/`AUTO_FUNGIBLE_POOL` as radio options, but Sala and Equipamento each show only one static "Sistema atribui automaticamente" radio with no `CUSTOMER_CHOICE` alternative in the DOM — even though `docs/02-DOMAIN_MODEL.md`'s `selectionMode` enum isn't scoped by resource type, and nothing in the docs says room/equipment selection can't be customer-chosen (e.g. picking a specific therapy room). Also unresolved: `AUTO_ANY` vs. `AUTO_FUNGIBLE_POOL` is never a distinct user-facing choice anywhere in the prototype — `docs/27-BUSINESS_LOGIC_REFERENCE.md` ties that distinction to `requiredQuantity`, so the UI may be meant to derive it rather than ask, but that's not stated. Needs a decision during `/story-discovery M22-S04` before `ServiceResourceRequirementsPanel` is built — don't infer either behavior from the current prototype markup.
...
- [ ] The missing intake-schema read path (above) needs a decision during `/story-discovery M22-S04`: fold a new `GET` endpoint into this story's scope, or split it into a small preceding backend/BFF story.
