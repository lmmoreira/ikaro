...
| UC-044 | Manager Views the Resource List | Promoted 2026-08-31 from `docs/discovery/multivertical-booking/` for `M21` (Cluster 1/Foundation). Backend + BFF shipped in `M21-S01`; dashboard UI lands with `M21-S04`. | `manager/resources.md` |
| UC-045 | Manager Creates a Resource | Same promotion as UC-044. Backend + BFF shipped in `M21-S01`; dashboard UI lands with `M21-S04`. | `manager/resources.md` |
| UC-046 | Manager Edits a Resource's Working Hours | Same promotion as UC-044. Backend + BFF shipped in `M21-S01`; dashboard UI lands with `M21-S04`. | `manager/resources.md` |
| UC-047 | Manager Deactivates a Resource | Same promotion as UC-044. Backend + BFF shipped in `M21-S01`; dashboard UI lands with `M21-S04`. | `manager/resources.md` |
| UC-049 | Manager Reactivates a Resource | Same promotion as UC-044. Backend + BFF shipped in `M21-S01`; dashboard UI lands with `M21-S04`. | `manager/resources.md` |
...
| UC-073, UC-077 | Future Commitment Exception (raised + resolved) | Promoted 2026-08-31 from `docs/discovery/multivertical-booking/` for `M21` (Cluster 3). UC-077 is manager-only; UC-073 is system-triggered with no dedicated screen. Draft — not yet shipped, no story assigned. | `manager/scheduling-exceptions.md` |
| UC-075 | System Bootstraps a New Tenant From a Preset | Same promotion. Manager-facing wizard; Presets A/B/C/G only in this cluster. Draft — not yet shipped, no story assigned. | `manager/onboarding.md` |
| UC-099 | Manager Creates or Cancels a Customer Class-Access Contract | Promoted 2026-08-31 from `docs/discovery/multivertical-booking/` for `M21` (Cluster 4). MANAGER-only. Draft — not yet shipped, no story assigned. | `manager/turmas.md` |
| UC-079, UC-080, UC-096, UC-103, UC-104 | Class template CRUD, range cancellation, enrollment views/manual creation | Same promotion. STAFF\|MANAGER, cross-listed — see `staff/use-cases.md`. | `manager/turmas.md` |
